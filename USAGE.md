...
The global **Voice companion** bar can operate Chat and coding-agent Terminals while you speak. Add and enable an OpenAI API key under **Settings -> Providers**, then choose **Start voice** and allow microphone access. The account needs access to `gpt-live-1` and the delegated `gpt-5.6-terra` backend. No separate vBot Agent or speech Task Model needs configuring.

Use a browser over **localhost or HTTPS**. The initial version has no native microphone fallback for a Desktop client connected to a remote plain-HTTP server. Turn off Desktop wakeword Voice before starting the companion. The voice connection remains active across Chat/Terminals navigation; **Mute** keeps it connected, while **End voice** closes it. OpenAI bills connected voice time, including idle time, separately from backend Model usage.

Example requests: "Open Terminals and start four Codex terminals in /home/me/project", "Maximize the second terminal", "Put the fourth terminal first", or "Open Joel's chat and tell him to continue". Codex and Claude Code must already be installed and authenticated **on the vBot server**; working directories refer to that server. The companion can read and send terminal input, select/reorder tiles, and maximize/restore the existing layout. It uses exact Session targets for chat messages and preserves normal Queue behavior.

While listening, it receives new vBot Run completion/failure updates, can relay Agent questions, and can summarize replies on request. It does not infer completion from quiet terminal output. Ending voice does not stop coding terminals or vBot Runs. Captions and action counts in **Details** are temporary; sent chat messages remain in their normal Sessions.
