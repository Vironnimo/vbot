...
Live voice can operate Chat and coding-agent Terminals while you speak. Enable **Live voice** under **Settings -> Tools & Media -> Voice** to show its compact control at the bottom of the Main menu, above the microphone and connection indicators. Add and enable an OpenAI API key under **Settings -> Providers**, then choose **Start Live** and allow microphone access. The same button becomes **Stop Live** while connected. Disabling Live voice in Settings hides the control and ends an active voice connection. The account needs access to `gpt-live-1` and the delegated `gpt-5.6-terra` backend. No separate vBot Agent or speech Task Model needs configuring.

Use a browser over **localhost or HTTPS**. The initial version has no native microphone fallback for a Desktop client connected to a remote plain-HTTP server. Turn off Desktop wakeword Voice before starting the companion. The voice connection remains active across Chat/Terminals navigation; **Stop Live** closes it. OpenAI bills connected voice time, including idle time, separately from backend Model usage.
