...
reads the Provider-owned automatic observations in ``provider-usage.db``, while
its ``.clear`` sibling explicitly deletes them; both run their database work on
that database's worker pool, off the Event Loop. All three remain separate from
...
async def _provider_usage_history(state: Any, params: JsonObject) -> JsonObject:
    ...
    report = await _usage_service(state).history_report(since=since, until=until)
    return report.to_dict()


async def _provider_usage_history_clear(state: Any, params: JsonObject) -> JsonObject:
    _reject_unsupported(params, set(), "provider.usage_history.clear")
    result = await _usage_service(state).clear_history()
