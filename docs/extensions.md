...
Start with `capture` to see the desktop. On Windows, `monitors` discovers displays; `monitor` selects one and omission captures all displays. `windows` lists native windows for optional `pid`/`window_id` targeting. Browser windows use the same mouse and keyboard actions as other applications.

`move`, `click`, `type`, `key`, `scroll`, and `drag` cover visible input. Set `apply: true` to execute; omission previews the action. Foreground input is the default and may move the real pointer or change focus. Windows timed key holds and drags accept `duration_ms` up to 2000. Window element actions, menus, launch, resize, and verification remain available.

For background window control, use `foreground: false` on capture and input, with `pid` and `window_id`. Cua uses window-directed input and its own window images; its `max_image_dimension` must be zero to preserve original coordinates. Capture again when switching delivery modes. Support depends on the application: an independent cursor overlay is not an independent Windows input session, and some applications can activate themselves. An observed target activation is reported and stops remaining sequence steps. The Extension never retries a background action in the foreground automatically. Desktop input, timed holds, and held modifiers remain foreground operations.
...
On Windows, press **Esc twice within 600 ms**, releasing it between presses, to stop computer control from any foreground application. Detection is armed from the first Computer Use action until that Run ends or closes its desktop session, including time spent waiting for the Model; merely having the Extension loaded does not arm it. Single presses, key auto-repeat, and injected Agent keystrokes do not trigger stop. The listener passes keys through to the foreground application and dispatches interruption outside the keyboard hook. A pending double-Esc immediately blocks further Tool input while the stop worker drains.

Chat's existing composer shows a **Stop computer control** icon only during computer control, and an **Allow computer control** icon while stopped. There is no additional app-wide bar. The stop tooltip explains double-Esc and reports unavailable native detection. Stop, Run cancellation, and double-Esc interrupt the owned input worker and release held keys/buttons. Stop stays latched across reconnects and restarts until the user allows control after the active call drains; the Agent cannot resume itself. Already delivered input cannot be rolled back; unrelated Tools such as Bash remain outside this interlock.
