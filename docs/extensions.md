...
The global **Stop computer control** button, Run cancellation, and Windows **Ctrl+Alt+Pause** stop the owned input worker and interrupt native action loops. Held keys and mouse buttons are released. Stop stays latched across reconnects and restarts until the user selects **Allow computer control** after active work drains. The Agent cannot resume itself. The UI reports unavailable global shortcuts, including registration conflicts. Already delivered input cannot be rolled back; unrelated Tools such as Bash remain outside this interlock.
