...
The bundled `computer_use` Extension supplies the opt-in `computer` Tool on the vBot server host. Install stable `cua-driver` 0.23.2 or newer, set its `max_image_dimension` to 0, reload Extensions, and explicitly enable `computer` for the Agent under Tools & Skills. A Project Agent also needs it in the Project Tool Whitelist and its Tool override. The Extension checks prerequisites and owns a persistent MCP connection; it never installs, updates, or reconfigures the driver. Windows does not require a separately started daemon for this connection. Other platform prerequisites still apply.

Window discovery, capture, click/type/key/scroll/drag, field values, menus, application launch, window resizing, and state verification share one Tool. Desktop capture provides the primary-display fallback. Browser actions add semantic tab capture, navigation, input, pointer gestures, JavaScript dialogs, uploads, and downloads using exact session-owned target/tab identifiers. Existing-profile attachment uses the driver's explicit preparation flow; isolated preparation requests a separate driver-owned browser.

Input defaults to preview; set `apply: true` to execute. Foreground delivery is explicit. Capture before input. Applied input returns a fresh observation automatically; `sequence` groups up to eight known click/type/key steps with one final observation and reports partial completion on failure. A sequence may select a target only in its first step. Verify the resulting state before repeating uncertain input.

Screenshots default to an overview bounded to 1600 pixels on the long edge and 1.5 million pixels. The unchanged original PNG remains available. Use `zoom` for native-resolution detail or `resolution: original` for the full image. Coordinate actions require the current `view_id` and coordinates from that displayed image; the Extension maps them back to native pixels (and CSS pixels for browser input). `mode: ax` skips image capture for windows/browser tabs; `query` and the window `limit` reduce accessibility output.
