...
class DecisionService:
    ...
    def __init__(
        ...
        self._workers = BoundedWorkerPool(name="decisions", max_workers=1)
    ...
    async def list_experiments(self) -> dict[str, Any]:
        return {
            "experiments": await self._workers.run(self._store.list),
            "available": self.available(),
        }

    async def get_experiment(self, identifier: str) -> dict[str, Any]:
        return await self._workers.run(self._store.get, identifier)

    async def save_experiment(
        self, draft: Any, identifier: str | None = None, revision: int | None = None
    ) -> dict[str, Any]:
        return await self._workers.run(self._store.save, draft, identifier, revision)

    async def delete_experiment(self, identifier: str, revision: int) -> None:
        await self._workers.run(self._store.delete, identifier, revision)

    async def history(self, identifier: str, before: int | None = None) -> dict[str, Any]:
        return await self._workers.run(self._store.history, identifier, before)

    async def evaluation(self, identifier: str) -> dict[str, Any]:
        return await self._workers.run(self._store.evaluation, identifier)
    ...
    async def _start(
        ...
        async with self._admission:
            previous = await self._workers.run(self._store.request, request_id)
            if previous:
                if (
            ...
            record = await self._workers.run(
                self._store.begin, identifier, revision, request_id, snapshot
            )
    ...
    async def _execute(self, record: dict[str, Any]) -> None:
        ...
        if not self._closed:
            await self._workers.run(
                self._store.finish, record["id"], status, result=result, error=error
            )
    ...
    async def cancel(self, identifier: str) -> dict[str, Any]:
        ...
        if task is not None and not task.done():
            ...
            await self._workers.run(self._store.finish, identifier, "cancelled")
