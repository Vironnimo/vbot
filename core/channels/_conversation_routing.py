...
class ChannelSessionRouting:
    ...
    def _resolve_active_session_id(self, agent_id: str, conversation_key: str) -> str:
        ...
        # deleted target is fine -- routing re-creates it empty downstream,
        ...
        def update(metadata: dict[str, Any]) -> None:
            ...
            if track_participant and conversation.kind == "group":
                ...
                if not isinstance(participants, dict):
                    participants = {}
                # Sighting times belong to the Channel access record, not to
                # every routed message of the Session.
    ...
    def _set_active_session_pointer(self, agent_id: str, anchor: str, new_session_id: str) -> None:
        ...
        active session); creating a missing anchor is a defensive floor for the
        rare case where it does not.
        """
        self._chat_sessions.ensure_metadata(
            _session_address(agent_id, anchor),
    ...
    def _point_conversation_at_session(
        ...
        previous, _updated = self._chat_sessions.ensure_metadata(
