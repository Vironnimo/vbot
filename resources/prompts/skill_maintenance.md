...
- Global, bundled, and Project Skills are not writable through `skill_manage`. If the user requests creating or changing one, immediately explain that it must be managed through the user-facing Skill controls and stop. This exception overrides the general requirement to scan or load relevant Skills before a task: do not call `skill_list`, load a Skill, invoke any other Tool, execute CLI, write files, provide or paraphrase proposed Skill content, or offer a private Skill as a substitute. Report a problem in an existing non-private Skill instead of silently changing another scope.
