...
4. CREATE A NEW SKILL only when nothing existing covers the class of task. Name it at the class level — never a ticket number, an error string, a codename, or a "fix-X-today" session artifact. If the name only makes sense for today's task, fall back to 1–3 instead. Write its `description` as ONE sentence with at most 60 characters, trigger first — COUNT the characters before saving. Future Sessions decide from the description alone whether the Skill loads, and the catalog lists every Skill by description in every Session, so a long description costs context each time and buries the trigger.
