...
| `session-snapshots/` and `session-recovery.json` | Verified Session database backups and recovery incident state | `vbot session-store ...`; Project and Identity Session history both live in `sessions.db` |
...
| `artifacts/speech/` | Durable generated speech artifacts | Use the returned artifact reference; generated images default to the caller’s Workspace or Project cwd under `image-gen/` |
| `artifacts/models/` | Complete runtime Model DB published by Model refresh | `vbot model list/refresh`; do not hand-edit generated catalogs |
...
4. Treat `sessions.db`, `session-store.json`, session snapshots and quarantine bundles, recall indexes, Model DB files, OAuth state, atomic staging, Queue state, and active Runtime files as system-owned. Inspect read-only unless a documented recovery procedure explicitly requires otherwise.
