...
```bash
vbot channel add <channel-id> --platform telegram|discord --agent <agent-id> --token-env <ENV_VAR> [--dm-scope <scope>] [--allow <chat-id> ...]
vbot channel list
vbot channel status <channel-id>
vbot channel update <channel-id> [--platform ...] [--agent ...] [--token-env ...] [--dm-scope ...] [--allow <chat-id> ...] [--enabled true|false]
...
- Group gating fields (`response_mode`, `mention_patterns`, `owner_user_ids`, `observe_unaddressed`) have no CLI flags — configure them through the WebUI channel editor or the `channel.update` RPC.
