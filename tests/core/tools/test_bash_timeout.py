...
@pytest.mark.asyncio
@pytest.mark.parametrize("mode,depth", [("foreground", 0), ("foreground", 1)])
...
@pytest.mark.asyncio
async def test_omitted_timeout_leaves_background_commands_unbounded(manager, tmp_path, monkeypatch):
    monkeypatch.setattr(bash_module, "_shell_argv", python_command)
    monkeypatch.setattr(bash_module, "DEFAULT_TIMEOUT_SECONDS", 0.5)
    result = await asyncio.wait_for(
        bash_handler(
            make_context(tmp_path),
            {"command": "import time; time.sleep(2)", "mode": "background"},
            manager,
        ),
        10,
    )
...
@pytest.mark.asyncio
async def test_explicit_timeout_still_bounds_background_command(manager, tmp_path, monkeypatch):
    monkeypatch.setattr(bash_module, "_shell_argv", python_command)
    result = await asyncio.wait_for(
        bash_handler(
            make_context(tmp_path),
...
@pytest.mark.asyncio
async def test_background_timeout_completion_note_explains_deadline(manager, tmp_path, monkeypatch):
    bodies: list[str] = []
    ...
    assert "tool timeout of 0.5 s elapsed and killed the process" in body
    assert "timeout: 0 for no limit" in body
    assert "Exit code:" not in body


@pytest.mark.asyncio
async def test_handoff_note_states_effective_timeout(manager, tmp_path, monkeypatch):
    monkeypatch.setattr(bash_module, "_shell_argv", python_command)
    result = await asyncio.wait_for(
        bash_handler(
            make_context(tmp_path),
    ...
    assert "A tool timeout of 120 s still applies" in result["data"]["handoff_note"]
    ...
    assert "No tool timeout applies to this command." in result["data"]["handoff_note"]
    await kill_background(manager, result)
