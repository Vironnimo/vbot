...
async def test_analysis_tool_retains_exact_images_as_presentation_only_snapshots(
    ...
    from core.tools.image import register_analyze_image_tool
    ...
    async def send(*args: Any, **kwargs: Any) -> Any:
        # Source changes after loading must not change the captured preview.
        first.write_bytes(b"changed")
        second.unlink()
        response = await original_send(*args, **kwargs)
        if provider_fails:
            raise ImageExecutionError("test failure")
        return response

    adapter.send = send  # type: ignore[method-assign]
    ...
    register_analyze_image_tool(registry, service, attachment_store=store)
    context = ToolContext(
        agent_id="agent",
        session_id="session",
        run_id="run",
    ...
    arguments = {"prompt": "Compare", "images": ["first.png", "second.png"]}
    result = await registry.dispatch(context, arguments)
    assert result["ok"] is not provider_fails
    assert result["artifacts"] == []
    display = registry.display_for_call("analyze_image", arguments, context=context, result=result)
    ...
    previews = restored.tool_display["images"]
    assert [image["filename"] for image in previews] == ["first.png", "second.png"]
    stored_bytes = [
        Path(store.get(image["attachment_id"]).file_path).read_bytes() for image in previews
    ]
    assert stored_bytes == originals
    sent_images = adapter.requests[0]["messages"][1]["content"][1:]
    assert stored_bytes == [base64.b64decode(image["base64"]) for image in sent_images]
    assert isinstance(restored.content, str)
    assert "attachment_id" not in restored.content
