...
async def test_build_session_chunks_skips_chunks_with_no_embeddable_text() -> None:
    ...
    messages = [
        ChatMessage.run_summary(run_id="r1", status="completed", timing={}, model_step_count=0),
        ChatMessage.run_summary(run_id="r2", status="completed", timing={}, model_step_count=0),
    ]
...
async def test_vector_backend_never_surfaces_run_summary_as_a_match(tmp_path: Path) -> None:
    ...
    session = sessions.create("coder", session_id="mixed")
    session.append(
        ChatMessage.run_summary(run_id="r1", status="completed", timing=timing, model_step_count=0)
    )
    session.append(ChatMessage.user("I love bananas and fruit", timestamp=timestamp(1)))
