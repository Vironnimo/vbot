...
## 2026-06-30 — Addendum: skill_manage dispatch gap closed (skill-system bug audit)

Follow-up to the "Agent-authorable skills" entry above, item 1. `effective_agent_allowed_tools` (`core/tools/availability.py`) now takes the agent's `workspace` and strips `IDENTITY_ONLY_TOOLS` (`skill_manage`) for an empty one, mirroring the existing `memory`-mode exclusion. Until this fix, that function (which feeds the dispatch-time allowlist `ToolRegistry.dispatch` actually enforces) did not know about `IDENTITY_ONLY_TOOLS` at all — only the prompt layer's `_apply_identity_only_tool_visibility` did, which controls what gets advertised to the model, not what is permitted to execute. Practical exploitation was already low (no provider adapter lets a model call an undeclared tool), but `project.allowed_tools` had no server-side check against `"skill_manage"` either (only the WebUI editor hid the checkbox), so the only thing standing between a widened Tool Whitelist and a real write into `<data_dir>/agents/<id>/skills/` was that prompt-layer hiding. This implements the "or block `skill_manage` for config agents" alternative item 1 named — the shared bare-id keying of the skill home itself (an identity agent and a same-named config agent resolve to the same path) is otherwise unchanged.

## 2026-07-02 — Full backend gate: failure from concurrent uncommitted usage-feature work

While closing the skill-resource-directory task (activation payload now announces the absolute skill directory, `{baseDir}` substitution, `assets/` resource dir), the full backend gate failed one test that is unrelated to that change: `tests/server/test_server_integration.py::test_bootstrap_agent_and_current_history` expects the `chat.history` result without a `session_usage` block, but the working tree contains uncommitted in-progress token-usage work (`core/chat/usage.py`, `server/rpc/chat_methods.py`, event bridge, WebUI tooltip files) that adds `session_usage` to the response and did not update this integration test. The skill task was committed with explicit paths only; the usage-feature files stay uncommitted in the tree. Whoever finishes the usage feature must update that integration test (and re-run both full gates); gate auto-fixes already in the tree (ruff format on `tests/core/runs/test_runs.py`, prettier on `webui/src/lib/__tests__/tokenUsageTooltip.test.js`) were kept and left uncommitted with that work.
