...
**Dependencies:** Groups `server`, `cli`, `desktop`, `local-speech`, `local-tts`, `dev` in `pyproject.toml`; the WebUI's in `webui/package.json`. Optional local STT uses native Transformers/PyTorch on the server, with Qwen3 ASR and Parakeet adapters and in-app dependency setup owned by `core/model_tasks/speech_setup.py`. Settings installs the shipped extra into the running server's interpreter; the server extra includes the existing `psutil` lifecycle dependency for restart. Local TTS uses isolated managed SDK environments for Qwen3-TTS and Chatterbox Multilingual V3; `speech_worker.py` is their standalone child entry point. See `USAGE.md` -> Local speech recognition / synthesis.
