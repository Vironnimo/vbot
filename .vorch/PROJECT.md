...
| `.vorch/domain-maps/statistics.md` | `core/statistics/` | Disposable incremental SQLite projection over canonical Sessions, run-summary segmentation, `statistics.report` RPC, Statistics tab |
...
```
`create` prints the worktree `path`, assigned server and fake-Provider ports, data dir, and URL. The primary development checkout reserves `8421`, so generated worktrees start at `8422`; port selection and Worktree initialization hold a cross-process repository lock so concurrent creates cannot reserve the same pair. Each new Worktree's Settings also seed the shared keyless fake Custom Provider and chat/fallback/image/speech Models. Start the complete UI test instance with `python scripts/test-env.py start`, which owns both the fake Provider and vBot process lifecycle for that data directory and records process identity as PID plus creation time. `delete` stops those owned processes before removal and reports any service, Worktree, data-directory, or branch cleanup failure instead of claiming success; `delete --force` additionally discards uncommitted worktree changes. If worktree commands fail or behave unexpectedly, read `scripts/README-worktree.md`.
...
- **Two complete snapshots, one active generated Model catalog.** The tracked system root is `resources/models/`; normal `vbot model refresh` atomically publishes a complete runtime snapshot at `<data_dir>/artifacts/models/`; the maintainer script refreshes the system root only as an explicit Model DB maintenance task, independently of releases. Both snapshots include generated catalogs, raw files, bundled Override snapshots, and a compatible timestamped manifest. *Load* chooses the newer root as a whole for generated canonical and Provider data (system wins a tie), then applies the current bundled system Overrides from `resources/models/` as the authoritative hand layer; generated files are never mixed across roots. Details in `.vorch/domain-maps/models.md`.
- **Generated catalogs are refreshable; bundled Overrides are editable load-time inputs.** `<provider>.json` and `models.json` are generated projections and are not a manual edit surface; `models.overrides.json` and `<provider>.overrides.json` are automatically discovered from `resources/models/`, require no registration, and apply only during Load, never baked into generated files. A normal runtime Refresh synchronizes the complete bundled Override set into its published snapshot so a later refresh preserves additions, changes, and removals. A discoverable Provider fact belongs in Adapter normalization/runtime policy; a durable, externally verified fact the feeds do not expose belongs in the matching Override layer.
