...
**Tool contracts:** `core/tools/contracts.py` compiles canonical Draft 2020-12 input and success-data schemas with `jsonschema`; `ToolRegistry` validates calls before handlers and successful results afterward. Agent-facing Tools expose one closed flat object: direct parameters for one behavior or target selection, and a required top-level discriminator only for genuinely different actions or execution contracts. Every discriminated schema is a union of closed flat branches, so each variant advertises and accepts only its own fields and structurally requires everything it needs. Nested `request.operation` contracts are retired and unsupported by current dispatch; the WebUI may still recognize them solely to label historical persisted calls. Provider adapters render the canonical input through explicit strict/best-effort profiles without weakening runtime enforcement, and sibling Tool Calls execute concurrently by default within shared limits unless a registered Tool explicitly declares a serial barrier.
