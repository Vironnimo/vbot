...
**Tool contracts:** `core/tools/contracts.py` compiles canonical Draft 2020-12 input and success-data schemas with `jsonschema`; `ToolRegistry` validates calls before handlers and successful results afterward. Multi-operation Tools use a closed `request.operation` discriminator. Provider adapters render the canonical input through explicit strict/best-effort profiles without weakening runtime enforcement, and sibling Tool Calls are serial unless their registered Tools explicitly opt into safe parallel execution.
