...
- **Right pane**: Content area for the selected section; Agents, Projects, Cron, and Settings introduce a 216px Secondary bar between the Main menu and their detail content
...
| **Projects** | Project list loads; Add project updates and selects the new entry without a global refresh; detail shows display name, default agent/model/temperature/thinking, auto-load files, and the tool & skill whitelists (the project ceiling); the single Rescan repository action refreshes Team, scan report, and Skills together |
| **Cron** | Job list or the "No scheduled jobs" empty state; create/edit a scheduled job; completed jobs are hidden from the list |
| **System Prompt** | Sortable block list renders: each block row has a drag handle (reorder via mouse drag **and** keyboard) and an on/off toggle that persists immediately, plus an "appears when: <owner>" label; editable text blocks show an inline autosaving textarea with per-block Reset, while data/auto-list blocks (SOUL, project files, tool/channel/skill lists) are read-only with a collapsed preview; a prompt-scope selector (Default / per-agent) shows "inherited" badges in an agent scope; "New block" creates a custom block and custom blocks have Remove; a "reset order & visibility to default" action exists; the preview (agent picker, token count, Copy) mirrors all blocks including extension-contributed ones and refreshes automatically after successful mutations |
| **Settings** | One scrolling document with a left index: a search field plus a grouped table of contents (Connect / Models / Behavior / System). All sections are always rendered as cards: Providers, Channels, Extensions, Agent defaults, Specialized Models, Compaction, Recall, Web Search, Sub-Agents, Reflection, Skills, Appearance, Voice (Desktop only), Debug, Server info. Index click scrolls to the section and highlights it; scrolling moves the highlight (scrollspy); the search filters rows live with a match count, dims non-matching index entries, matches text inside collapsed disclosure rows, and clearing it restores everything. Dense detail collapses behind ▸ disclosure chevrons that open an indented sub-block: each provider is one collapsed row (name, summary status chip, ▸) hiding its connections, accounts (credential status, Replace key/Remove, Add account), Add connection, and the local-model-context editor; Specialized Models hides per-target options; Extensions hides each config form. Providers keeps Add provider and Update Model DB in the section toolbar; Appearance has language and chat-width with a Save button |
...
| **Statistics** | On-demand report (nothing extra stored) with tabs Overview / Usage / Runs & errors / Tools / Skills / Limits; counts and charts render; the report Refresh re-aggregates. Limits has no Refresh button: opening it loads live Provider usage, visible state updates every 10 seconds without overlapping requests, switching away or hiding the page pauses polling, and returning refreshes immediately |
