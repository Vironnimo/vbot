...
**Not:** A bare cwd, a Workspace, or an Agent. The cwd is one field of a Project; a Workspace is an Agent's identity home in the data-dir, never the project repo. vBot reads the repo (to discover the Team) but never writes it — runtime data (the project anchor, Sessions) lives in the data-dir. The project's data-dir home (Project Anchor) and its trust-boundary ceilings (Project Tool/Skill Whitelist, Ceiling) are defined in `projects.md`.
...
**Not:** A Project Agent (a born-in-project team member — defined in `projects.md`), nor a rooted identity agent (whose home *is* the project, cwd = repo, project files in the system prompt). A visit is a reach-in from a home elsewhere.
