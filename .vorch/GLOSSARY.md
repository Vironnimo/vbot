...
## Project Context
**Definition:** The current instructions, absolute cwd, and Project Skill references that tell an Agent how to work in a registered Project. It is supplied automatically when that Project is the Agent's working Project or loaded explicitly by an Identity Agent through the `project` Tool.
**Not:** An Agent type, Project membership, Rooting, a cwd change, or persistent Project state; an explicit load changes none of those, and every one-shot `bash` call must set `workdir` again.
