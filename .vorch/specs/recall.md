...
- **Per-session granularity:** one vector per session. The session's concatenated searchable text (reusing the same search-text extraction the JSONL backend builds) is truncated to the bound model's `context_length` before embedding. Truncation is a **character heuristic** (`VectorStore.truncate_to_input_limit`: a conservative chars-per-token budget with headroom), so it cannot guarantee the *token* count — dense/German/code text can still overflow. `_run_embed` therefore catches the provider's context-length rejection and **halves the over-long inputs, retrying** (up to `_EMBED_OVERFLOW_RETRIES`) until they fit; only then does it fall back. The first-backfill truncation resolves `context_window` from the binding header (the `_resolved_header` dimension is unknown until the first embed).
