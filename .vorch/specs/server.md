...
- `settings.set_key` accepts `{ key, value }`, where `value` may be any JSON type including `null`, updates one raw top-level `settings.json` key through the storage settings transaction, persists it, and returns `{ settings }` with the updated raw dict.
