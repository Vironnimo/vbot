...
- `await TriggerService.retry_run(agent_id: str, session_id: str) -> Run` — retries the latest user turn in an existing Session through the injected ChatLoop. This does not append a new user message and does not queue behind an active Run.
