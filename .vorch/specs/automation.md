...
- `await trigger_run(agent_id: str, message: str | list[ContentBlock], session_id: str | None = None, *, internal: bool = False) -> Run` - creates a Session when `session_id` is omitted, starts the Run on the trigger ChatLoop, catches `ActiveRunError`, queues through `ChatLoop.queue_run(...)`, and awaits the queued item future. `internal=True` is for string system-reminder notes only: it persists the message as a Session note instead of a visible user turn and preserves that flag when queued.
...
- `await compact_session(agent_id: str, session_id: str) -> str` - command accessor helper that returns user-facing reply text. It uses `chat_run_manager.active_run(...)` to refuse active Sessions, loads normal compaction settings, resolves an optional `summary_model` best-effort, appends one checkpoint on success, closes any opened adapters, and converts compaction failures into `"Compaction failed: ..."` replies.
...
- `status` is `active`, `paused`, or `completed`. Paused and completed jobs do not own scheduler tasks, and completed jobs cannot be re-enabled or paused.
