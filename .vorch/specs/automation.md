...
Automation owns kernel-level primitives for starting Runs without going through the normal WebUI send flow plus lightweight scheduling that decides when to fire those triggers. `TriggerService` starts a Run immediately when the target Session is idle and queues triggers FIFO when a Session already has an active Run. `CronService` persists scheduled jobs under `<data_dir>/cron/jobs.json`, manages per-job asyncio tasks, and fires `TriggerService.trigger_run(...)` when jobs become due.
