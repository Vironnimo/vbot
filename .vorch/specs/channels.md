...
- Channel session metadata lives in `<data_dir>/agents/<agent-id>/sessions/<session-id>.meta.json` beside the JSONL transcript and is owned by `ChatSessionManager`. Channel-owned keys are `source_channel_id`, `platform`, `platform_conv_id`, `last_reply_target: { channel_id, platform_target }`, and — for group conversations only — `participants: { <user_id>: { display_name, last_seen_at } }` (engine-maintained on every inbound message; `last_seen_at` is UTC ISO 8601; existing entries are preserved and merged; outbound sends never write it).
...
- `ConversationTransport` is the `typing.Protocol` the adapter implements so the engine stays platform-free: `platform_display_name` (used to render reply/reminder text), `send_text(platform_target, text)`, `activity_indicator(platform_target)` (async context manager), and `build_media_blocks(raw_message) -> list[ContentBlock]`.
