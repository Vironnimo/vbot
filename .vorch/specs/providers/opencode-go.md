...
- When a catalog entry has a known `max_output_tokens`, OpenCode Go uses it as the request `max_tokens` unless the caller supplied an explicit output limit. Unknown (`null`) catalog limits fall back to the provider request default.
