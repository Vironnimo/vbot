...
- Agent models and task targets may pin an account via the suffix grammar `<provider>/<model>::connection[:account]`; the suffix passes through verbatim into the compositional connection id. `TaskModelTargetRef.account_id` is empty when not pinned, and target listing/expansion stays connection-level (no per-account target cross-product).
