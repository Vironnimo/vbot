...
- `Run.request_cancel()` marks cancellation requested, schedules registered sync/async cancel callbacks, and cancels the background executor task if one exists.
...
- `ChatRunManager.enqueue(...)` starts immediately when idle and resolves the item future at once; otherwise it appends a FIFO `QueuedRunItem` for that `(agent_id, session_id)`.
...
- `ChatRunManager.add_run_started_callback(callback)` registers a process-local observer for every Run the manager starts, including queued/internal Runs; the returned callable unregisters it.
...
- Only one Run may be active per `(agent_id, session_id)`; Runs in different Sessions may execute in parallel.
