...
- `_message_to_request_dict()` strips `usage` and `reasoning_meta` from assistant messages before they are sent to provider APIs. Readable `reasoning` is preserved in canonical assistant history so reasoning-aware adapters can round-trip it when their wire format requires it.
