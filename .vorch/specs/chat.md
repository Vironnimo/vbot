...
- `CommandDispatcher` — shared pre-run slash-command router for pure-text messages. Recognized built-ins either return a handled result with optional reply text/data or a command action for accessor-level execution; unknown slash text falls through so normal chat behavior, including skill activation, stays intact. Current built-ins are `/compact`, `/help`, `/new`, `/retry`, `/status`, and `/stop`.
