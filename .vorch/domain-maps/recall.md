...
`core/tools/session_search.py` owns Tool definitions, validation, visibility and execution; `_session_recall_results.py` holds internal result projection, descriptors and lossless read continuations under the same owner.
...
- All built-in and Extension backends implement `RecallBackend.search_capabilities` and `search_page`; registry construction rejects missing methods or invalid capability values. Synchronous search methods run off the event loop. Session listing and exact reads remain Tool-owned; the old browse/overview/search/scroll API and payload adapters are removed.
