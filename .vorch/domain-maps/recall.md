...
- `RecallRequest` carries normalized tool arguments: `agent_id`, optional `project_id`, optional `session_id`, optional `around_message_id`, optional `query`, time bounds, roles, match mode, limit, context size, bookend size, and sort.
- **Project scoping (per request).** A recall run searches **its own** scope: `project_id` is filled from `context.project_id` in `core/tools/session_search.py` (the running Run's project, not a tool argument), so a project agent's recall sees that project's sessions and an identity agent's recall sees the global ones. All backends thread `project_id` into `ChatSessionManager.list_with_metadata(...)`/`get(...)`, and the derived indexes key by `(project_id, agent_id, session_id)` so a global and a project session with the same UUID never collide or overwrite each other. `project_id=None` (identity) maps to the empty-string scope sentinel and uses the global sessions exactly as before. The indexes are disposable, so the added scope column shipped as a plain schema bump (vector → v4, FTS → v2) — a mismatched on-disk index is dropped and rebuilt, no migration.
...
- `SupportsSessionRemoval` is an optional `@runtime_checkable` capability `Protocol` with `remove_session(agent_id, session_id, project_id=None)`. The derived-index backends (`sqlite_fts`, `vector`, `hybrid`) implement it to evict one Session's rows immediately; `jsonl_scan` deliberately does **not** (the live scan has no index — an archived Session is already absent from the directory it reads). `Runtime.remove_session_from_recall(...)` checks `isinstance` before calling, so a backend without removal falls back to self-healing; index I/O errors there are logged, not fatal (the index is disposable). `session.delete` calls it so a deleted Session leaves search at once instead of waiting for the next on-search staleness reconcile.
...
- **Per-chunk granularity:** multiple vectors per session, built by walking a session's messages in order and packing consecutive messages' search-text into chunks (target: `_CHUNK_TARGET_CHARS = 1500` characters, ~500 tokens). A `_CHUNK_OVERLAP_MESSAGES = 1` boundary overlap provides context across chunk boundaries. Each message's contributed text is capped at `_PER_MESSAGE_CHAR_CAP = 2000` characters before packing; a single message larger than the target becomes its own chunk, hard-capped via `VectorStore.truncate_to_input_limit`. The schema is chunk-keyed (`chunks` metadata table, `UNIQUE(project_id, agent_id, session_id, chunk_index)`). `PRAGMA user_version` tracks on-disk index validity and is bumped on a build/index-policy change as well as a DDL change (currently v4; v4 added the `project_id` scope column to the key — see Project scoping above; v3 had stopped indexing empty-text chunks). The store drops and rebuilds on a version bump automatically.
