...
- **READ THE PROVIDER'S OFFICIAL DOCS THOROUGHLY BEFORE ANY WIRE WORK - ALL relevant pages, not a search-result skim.** This is a hard prerequisite, established after a month of reasoning-replay debugging (2026-08) where unverified assumptions caused wrong pins, wrong conclusions, and repeated rework. Minimum bar before touching or judging any Provider wire behavior: read the API reference for every endpoint the Provider offers, every capability page that touches the task (e.g. thinking/reasoning, streaming, tool calling), and the compatibility pages (OpenAI/Anthropic/Responses flavors). Providers routinely expose MULTIPLE wires with different contracts (Ollama: four - native `/api/chat`, OpenAI chat, Responses, Anthropic-compatible `/v1/messages`; only the last documents thinking-block round-trip in history). "The docs say nothing about X" is only a valid conclusion after reading all pages; silent stripping/acceptance of fields must be measured AND checked against the documented contract. Record doc-read date + pages in the measurement notes alongside live probes.
- Provider compatibility evidence must name the exact Connection/wire, endpoint, request control field, response carrier field, materially distinct request shape, and verification date. When whether an input field reaches the Model is in question, require a controlled comparison of Provider-reported input tokens for identical requests with and without the field plus a visible-content accounting control; repeat it for request shapes whose contract differs, such as Tools present versus absent. Behavioral recall, a successful HTTP status, emitted output, serializer unit tests, and another Agent's catalog are useful diagnostics but are not transport proof.
...
- `ProviderAdapter.estimate_request_input_tokens()` is the single Adapter-owned seam for local Context budgeting: output-limit clamps, Chat's measured pre-send guard, and automatic Compaction all ask it for the selected Model's actual wire footprint. The default counts Chat Completions-shaped messages; Responses-family routes (OpenAI, xAI, GitHub Copilot, OpenCode Go) count rendered input items, instructions, and Tool definitions with `estimate_responses_input_tokens` (`github_copilot_responses.py`). This avoids counting redundant persisted `reasoning_meta` copies (`response_output`, `reasoning_items`, `encrypted_content`) or canonical fields that do not reach a stateless Responses wire. Chat takes the larger of this wire estimate and any measured Provider Context projection, without adding already-measured System Prompt or Tool overhead a second time.
