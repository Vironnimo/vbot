...
- Native Reasoning replay uses one explicit precedence chain: a Model's top-level `reasoning_replay` override wins, then the root `reasoning_replay` in its `<provider>.overrides.json`, then the system default `full_history`. Provider-specific wire selectors remain scoped to the resolved Connection/Wire plus Model, and mechanics stay in the Adapter. Do not hide replay policy inside provider metadata or adapter-specific fallback sets, add generic Model fields for one Provider's protocol quirk, or route Provider Models in Runtime/Chat.
