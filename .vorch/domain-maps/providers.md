...
- **Before changing or judging any Provider wire behavior, read `providers/compatibility-verification.md` and the Provider-specific reference.** Complete official documentation review plus controlled live evidence is a hard prerequisite; search snippets, accepted payloads, behavioral recall, serializer tests, generated catalogs, and third-party implementations are not transport proof by themselves.
...
- Adapters rebuild authorization headers inside retry attempts so a refreshed OAuth token is used after backoff. OAuth HTTP consumers share `OAuthRequestRecovery` in `token_getter.py`; Provider status classification and the OAuth flow distinguish token rejection from entitlement before one renewal (see `providers/connections.md`). Do not cache raw OAuth tokens outside `TokenGetter`.
- `NetworkError` is retryable and remains Provider-neutral. Chat exhausts same-Model recovery first, then its normalized network interruption may advance an explicitly configured Model fallback chain because a different route can reach another host. Only `ProviderStreamingUnsupportedError` permits Chat's streaming-to-nonstreaming fallback.
- `ProviderAdapter.estimate_request_input_tokens()` is the single Adapter-owned seam for local Context budgeting: output-limit clamps, Chat's measured pre-send guard, and automatic Compaction all ask it for the selected Model's actual wire footprint. Compatible Chat and Messages adapters count their rendered message and Tool representations; Responses-family routes (OpenAI, xAI, GitHub Copilot, OpenCode Go, OpenRouter) count rendered input items, instructions, and Tool definitions with `estimate_responses_input_tokens` (`github_copilot_responses.py`). This avoids counting redundant persisted `reasoning_meta` copies (`response_output`, `reasoning_items`, `encrypted_content`) or canonical fields that do not reach a stateless Responses wire. Within a stable Run request scope, Chat anchors to measured Provider input and estimates only request changes. `request_input_budget` scopes that projection to the send and its inner wire routing, so output clamps do not replace it with a biased full estimate. Without a compatible anchor, Chat and standalone Adapter requests use the full wire estimate. The output-capacity reserve remains separate (`chat/usage.md`, `tests/core/chat/test_chat_loop_usage.py`, `tests/core/providers/test_adapter.py`).
...
- Verifying or revising a Provider/Connection/Model's live compatibility, endpoint choice, Reasoning control/replay, Tool interaction, Usage, media, sampling, Context, output limit, or error behavior -> `providers/compatibility-verification.md`
...
- Changing OpenCode Go's per-Model Chat/Messages/Responses routing or Reasoning controls -> `providers/opencode-go.md`
