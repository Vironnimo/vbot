...
- `core/providers/tool_schema.py` owns canonical-to-wire Tool rendering through the `openai_non_strict` and `best_effort` profiles. vBot never enables Provider strict mode and never rewrites canonical Tool schemas to satisfy a Provider's strict-schema subset. OpenAI requests explicitly send `strict: false` because Responses may otherwise normalize an omitted flag into strict mode; other Providers receive the unchanged canonical schema without a strict field. Runtime argument validation remains authoritative on every wire.
...
- Provider strict mode is forbidden. OpenAI wire builders must retain the explicit `strict: false` opt-out, including for Responses where omission can trigger automatic strict normalization; every other Provider must omit the field. Never make optional fields required, add nullable placeholders, or strip schema keywords for Provider compatibility; preserve the canonical schema and runtime enforcement.
