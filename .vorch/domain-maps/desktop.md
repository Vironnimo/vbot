...
The Desktop also includes a local wakeword voice pipeline (`desktop/wakeword/`) that first verifies the active server has a configured, currently usable `speech_to_text` Task Model, then runs entirely on-device until transcription: one or two active TFLite detector heads sharing one feature extractor → sounddevice recording with WebRTC VAD silence detection → WAV upload to the vBot speech endpoint → server-side conversion through the same transcription-audio profile used by the Chat microphone → send transcript as a chat message through server RPC. `desktop/wakeword/engine.py` owns the curated built-in model catalog, shared `pyopen-wakeword` inference runtime, deterministic same-window winner selection, and validated Desktop-local imports of finished custom TFLite models; model training remains external to vBot. Okay Nabu and Hey Nabu are active by default. `pyopen-wakeword`, `sounddevice`, and `webrtcvad-wheels` ship in the `[desktop]` optional-dependency group, so a standard Desktop install runs real (non-mock) wakeword detection out of the box.
...
- `python desktop/main.py [--host] [--port] [--mock-wakeword]` (the same entrypoint `vbot desktop` invokes; see `cli.md`); package installation also exposes the `vbot-desktop` GUI-script entrypoint, which Windows Start-menu shortcuts target so normal app launch uses the Windows GUI subsystem without a console.
  ...
  - `--mock-wakeword` explicitly selects the no-microphone `MockWakewordWorker` for UI validation; it is the only path that simulates the state cycle. Without the flag, an import failure in `pyopen-wakeword`, `sounddevice`, or WebRTC VAD selects `UnavailableWakewordWorker`, which exposes `voice_stack_unavailable` and never simulates listening or sending. `getWakewordStatus().mode` distinguishes `real` / `mock` / `unavailable` and the WebUI explains the selected mode.
...
- **Per-user Desktop settings store (`desktop/settings.py`).** Settings live in the **OS per-user config dir**, resolved by `resolve_config_dir`: Windows `%APPDATA%\vbot` (fallback `~/AppData/Roaming/vbot`), every other platform `$XDG_CONFIG_HOME/vbot` else `~/.config/vbot` (macOS falls into the XDG branch until a Mac installer exists). The file is `<config-dir>/settings.json`.
  ...
  - on-disk schema `{ servers: [{host, port, label?}], last_used: {host, port}, window: {width, height}, wakeword: {…} }`; `last_used` is a `{host, port}` reference (not an index), so it survives list reordering
...
- **Standalone Desktop logging.** `desktop/main.py` attaches the `vbot.*` logger tree to daily `<config-dir>/logs/YYYY-MM-DD.log` files using the same `timestamp [LEVEL] name - message` format as the server and records normal or unexpected process shutdown. `DesktopBridge.publish_state()` additionally records every actual Voice state change with its event sequence, previous/new state, and stable error code when present; unchanged repeated states and WebUI status polls stay silent, while the worker's Detection record carries the winning model, score, and threshold. Logs never contain transcripts or Session content. This is Desktop-local because a Desktop Client may connect to a remote server and has no server `data_dir`; server-side Speech execution failures continue to appear in that server's own logs.
- **Window sizing.** `desktop/main.py` creates a first-run window at roughly 80% of the primary display, capped at 1440×960 logical pixels (1280×800 if screen discovery fails), with an 800×600 resize floor when the display permits it. Normal close persists only width and height through the Desktop settings store; startup restores and clamps that size to the current primary display. Position is deliberately not persisted, so OS centering keeps the window reachable after monitor removal or layout changes.
...
- `microphone` — sounddevice device index or `null` for automatic selection; the worker tries the host defaults then compatible input devices, captures in a supported native format, and projects each microphone read into a 16 kHz mono PCM stream for local Wakeword detection/VAD while retaining native-rate mono PCM16 for the following command recording. That command is uploaded as WAV and then normalized by the server's shared transcription-audio profile before STT execution; the detector format is not a competing Provider-facing Voice setting.
...
| `setWakewordEnabled(enabled)` | `{ enabled, error_code }` | Enable/disable the worker; real activation first checks server-side STT readiness and returns `enabled: false` without persisting or starting on failure |
...
| `retryWakeword()` | — | Stop the enabled worker, reinitialize PortAudio device discovery in real mode, then rebuild/restart so microphones hot-plugged after launch are visible |
| `startWakewordCalibration()` | calibration status | Start bounded guided calibration while the real worker is listening: measure ambient detector noise, capture three separated repetitions per active model in order, and calculate per-model sensitivity recommendations while command activation is paused |
| `stopWakewordCalibration()` | calibration status | Stop guided calibration without changing persisted sensitivities |
| `restartWakewordCalibration()` | calibration status | Discard the current calibration measurements and restart from ambient-noise measurement |
...
The WebUI polls `getWakewordStatus()` every 500ms while Desktop is detected; settings UI polling carries only runtime fields into editable state. Status includes `active_model_ids` and their effective `model_sensitivities` alongside the current `state`, stable `error_code`, concrete `active_microphone`, `mode`, transient guided-calibration phase, scores, noise levels, repetition counts, target model, and recommendations, plus a bounded sequence-numbered event history so short transitions such as detection are not lost between polls. App-level event consumption plays non-verbal Web Audio cues for detection, success, cancellation, no-speech/transcription failure, and fatal error; it also raises global Toasts for sent commands, no-speech, transcription failure, and fatal errors, so feedback does not depend on the Voice panel being open. Visual state remains authoritative if the host cannot play audio.

Worker states (exposed in `getWakewordStatus().state`): `off` → `starting` → `listening` → `wakeword_detected` → `recording` → `transcribing` → `sending` → `sent` → `listening`; recoverable utterance outcomes are `cancelled`, `no_speech`, and `transcription_failed`. Losing a microphone after listening started enters the non-fatal `microphone_disconnected` state with its machine-readable reason, clears `active_microphone`, and produces one auto-dismissing warning Toast; an unavailable microphone during initial startup and other fatal startup/device/routing/send failures still enter `error`. Real activation is rejected before persistence when `task_model.status(speech_to_text)` reports unconfigured or unusable, and an already-enabled worker repeats that fail-fast check before loading its engine or opening its microphone; stable reasons distinguish `speech_to_text_unconfigured`, `speech_to_text_unavailable`, a readiness-contract failure, and an unreachable server. The real worker closes the microphone while transcribing/sending, shows the outcome briefly, then reopens it. All active TFLite heads consume embeddings from one shared feature extractor; if multiple heads cross their thresholds in one window, the greatest threshold-normalized score wins and only one activation is emitted. Detection re-arms after all heads fall below threshold. Guided calibration first takes a three-second per-model 95th-percentile ambient baseline, then captures three released score peaks for each active model in selection order; it places a conservative threshold between ambient noise and the weakest intentional peak, quantizes the resulting sensitivity to the supported 0.05 step, and exposes the recommendations without persisting them. Apply remains an explicit WebUI action. Matched activations, recordings, and sends stay suppressed throughout calibration; it ends explicitly, after five minutes, or whenever the worker stops or leaves `listening`. The worker carries a rolling 320 ms detector pre-roll into the existing 300 ms pre-speech queue, so speech beginning in the trigger window is preserved but the wake phrase alone cannot become a command. WebRTC VAD then waits up to four seconds for post-detection command speech, ends after 1.5 seconds of trailing silence, and caps the command at 15 seconds.
...
- Startup validates the active server, a usable `speech_to_text` Task Model, and that server's configured Personal Agent before resolving the selected catalog model, loading the engine, or opening the microphone. Missing/unreachable server, missing/unusable STT, missing target, a stale Agent id, missing/invalid wakeword model, model startup, device incompatibility, repeated reads, detection, transcription, session resolution, and send failures have distinct `error_code` or recoverable outcome values and structured log records; stopped in-flight validation never continues into engine/microphone startup.
...
- Isolated microphone read errors during detection are recovered by reopening the stream; a failed reopen or three consecutive failures after listening began pause the worker in `microphone_disconnected`, while failure to open any compatible device during initial startup remains `error`.
...
- **webrtcvad-wheels** — Google WebRTC VAD for post-Wakeword speech onset and trailing-silence detection. Its Python wrapper avoids the removed `pkg_resources` dependency; an import failure makes Voice unavailable instead of silently degrading to fixed-duration capture.
...
- `pyopen-wakeword`, sounddevice, and WebRTC VAD are optional/lazy imports — normal Desktop mode reports Voice unavailable when any is missing; only the explicit mock flag simulates activity.
...
- The real wakeword worker runs in a daemon thread. Fatal failures leave a stable reason until config change, Retry, disable, or server switch rebuilds it; recoverable utterance failures return to listening automatically. Retry closes the existing stream before reinitializing PortAudio, and microphone enumeration/stream opening share an audio-backend lock so the WebUI can safely reload the device picker while the replacement worker starts.
