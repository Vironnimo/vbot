...
- **Probe contract (`probe_target`):** probe `/health` against the vBot identity contract owned by `server.md`, then `/` accepting a 2xx/3xx WebUI root. The four outcomes (`server_unreachable`, `not_vbot_server`, `webui_unavailable`, `invalid_target`) all render inline in the connection screen.
...
- **Window-first startup:** the window is created before the GUI loop with the connection screen as initial content; `load_url`/`load_html` may only run once the loop is live. With Voice disabled, startup never probes or imports the ML/audio dependencies; enabled startup resolves them only after the window is visible.
...
- RPC retries allowlist side-effect-free reads only (`agent.get`, `session.list`); `session.create` and `chat.stream` get exactly one transport attempt because post-commit retries could duplicate Sessions or Runs.
