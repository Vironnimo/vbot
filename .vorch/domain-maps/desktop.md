...
- **Standalone Desktop logging.** `desktop/main.py` attaches the `vbot.*` logger tree to daily `<config-dir>/logs/YYYY-MM-DD.log` files using the same `timestamp [LEVEL] name - message` format as the server. This is Desktop-local because a Desktop Client may connect to a remote server and has no server `data_dir`; server-side Speech execution failures continue to appear in that server's own logs.
