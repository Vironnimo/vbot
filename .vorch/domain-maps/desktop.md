...
| `startWakewordCalibration()` | calibration status | Start a bounded local score-inspection session while the real worker is listening; command activation is paused |
| `stopWakewordCalibration()` | calibration status | Stop score inspection without changing persisted sensitivities |
| `resetWakewordCalibrationPeaks()` | calibration status | Clear the session's per-model peak scores |
