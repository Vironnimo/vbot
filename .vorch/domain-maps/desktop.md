...
The Desktop also includes a local wakeword voice pipeline (`desktop/wakeword/`) that first verifies the active server has a configured, currently usable `speech_to_text` Task Model, then runs entirely on-device until transcription: TFLite detector heads sharing one feature extractor -> sounddevice recording gated by a WebRTC VAD -> anti-aliased soxr resampling to 16 kHz -> WAV upload to the vBot speech endpoint -> server-side conversion through the same transcription-audio profile used by the Chat microphone -> transcript submitted as a chat message through RPC. `desktop/wakeword/engine.py` owns the curated built-in model catalog, the shared openWakeWord inference runtime, VAD-gated score confirmation (a clear crossing confirms immediately; a marginal crossing must repeat within a short recent-score window), deterministic same-window winner selection, and validated Desktop-local imports of finished custom TFLite models; model training remains external to vBot. Okay Nabu and Hey Nabu are active by default. `openwakeword`, `sounddevice`, `soxr`, and `webrtcvad-wheels` ship in the `[desktop]` optional-dependency group, so a standard Desktop install runs real (non-mock) wakeword detection out of the box.
...
- **webrtcvad-wheels** - WebRTC VAD for speech onset/trailing-silence detection plus the pre-detection score gate; import failure makes Voice unavailable instead of silently degrading.
