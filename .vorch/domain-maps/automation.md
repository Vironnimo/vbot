...
- Schedules: `schedule_type` is `cron` / `interval` / `once` with kind-owned fields cleared on change; `remaining_runs` patches preserve on omission, replace on integer, `null` unlimited only for recurring. Statuses: `completed`/`missed` are immutable terminal history; `failed` re-enables after cause fix (resetting its failure counter); only `active` holds a scheduler task. Timestamps persist UTC with explicit offsets; naive once `run_at` interprets in the application timezone from `server.timezone`; legacy per-job `timezone` fields ignore-and-drop on next save.
