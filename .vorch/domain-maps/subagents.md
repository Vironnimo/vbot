...
- **Project inheritance:** a child inherits the parent Run's project from `context.project_id`. The child Session is created/opened under the project anchor (`chat_sessions.*(target_agent_id, project_id=context.project_id)`), and the child Run carries the same `project_id` (set via `chat_run_manager.start/enqueue(project_id=...)` → `Run.project_id`; the dedup key stays `(agent_id, session_id)`), so `_execute_run` reads `run.project_id` and the child's tool cwd resolves to the project repo. The child executor (`run_executor(content)`) takes no project_id. `None` (identity parent) keeps the legacy global behavior. The `SubAgentBatchTracker` parent key stays `(agent_id, session_id, run_id)` — the parent Run id is globally unique, so the project does not change the parent's identity; the child's addressable location lives in its Session metadata (`subagent_parent.project_id`). The parent's `project_id` is still captured on the batch (at `reserve_slot`, the first tracker call of the spawn flow) because the batch-completion trigger must continue the parent **under its project** — a project (config) parent like `orchestrator@vbot` resolves on its Team, not on the identity store. `reserve_slot` always runs before any `register_*`, so the batch's `project_id` is set before completion can fire.
