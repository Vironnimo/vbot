...
- The optional string spawn arguments (`session_id`, `agent_id`) treat a blank/whitespace value the same as omitted: a blank `session_id` creates a new Session, a blank `agent_id` falls back to the calling agent. Models routinely emit an omitted optional field as `""` (schema-valid for `type: string`), so this leniency stops an otherwise valid call from failing `invalid_arguments`; a present-but-non-string value is still rejected (`_optional_string_argument`). `content` (spawn) and `session_id` (`subagent_result`) stay strictly required.
