...
- Optional spawn arguments are coerced through the shared lenient policy in `core.tools.arguments` (see `tools.md`): a blank `session_id`/`agent_id` means omitted (creates a new Session / falls back to the calling agent), and `blocking` accepts string/0-1 booleans. A present-but-non-string value still fails `invalid_arguments`. `content` (spawn) and `session_id` (`subagent_result`) stay strictly required.
...
- **Forced blocking at nesting depth ≥ 1.** A sub-agent's Session is ephemeral — nobody reads it once it returns its single result — so a sub-agent cannot park work for a later run the way the top level can; a non-blocking spawn at depth would strand the child's result one level down (the batch-completion hand-off reaches only one level up). A spawn at `nesting_depth >= 1` is therefore forced to run blocking regardless of the requested `blocking` flag, gated by the single constant `FORCE_BLOCKING_AT_DEPTH` in `core/subagents/subagents.py` (set it to `False` to allow non-blocking spawns at depth — the single flip-back point). The top level (depth 0) is unchanged. When an explicit non-blocking request is overridden this way, the success result carries a `spawn_note` explaining why it blocked and that sibling spawns issued in one turn run concurrently (expendable guidance — the result payload is complete without it). Consequence: a forced-blocking child registers the parent-cancel cascade like any blocking spawn, so it dies with its parent.
