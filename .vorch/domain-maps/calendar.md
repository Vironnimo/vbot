...
- Storage: `<data_dir>/calendar/events.json`, a Generation 1 document `{"format_version": 1, "events": [...]}` written atomically (contract in `settings.md`; unknown event and `rrule` fields survive saves). Persisted recurrence fields use the same semantic validation and defaults as event creation; invalid rules are reported by `doctor config` and isolate only their event. Invalid entries are preserved and skipped on load; if the file is unreadable the service degrades (reads return empty, mutations raise) rather than destroying data.
