...
- Each admitted executor captures immutable request inputs, including the optional two-field Sub-Agent Run override, static Tool restriction, ephemeral Session-scoped Tool grants, and optional live Tool-denial resolver, then Chat resolves one Run-local execution context containing the Agent, Session, working Project inputs, primary Model target, Skill scope/catalog, Tool policy, and Continuation state. Agentic progression, Tool dispatch, Model fallback, and automatic Compaction retain those grants for the complete Run without persisting them into Session metadata. The static restriction and live denial remain dispatch-only and do not change System Prompt text or Provider Tool definitions; a Run grant intentionally adds only its matching Session-scoped Tool to the model-facing definitions.
