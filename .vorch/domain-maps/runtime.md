...
- `tools` — runtime `ToolRegistry` with built-ins registered at startup: `bash`, `cron`, `edit`, `glob`, `grep`, `image_generation`, `memory`, `process`, `read`, `session_search`, `skill`, `skill_manage`, `status`, `subagent`, `subagent_result`, `text_to_speech`, `web_fetch`, `web_search`, `write` (`skill`/`skill_manage` are ordinary allow-list tools). `channel_send` is registered dynamically while ≥1 channel is active; the Home Assistant tools (`ha_list_entities`, `ha_get_state`, `ha_list_services`, `ha_call_service`) only when `HASS_TOKEN` is configured.
