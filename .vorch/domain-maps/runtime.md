...
6. **Sessions, recall & chat** - one Runtime-owned `ChatSessionManager` opens the marker-authorized `<data-dir>/sessions.db` with lightweight readiness checks, then is injected into AgentStore, ProjectStore, Recall, Statistics, and Chat. Normal Runtime operation never creates Session snapshots; explicit snapshot and recovery workflows belong to Sessions (`sessions.md`). The Session-scoped `history` Tool registers immediately (**before** extension tools, preserving built-in name ownership); Project/Agent resolution and bootstrap Agent start **before** channels; recall defaults and falls back to `sqlite_fts`, whose construction failure is fatal because no lower registration fallback remains; ChatRunManager receives the shared Sessions persistence owner at construction. Runtime builds Reflection, titles and ChatLoopDependencies, and supplies one CompactionService to both Chat loops.
