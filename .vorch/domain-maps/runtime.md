...
6. **Sessions, recall & chat** - one Runtime-owned `ChatSessionManager` opens and verifies `<data-dir>/sessions.db`, then is injected into AgentStore, ProjectStore, Recall, Statistics, and Chat. The Session-scoped `history` Tool registers immediately (**before** extension tools, preserving built-in name ownership); Project/Agent resolution and bootstrap Agent start **before** channels; recall registry `canonical_scan` construction failure is fatal because no lower canonical fallback remains; ChatRunManager, Reflection, titles, ChatLoopDependencies, and both chat loops share one CompactionService.
