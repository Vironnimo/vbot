...
9. Load the `SkillRegistry` (`<data_dir>/skills`, bundled `resources/skills`, configured `skill_directories`) and register the internal `skill` tool; build the `SkillAuthoringService` (bundled skills root protected) and register the internal `skill_manage` write tool; load the `ExtensionRegistry` (`<data_dir>/extensions` + configured `extension_directories`).
...
- `tools` — runtime `ToolRegistry` with built-ins registered at startup: `bash`, `cron`, `edit`, `glob`, `grep`, `image_generation`, `memory`, `process`, `read`, `session_search`, `status`, `subagent`, `subagent_result`, `text_to_speech`, `web_fetch`, `web_search`, `write`, plus the internal `skill` and `skill_manage` tools. `channel_send` is registered dynamically while ≥1 channel is active; the Home Assistant tools (`ha_list_entities`, `ha_get_state`, `ha_list_services`, `ha_call_service`) only when `HASS_TOKEN` is configured.
...
- `reload_skills()` — reload the skill registry from current settings, drop the cached per-project and per-agent registries, re-register the internal `skill` and `skill_manage` tools, and update `SystemPromptManager` so prompt catalogs and provider tool visibility use the new registry without restarting the app. The runtime also exposes `agent_skills_dir(agent_id)`, `skills_for(project_id, agent_id=None)`, and `invalidate_agent_skills(agent_id=None)` for the agent-aware skill pool.
