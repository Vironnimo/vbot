...
12. Create `ChatRunManager` (also exposed as `runtime.chat_runs`), the `CommandDispatcher` for built-in slash commands, the `ReflectionService` (constructed with the runtime handle before the loops — it reaches the streaming loop lazily at review time), and one `SessionTitleService`; then create resolver-wired non-streaming + streaming `ChatLoop` instances with the shared title service, one shared `CompactionService(SummarizationStrategy())`, and the shared `ReflectionService` (constructor injection; no post-hoc wiring).
