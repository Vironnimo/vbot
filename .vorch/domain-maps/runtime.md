...
12. Create `ChatRunManager` (also exposed as `runtime.chat_runs`), `ReflectionService` (it reaches the streaming loop lazily), and one `SessionTitleService`; then create resolver-wired non-streaming + streaming `ChatLoop` instances with the shared title service, one shared `CompactionService(SummarizationStrategy())`, and the shared Reflection service.
