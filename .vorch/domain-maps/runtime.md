...
2. Build `StorageManager`, `ensure_directories()`, start its `TemporaryFileManager` (immediate expired-file sweep plus periodic cleanup when an event loop exists), load usable `settings.json` values, and read the `attachment_max_size_bytes` / `speech_upload_max_size_bytes` size limits (positive-int validated, else default with a warning). Invalid Settings never abort startup: malformed JSON/root shape falls back to defaults; schema-invalid top-level keys are logged and omitted individually while valid siblings remain active. The source file is not rewritten, and Settings mutations stay strict so they cannot overwrite invalid data.
...
- `tools` — runtime `ToolRegistry` with built-ins registered at startup: `bash`, `cron`, `edit`, `glob`, `grep`, `history`, `image_generation`, `memory`, `process`, `read`, `session_search`, `skill`, `skill_manage`, `status`, `subagent`, `subagent_result`, `text_to_speech`, `web_fetch`, `web_search`, `write` (`history` is Session-scoped; `skill`/`skill_manage` are ordinary allow-list tools). `channel_send` is registered dynamically while ≥1 channel is active; the Home Assistant tools (`ha_list_entities`, `ha_get_state`, `ha_list_services`, `ha_call_service`) only when `HASS_TOKEN` is configured.
...
- `skill_authoring` — shared `SkillAuthoringService` (validated, path-confined skill write core) used by the `skill_manage` tool and the skill-mutation RPCs; refuses writes under the bundled skills root.
...
- `command_dispatcher` — shared end-to-end `CommandDispatcher`: prepares and executes every Built-in Command and returns neutral outcomes to RPC and Channel projectors.
