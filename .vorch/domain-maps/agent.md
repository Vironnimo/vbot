...
- `reset_current_after_move(agent_id, moved_session_id) -> Agent` — re-homes the source identity agent's current pointer after a Session was moved away by `/agent`. If `current_session_id == moved_session_id`, it points at the most recently active *remaining* Session (max `last_active_at` from `ChatSessionManager.list_with_metadata`), or a fresh empty Session when none remain; otherwise it leaves the pointer untouched. Reads the stored config **without** the read-time `_ensure_current_session` normalization — `get` would replace the just-moved dangling pointer with a fresh empty Session before this method could choose the last-active landing. Identity-source only; project config agents carry no server-side current pointer (the target identity agent's pointer is set to the moved Session by the move orchestrator via `update`).
