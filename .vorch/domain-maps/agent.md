...
**Model chain.** Identity agents keep model→global→empty (the store). Config agents resolve **override→**model→project-default→global→**error**: the per-agent override (`project.model_overrides[agent_id]` — vBot-owned, data-dir only) is the top tier and wins over the repo-declared model, then the repo value, project default, and global default. A model counts only if it is *configured in this instance* (provider registered, model in catalog, a usable credential); an unconfigured candidate (including an override whose credential later vanished) falls through the chain. A declared (repo) model that is unconfigured is surfaced as a `BAD_MODEL` scan-report finding at scan time, not at first run. If the chain falls all the way through, resolution raises `AgentResolutionError`. The run paths map that to a clean failure (chat → domain error at the RPC edge via `error_mapping.py`; `subagent` → `agent_not_found` tool failure; `status` → `agent_not_found`). The override is set/cleared persistently through the `/model` command (project session → `ProjectStore.set_model_override`/`clear_model_override`); for an identity session `/model` mutates the agent's own `model` field instead. The identity-CRUD and server-RPC paths (`agent_methods`, `chat_methods`, `channel_methods`, automation) deliberately stay on `AgentStore` until project RPC lands.
