...
- `model` / `fallback_model` are user-facing `<provider>/<model-id>` optionally pinned `::<connection-local-id>[:<account-id>]`; empty resolves against `defaults.agent.model` at read time without rewriting disk. `fallback_model` activates only when a retryable provider error escapes built-in retries and stays active for the rest of that Run only.
