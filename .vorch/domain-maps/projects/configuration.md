...
Only `project_id` and `cwd` are required in a hand-edited persisted file: they identify the Project Anchor and its repository. Every other field is optional and defaults during load. The persisted contract contains:
...
- `project.set` updates persisted fields. Clearing `display_name` sends `null` and restores the `project_id` fallback. For `allowed_tools`, it accepts registered Tools whose declarative activation is directly configurable and whose constraints permit Project Agents; automatic or Identity-only Tools are excluded. An unavailable name already present may be carried forward or removed, but an RPC caller cannot introduce a new unavailable name. Changing `cwd` or `source_format` invalidates discovery/resolution caches because Team membership may change.
