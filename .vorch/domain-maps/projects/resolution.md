...
Resolution reads Agent configuration, may seed a workspace or normalize the current Session, and can wait for the Agent store's write lock. Event-Loop callers therefore use `resolve_agent_async` / `resolve_temporary_agent_async`, which run the same resolution in the bounded `agent-resolution` worker pool; Chat Run admission and execution, Subagent spawns, and Reflection reviews resolve this way. Synchronous callers already on a worker or outside a Run path keep the plain methods.
