...
- Schema: one closed root `request` object with `request.operation: "send"` and required `channel_id`; at least one of `message` or non-empty `file_paths` is structurally required, and `buttons` cannot coexist with `file_paths`. Public calls accept only this canonical discriminator shape.
