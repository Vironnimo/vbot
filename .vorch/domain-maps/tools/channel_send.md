...
Recognizable `request.operation: send`, `send: {...}`, and redundant `action: send` calls are normalized before canonical profile validation. Unsupported actions and conflicting fields fail before delivery.
