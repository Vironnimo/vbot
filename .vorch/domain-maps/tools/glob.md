...
- Exact mtime ordering still stats every observed match, but bounded Top-N selection retains only `offset + limit` ranked entries while consuming the walk. The input caps make 11,000 entries the hard in-memory maximum regardless of tree size; timeout/cancel continues to bound walk duration.
