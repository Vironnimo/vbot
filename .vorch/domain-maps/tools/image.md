...
- Success data: `{ message, images: [artifact, ...] }`. The model-facing `images` entries carry the artifact dict **plus** `path` (the absolute file path on disk, rendered with the shared forward-slash Model presentation) so the agent can copy/send/edit the file outside the chat; the top-level `artifacts` list stays path-free. `message` distinguishes delivery surfaces: WebUI/Desktop publishes each generated image by including `file:<path>` anywhere in the reply, while any Channel file delivery must call `channel_send` with the image `path` in `file_paths` instead of sending the marker.
