...
- Success data: `{ message, images: [artifact, ...] }`. The model-facing `images` entries carry the artifact dict **plus** `path` (the absolute file path on disk, rendered with the shared forward-slash Model presentation) so the agent can copy/send/edit the file outside the chat; the top-level `artifacts` list stays path-free. `message` reports completion only; reply-surface reminders own all instructions for showing or sending the generated files.
