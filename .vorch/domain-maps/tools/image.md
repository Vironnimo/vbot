...
- Success data: `{ images: [{ path }, ...] }`. Each `path` is the generated image's absolute file path rendered with the shared forward-slash Model presentation, which is the only result data the agent needs to copy, send, edit, or publish through its reply surface. The separate top-level `artifacts` list retains path-free transport metadata for non-Model consumers.
