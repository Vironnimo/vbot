...
- Success data: `{ images: [{ path, media_type, size_bytes }, ...] }`. Each `path` is the generated image's absolute file path rendered with the shared forward-slash Model presentation; `media_type` and `size_bytes` retain useful file facts for the agent. Transport identity (`id`, `kind`, `filename`, `url`, and `index`) is absent, and the generic top-level `artifacts` list is empty.
