...
- **binary** (a NUL byte in the bounded leading probe, checked *after* media routing so PNG/MP3/MP4 still take their own branch) → a short `[Binary file: <name> — Path: <resolved>]` notice without reading the remainder of the file. NUL detection is content-based and non-UTF-8 text without a NUL still streams with replacement characters.
- **image/\*** → `AttachmentStore.ensure_within_limit(file_size)` rejects oversize input before materialization; a bounded read guards concurrent growth, then `AttachmentStore.store()` rechecks size and MIME as the backstop. Successful reads return a short note plus the `read_media` artifact consumed by the chat loop.
