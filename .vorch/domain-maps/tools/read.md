...
- After the path is confirmed to be a file, the bytes are read once and classified with `sniff_media_type` (see `attachments.md`). The branch is chosen by the sniffed media type, not the file extension — **except** the document-extraction branch, which is chosen by `detect_extractable_document(name, media_type)` (sniffed type first, extension fallback; see below).
...
- **text/\*** or anything not image/audio/video → a leading UTF-8 BOM is stripped, then UTF-8 decode with replacement, then each line is prefixed with a compact, unpadded `N|` reference gutter (file-absolute line numbers starting at the requested offset). A resumed partial line uses `N:C|` (`C` is a 1-indexed character position); that `N:C` address is always accepted as `offset`, but is emitted only when the 50 KB cap cuts through a line. Line/byte truncation (2000 lines or 50 KB) is applied *after* numbering, so the gutter counts against the byte budget. No attachment is created.
- **extractable documents** (PDF / `.docx` / `.xlsx` / `.ipynb`, matched by `detect_extractable_document(name, media_type)` — sniffed type first, extension fallback — checked *after* media routing but *before* the binary check) → rendered as plain text by `core/tools/read_extract.py`, which works on the bytes the handler already read (not the path). Office/notebook rendering is stdlib only (`zipfile` + `xml.etree` + `json`); **PDF text extraction uses `pypdf`** (pure Python, a core dependency). Output is `[Extracted text from <name> (<label>)]:\n<body>` where the body is gutter-free (a rendering is not editable source) but still passes the shared line/byte truncation (`_render_text(..., number=False)`). docx → paragraph lines (tabs/breaks preserved); xlsx → tab-separated rows per sheet with a `# Sheet: <name>` header and row/column caps; ipynb → `# Cell N [type]` blocks; pdf → `# Page N` blocks of the text layer. The shared extractor admits at most 128 MB of uncompressed document content, so an Office archive cannot inflate without bound; exceeding it returns `document_too_large`. Detection needs sniff-or-extension because `.ipynb` sniffs as `text/plain` while docx/xlsx/pdf are NUL-laden and the binary check would otherwise dismiss them. A **scanned PDF** (no text layer) extracts to empty, and the body becomes `(no extractable text)`. A malformed document raises `ExtractionError`, and the handler falls through to the binary-notice / text path instead of failing.
