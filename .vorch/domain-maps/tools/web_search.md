...
- Display: the flexible primary is a future nonblank `description` when present, otherwise `query`. A successful call derives a fixed `results` fact from the existing `result_count`; failures publish no count, and the Tool result envelope is unchanged.
