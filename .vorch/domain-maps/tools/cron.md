...
- Every job payload is a curated projection, never the raw persisted record: raw parser fields (`schedule_type`, `cron_expression`, `interval_seconds`, `interval_anchor_at`, `run_at`), the internal `session_id`, the split `agent_id`/`project_id` pair, and `last_run_id` stay out of Tool results.
...
- Target failures (`CronTargetError` from CronService, or a malformed `target` address) stay `invalid_arguments` but carry target guidance instead of the action's call-shape examples: a missing Agent or Project, or a malformed address, tells the Agent to set `target` to an existing Agent id or `agent@project`; a target that exists but cannot run keeps the resolver's reason and tells the Agent to choose another target or tell the user. Coverage: `tests/core/tools/test_cron.py`.
- The Tool-owned argument normalizer repairs known nested `request.operation`, operation-key, and stringified operation shapes before the handler runs; the handler maps recognizable `agent_id` spellings to `target`, rejects conflicting targets, and rejects removed `timezone`, `session_id`, or `status` arguments.
