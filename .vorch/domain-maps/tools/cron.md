...
- Shared dispatch repairs recognizable nested `request.operation`, operation-key, and stringified operation shapes before the handler runs; the handler maps recognizable `agent_id` spellings to `target`, rejects conflicting targets, and rejects removed `timezone`, `session_id`, or `status` arguments.
