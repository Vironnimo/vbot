...
- **Model-facing output cap:** `output` keeps only the newest `BASH_MODEL_OUTPUT_CAP_CHARS` (30k) characters. `truncated: true` covers both cut causes (model cap here, process buffer cap upstream); either way the *beginning* is what's missing, and a leading marker line says so and names the complete per-process log file (see `process.md` → spool file) so the agent can grep/read it directly. Without a spool dir the marker carries no path.
- Background runs return a `session_id` for the `process` tool plus capped combined `output` captured before backgrounding, and always include `log_file` (when spooling is active) since output keeps growing after the hand-off. The background completion wake-up message applies the same cap and marker.
