...
- Tool name: `subagent`.
- Schema: one open flat object requiring `action: "run" | "status" | "cancel"`, with optional siblings `content`, `agent_id`, `session_id`, `model`, `thinking_effort`, and `id`. The handler requires `content` for `run`, requires `agent_id` when continuing `session_id`, requires `id` for `status` and `cancel`, and rejects unknown or action-inapplicable fields. The model-facing schema emits no branch or conditional keywords and no `additionalProperties`. Legacy `request`, `operation`, `background`, `run_id`, and `queue_item_id` fields are not public.
