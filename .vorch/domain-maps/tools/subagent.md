...
- Result fields carry the bare `agent_id` beside `project_id`, but the `agent_id` argument resolves a bare id in the caller's scope. Every continuation or resume note therefore renders the exact values to pass back: `agent_id` as `agent@project` when the child has a Project, and `session_id`. A successful `cancel` of started work carries such a resume note; removing queued work does not. Coverage: `tests/core/tools/test_subagent_results.py`, `tests/core/tools/test_subagent_explicit_cancellation.py`.
