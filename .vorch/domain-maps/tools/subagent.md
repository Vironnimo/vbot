...
- Tool name: `subagent`.
- Schema: one flat discriminated union with a closed branch for required `action: "run" | "status" | "cancel"`. `run` alone exposes and structurally requires `content`, optionally accepts `agent_id`, `session_id`, `model`, and `thinking_effort`, and structurally requires the owning `agent_id` when `session_id` is present. `status` and `cancel` expose and require only the stable public `id` returned by `run`. Legacy `request`, `operation`, `background`, `run_id`, and `queue_item_id` fields are not public.
