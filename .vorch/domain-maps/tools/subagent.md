...
- `status` with `id` returns one snapshot. Omitting `id` returns `data.subagents`, a list of the same snapshots for all retained entries across the calling Parent Agent Session's Runs in its Project, in batch/entry insertion order; no entries returns `[]`. Each lookup failure stays with its public `id` and `error` without suppressing siblings. Both forms acknowledge terminal results only after the Parent Tool Result is durably persisted; already-pruned work is absent. Source: `SubAgentBatchTracker.owned_entries` and `_handle_subagent_status`; coverage: `tests/core/subagents/test_subagents.py`.
