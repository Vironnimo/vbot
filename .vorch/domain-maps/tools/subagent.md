...
- Successful `run` results carry `activity_file: string | null`; when allocated they also carry an `activity_note` with its concrete path. Status snapshots keep `activity_file`, never repeat the activity note, and carry a behavioral `note` while queued or running that names automatic delivery and how to wait for the result.
