...
- Runtime shutdown remains the primary owner and explicitly kills every tracked process tree. Job close, guardian pipe EOF, and the systemd cgroup are crash-only backstops that prevent an obsolete Runtime's children from becoming untracked; they do not create durable or reattachable Process Sessions.
...
- Raw stdin support is not interactive-terminal support: there is no PTY/ConPTY, terminal resize, host prompt, or TUI contract. The separate `terminal_beta` Tool owns that capability (see `tools/terminal_beta.md`); do not weaken the non-interactive Bash default or imply terminal semantics through `process.input`.
