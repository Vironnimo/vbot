...
- `ProcessManager.snapshot(process_id, agent_id, *, project_id)` provides the public non-consuming status view; `ProcessManager.send_input(process_id, agent_id, text, *, newline, eof, project_id)` owns stdin delivery; `ProcessManager.kill(process_id, agent_id, *, project_id)` owns termination.
