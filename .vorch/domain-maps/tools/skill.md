...
- `skill` is governed by both lists: `allowed_tools` decides whether the Tool and its `skill_list` companion are offered, and `allowed_skills` decides which Skills either Tool may expose, activate, or read. `skill_list` is not independently configurable; its result applies the same `allowed_skills` filter and groups the current live catalog by origin.
