...
- Schema: one closed flat object with optional `name` and optional `file_path`; no fields lists, `name` alone activates, and `name` plus `file_path` reads. `file_path` accepts `SKILL.md` or a relative path below `scripts/`, `references/`, or `assets/`; the handler rejects it without a non-empty `name`.
...
- Keep the three modes in one flat Provider-visible object. A root `oneOf` whose first branch is the valid empty list call can cause models or Provider tooling to privilege that branch and stop exposing or selecting `name`, producing repeated list calls instead of activation.
