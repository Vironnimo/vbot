...
- `list` returns events that have at least one occurrence in the window plus their occurrences; `find_free` returns up to five earliest free slots and ignores cron jobs. Timed event payloads carry `start`/`end` with no `duration`; `end` comes from `CalendarService.event_span`, the occurrence arithmetic (a single event lasts its duration in real time, so across a DST transition local end minus start differs from the duration). All-day payloads carry `start` and `duration` in days.
