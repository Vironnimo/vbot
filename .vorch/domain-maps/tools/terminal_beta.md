...
- The manager publishes an authoritative ANSI snapshot followed by contiguous sequenced output/state events. `server/rpc/terminal_methods.py` maps operator start/list/input/resize/kill and `/ws/terminals/{terminal_id}` carries the selected stream. `terminal.start` accepts an optional command, exact argument list, working directory, and dimensions; omitted command and working directory select the host user's default shell and home directory through the same generic launch path and return `owner: null`.
- `webui/src/lib/terminalsView.js` owns manual start, active-catalog reconciliation, stream reconnect and gap recovery, exact xterm input batching, resize debounce, and teardown. `TerminalsView.svelte` dynamically mounts xterm.js, offers a New terminal dialog, starts manual terminals in focused direct-control mode, defaults Agent-owned terminals to observe mode, makes a primary click on the terminal take control and focus native xterm keyboard/paste input, preserves bounded scrollback behind an always-visible scrollbar with Jump to latest recovery, and confirms process-tree stop. There is no secondary text composer or program-specific input path. Navigation and socket teardown never own the Terminal Session lifetime.

## Runtime and lifecycle integration

Runtime constructs `TerminalManager` after `TriggerService`, registers `terminal_beta` before System Prompt assembly, exposes it through `RuntimeServices`, and closes it before temporary-file cleanup. Run cancellation never closes Terminal Sessions. Session deletion, Agent removal, Project removal, and successful `/agent` moves close or transfer exact Agent-owned Terminal scopes inside their existing lifecycle boundaries; manual Terminal Sessions end only through explicit stop, natural exit, terminal failure, or Runtime shutdown. `terminal_beta` is an ordinary built-in Tool and is part of the default Project Tool allowlist.

Focused coverage lives in `tests/core/tools/test_terminal_backend.py`, `test_terminal_manager.py`, and `test_terminal_beta.py`, with Runtime, lifecycle, RPC, WebSocket, controller, and rendered-component coverage in their owning test trees. `tests/e2e/tests/terminal-lifecycle.spec.js` exercises the platform default shell and multiple simultaneous real PTY/ConPTY processes through browser start, native input, background output, selection changes, stream reconstruction, navigation, and selective stop. Real Windows validation must use actual ConPTY programs and an unmodified installed Codex CLI because fake adapters cannot prove executable launch fidelity, alternate-screen rendering, browser keyboard/paste behavior, resize, or process-tree stop.
