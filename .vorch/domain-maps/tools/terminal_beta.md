...
- `webui/src/lib/terminalsView.js` owns manual start, active-catalog reconciliation, stream reconnect and gap recovery, exact input batching, resize debounce, and teardown. `TerminalsView.svelte` dynamically mounts xterm.js, offers a New terminal dialog, starts manual terminals in focused direct-control mode, defaults Agent-owned terminals to observe mode, offers deliberate text-plus-Enter input, and confirms process-tree stop. Navigation and socket teardown never own the Terminal Session lifetime.
