...
The Tool accepts one exact `project_id`. On success it returns status, Project id/display name/cwd, one model-facing `content` string containing the Project contract, readable auto-load files, and a path-bearing Project Skill list, plus structured `loaded_files` and `skills` metadata. Missing auto-load files are skipped through the prompt renderer's existing fail-soft behavior; a missing Project, invalid id, unreachable cwd, or Project-load failure returns a stable non-retryable Tool failure.
...
- Loading Project Context changes no cwd, Rooting, Workspace, Session ownership, Skill scope, permissions, addressing, or persistent active-Project state. File Tools must use absolute paths. Every `bash` call targeting the Project must set `workdir` to the returned cwd again because each call is a one-shot shell and retains no cwd change from an earlier call.
...
- The `project` call must be alone before dependent Tool calls because sibling calls execute concurrently. This ordering requirement is stated in both the Tool description and its prompt block.
...
- Project Skills are listed by absolute `SKILL.md` path because loading Project Context does not widen the Identity Agent's `skill` Tool registry; using one requires reading that file explicitly.
...
- The Project catalog is live at every System Prompt build; the Tool result is live at every call. Neither is Session-pinned, and there is no `visited_projects` metadata.
