...
- Result paths are rendered relative to the **working directory** when the file lies under it, absolute otherwise (`display_search_path` in `core/tools/search.py`) — so every result round-trips directly into a follow-up `read`/`edit` call. The `glob` filter still matches search-root-relative paths, bare names at any depth, **case-insensitively on every platform**, and expands `{py,js}` brace alternations rg-`--glob`-style in **both** engines (`_expand_brace_alternations` in `core/tools/search.py`; fnmatch itself has no brace support — without expansion `**/*.{py,js}` silently matched nothing). Comma-less and unmatched braces stay literal. When ignore rules are active, a supplied `glob` selects the Python fallback because ripgrep's positive `--glob` overrides `.gitignore`; when ignore rules are disabled, rg receives the filter with `--glob-case-insensitive`.
...
- The rg subprocess uses `process_manager.subprocess_creation_flags()`, so searches never flash a console window on Windows; the Python fallback spawns no child process.
