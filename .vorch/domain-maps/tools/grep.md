...
- Result paths are rendered relative to the **working directory** when the file lies under it, absolute otherwise (`display_search_path` in `core/tools/search.py`) — so every result round-trips directly into a follow-up `read`/`edit` call. The `glob` filter still matches search-root-relative paths, bare names at any depth, **case-insensitively on every platform**. When ignore rules are active, a supplied `glob` selects the Python fallback because ripgrep's positive `--glob` overrides `.gitignore`; when ignore rules are disabled, rg receives the filter with `--glob-case-insensitive`.
