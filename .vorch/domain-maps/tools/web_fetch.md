...
- Schema: required `url` and required `output`; `output` is exactly one of `markdown` (cleaned HTML with Markdown link targets), `text` (cleaned HTML without link targets), or `raw` (HTML without cleanup); `additionalProperties: false`. The former `include_links` and `raw` booleans are invalid legacy fields. With no optional properties the OpenAI wire renders the Tool strict without changing the canonical required set.
