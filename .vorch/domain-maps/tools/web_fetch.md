...
- Schema: required `url`; optional `output` is exactly one of `markdown` (cleaned HTML with Markdown link targets), `text` (cleaned HTML without link targets), or `raw` (HTML without cleanup) and defaults in the handler to `markdown` when omitted; `additionalProperties: false`. The former `include_links` and `raw` booleans are invalid legacy fields. The OpenAI wire preserves the canonical optional field and explicitly sends `strict: false`; it never rewrites omission as required-nullable input.
