...
- Put fields required for every call in every relevant branch's `required` list. For a flat multi-action Tool, use `action_schema` so each action branch structurally requires what it needs and contains no properties belonging to another action.
- Reject fields that are inapplicable to the selected action at the schema boundary, with handler checks retained as defense in depth for direct callers. A typo or stale field must fail with `invalid_arguments`, not be ignored.
...
- The canonical input has a JSON-object root. A direct fixed-shape root sets `additionalProperties: false`; a flat action union closes every object branch instead.
- The schema owns field names, JSON types, enums, action-specific field sets and requirements, ranges, lengths, and structurally simple dependencies. The handler owns cross-field meaning, authorization, existence, state transitions, and other semantic checks, while retaining action-field checks as defense in depth.
- A multi-action Tool uses one flat `oneOf` union whose branches each carry a single-value `action` enum, only that action's properties, its requirements, and `additionalProperties: false`. Unions also remain appropriate for genuine value-shape alternatives within one behavior and for a cursor-only continuation that must reject every initial-call field.
...
3. Define the smallest canonical flat schema. For multiple behaviors, use `action_schema` and make each branch action-complete rather than publishing one optional-property superset. Preserve behavior and established domain field names unless a name itself causes ambiguity.
