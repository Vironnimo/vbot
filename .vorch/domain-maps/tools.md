...
- `ToolPromptBlockRegistry` is the **tool-side System Prompt block-declaration seam**: a tool that wants prompt content declares a block here (`register(tool_name, *, default_text=None, render=None)`, exactly one of text/render, first-wins-with-warning on a duplicate tool name), and the runtime gathers `block_definitions()` and merges them with the extension blocks before handing the list to the prompt manager. A declared block is id `tool:<name>` and owner `tool:<name>`, so gate 2 renders it only when `<name>` is on the agent's effective allowlist; static or dynamic, the same split as a core or extension block. **The prompt domain imports no tool classes** — it only ever consumes a list of `core.prompts.BlockDefinition` (the `core.prompts` import in `block_definitions()` is lazy so the tools domain has no import-time dependency on prompts either). The built-in `subagent` Tool uses a dynamic block for delegation guidance and its additional-Agent catalog. See `.vorch/domain-maps/prompts.md` (block model), `.vorch/domain-maps/subagents.md` (catalog semantics), and `.vorch/domain-maps/extensions.md` (the parallel extension-block path).
