...
- `ToolContext`: `agent_id`, `session_id`, `run_id`, `tool_call_id`, `tool_name`, `tool_call_index`, `workspace`, `cwd`, `project_id`, `app_root`, `data_root`, `nesting_depth`, `allowed_skills`, `session_tool_grants`, plus emit/cancel/note/skill hooks, per-call cancel hooks (`cancel_registration_hook`, `cancel_check_hook`), and the optional post-persistence hook used by `after_result_persisted(callback)`.
