...
- `ToolContext`: `agent_id`, `session_id`, `run_id`, `tool_call_id`, `tool_name`, `tool_call_index`, `workspace`, `cwd`, `project_id`, `app_root`, `data_root`, `nesting_depth`, `allowed_skills`, plus emit/cancel/note/skill hooks and per-call cancel hooks (`cancel_registration_hook`, `cancel_check_hook`).
  ...
  - `project_id` (`str | None`) is the project of the running Run, set by the chat loop. Project-aware tools receive it as the caller scope; `subagent` preserves it for a bare target but a qualified `agent@project` supplies an independent target scope. `None` means global/identity.
...
- **Readiness is a separate axis from the allowlist.** A not-ready tool stays *registered* (persisted permissions survive) but is filtered out of the **model-facing** surfaces and returns a `tool_not_ready` envelope on a direct dispatch. Two surfaces apply the readiness filter: `provider_definitions` and `prompt_definitions` (both default `ready_only=True`, so the prompt tool list, the provider tool definitions, and — through the prompt definitions — gate 2 of a `tool:<name>`-owned prompt block all drop a not-ready tool). The `tool.list` RPC (`server/rpc/catalog_methods.py` `_list_tools`) now calls `list_tools()` **without** the readiness filter — it returns **every** registered tool, each carrying `ready` (bool, via `tool_is_ready`, a raising predicate counting as false), `readiness_hint` (or null), and `extension` (owning extension or null) — so the agent tool picker and the Project Tool Whitelist editor **see** a not-ready tool and style it from those fields rather than the tool vanishing. Consumers that keep seeing not-ready tools with no styling deliberately leave `ready_only` at its default: `ExtensionRegistry.apply_tools` collision detection (`include_internal=True`), `core/chat/tool_dispatch.py` `registered_tool_names` (effective-allowlist computation — dispatch is not readiness-gated at the allowlist level), and the runtime startup inventory count. A `tool:<name>`-owned prompt block still drops when its tool is not ready, because gate 2 reads the readiness-filtered prompt definitions.
