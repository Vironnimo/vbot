...
- `Tool`: `name`, `description`, `parameters`, `handler`, `internal`, `display`, and `ready` (an optional zero-arg readiness predicate, `Callable[[], bool] | None`; `None` = always ready).
...
- `ToolRegistry.register(name, description, parameters, handler, *, internal=False, display=None, ready=None) -> Tool` (`internal`/`display`/`ready` are keyword-only; `ready` validated callable-or-None).
...
- `ToolRegistry.list_tools(allowed_tools=None, *, include_internal=False, ready_only=False) -> list[Tool]` — filter order **registered → allowed → ready**; `ready_only` applies the readiness filter *after* the allowlist/internal filters and defaults **off**.
- `ToolRegistry.provider_definitions(..., *, ready_only=True) -> list[dict]` returns provider-visible `name`, `description`, and JSON Schema only. `ready_only` defaults **on** (model-facing surface).
- `ToolRegistry.prompt_definitions(..., *, ready_only=True) -> list[dict]` returns prompt-visible name/description pairs; `ready_only` defaults **on**.
- `ToolRegistry.dispatch(context, arguments, allowed_tools=None) -> dict` executes a tool and validates the result envelope. **Dispatch is not list-filtered by readiness**, but after the allowlist check it re-evaluates readiness live and, when not ready, returns a `tool_not_ready` failure envelope (`retryable=False`) without running the handler — the safety net for a prompt built moments before the credential vanished.
...
- **Readiness is a separate axis from the allowlist.** A not-ready tool stays *registered* (persisted permissions survive) but is filtered out of the model-facing surfaces and returns a `tool_not_ready` envelope on a direct dispatch. Only these three surfaces apply the readiness filter: `provider_definitions` and `prompt_definitions` (both default `ready_only=True`, so the prompt tool list, the provider tool definitions, and — through the prompt definitions — gate 2 of a `tool:<name>`-owned prompt block all drop a not-ready tool), and the `tool.list` RPC (`server/rpc/catalog_methods.py` `_list_tools` passes `ready_only=True`, hiding it from the agent tool picker and the Project Tool Whitelist editor). Consumers that must **keep seeing** not-ready tools deliberately leave `ready_only` at its default and are **not** filtered: `ExtensionRegistry.apply_tools` collision detection (`include_internal=True`), `core/chat/tool_dispatch.py` `registered_tool_names` (effective-allowlist computation — dispatch is not readiness-gated at the allowlist level), and the runtime startup inventory count. A `tool:<name>`-owned prompt block now drops when its tool is not ready, because gate 2 reads the readiness-filtered prompt definitions.
