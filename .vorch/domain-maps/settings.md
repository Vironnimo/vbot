...
- `core/settings/__init__.py` exports `parse_settings_update`, `SettingsValidationError`, central JSON validator report types, load helpers, file validators, data validators, and shared Settings schema constants (including `DEFAULT_APPEARANCE_LANGUAGE`, `SUPPORTED_APPEARANCE_LANGUAGES`, `DEFAULT_APPEARANCE_CHAT_WIDTH`, `SUPPORTED_APPEARANCE_CHAT_WIDTHS`, `DEFAULT_RECALL_SETTINGS`, `ALLOWED_THINKING_EFFORTS`, `MIN_TEMPERATURE`, `MAX_TEMPERATURE`). The `chat_width` constants live in `settings.py` (not `normalizers.py`, which imports them) so the public update parser can validate membership without a circular import.
