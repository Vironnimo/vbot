...
Most stable event constants live in `core.runs`: lifecycle (`run_started`, `run_completed`, `run_failed`, `run_cancelled`), output (`user_message_persisted`, `reasoning`, `tool_call_started`, `tool_call_result`, `assistant_output`, `error_message_persisted`, `model_fallback_activated`, `model_step_usage`, `compaction_completed`), and SSE-only deltas (`assistant_output_delta`, `reasoning_delta`, `tool_call_delta`, `tool_call_stdout`, `tool_call_stderr`). Chat emits the non-visual `model_step_usage` payload `{usage, session_usage}` after the corresponding assistant turn is persisted, including estimated and tool-only turns; server bridges it as ordinary `run_output`, and UI timelines ignore it. Chat's `compaction_completed` payload is `{message, checkpoint, checkpoint_id, history_available}`; `message` is retained for compatibility. `subagent_session_started` is emitted on a Run timeline too, but its constant belongs to `core.subagents` because the payload is owned by the sub-agent domain.
...
- `ChatRunManager.has_activity_for_agent(agent_id, *, project_id)`, `has_activity_for_working_project(project_id)`, and `has_activity_for_session(...)` are snapshot queries for diagnostics and non-destructive decisions. They are not synchronization primitives: destructive Session/Agent/Project workflows use the corresponding Run Admission Guard so a new Run cannot enter after an idle check.
