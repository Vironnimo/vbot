...
- RPC methods are `prompt.list`, `prompt.update`, `prompt.reset`, and `prompt.preview`. `prompt.preview` returns rendered `text`, token count, and `estimated`; for an explicit Agent scope the server can infer `agent_id` from the scope, while default/no scope requires `agent_id`. The `agent_id` param is an `agent@projekt` **address** (parsed once at the edge via `parse_agent_address`): a bare value previews the identity agent (and, when that agent is **rooted** — its workspace IS a registered repo cwd — its rooted project's files too); a project-qualified value resolves that project's config agent through `AgentResolver` and threads its `agent_body` + the project's `ProjectPromptContext` into the build, so `{project_files}` and the imported body render exactly like a project-born run. The project context is sourced through the **same** `resolve_prompt_project` policy the chat loop uses (`_preview_project_context`), so a preview never drifts from a real run — including the rooted case, which applies even on an Agent prompt scope (the address is identity-only, but rooting still feeds `{project_files}` if the agent is rooted).
...
- Workspace includes accept only safe flat filenames. `{include:filename}` resolves under the Agent workspace, wraps content as `<file name="filename">\n...\n</file>`, and drops missing **or unreadable** files with a warning — a prompt file never aborts the run (user decision), so any read failure (missing, locked, no permission, a directory, binary/non-UTF-8) is logged and the block omitted. It still raises `PromptError` for an **unsafe include path** (that is a malformed template directive, not a file-readability issue). An **empty workspace** (a config agent, `workspace == ""`) means "no includes": every `{include:…}` is dropped, with no file read and no warning. It must never resolve against `Path("")` (= `Path(".")`), which would read SOUL.md/USER.md from the server's process CWD.
...
- **`{project_files}`** renders the project's auto-load files in `auto_load` order (AGENTS.md is the seeded first entry, not special-cased), each wrapped via the same `_wrap_include_file` source as `{include}`. **Auto-load paths are taken verbatim with no location restriction** — a relative path resolves against the **cwd** at any subfolder depth, an absolute path is read as-is (the auto-load list is the user's own config naming the user's own files; project philosophy: maximum agency). Unlike the flat `{include}` workspace path, project files do **not** go through `_validate_workspace_include`. Lazy and fail-soft: a missing file is skipped silently, and a present-but-unreadable file (locked, no permission, a directory, binary/non-UTF-8) is logged and skipped — a prompt-load file never aborts the run (user decision). No size limit or truncation on readable files (technical-user contract: the file goes in 1:1). Config agents keep the inherited `{runtime}`/`{tools}`/`{channels}`/`{skills}` blocks from the same root.
