...
**Runtime variables:** `{server_hostname}`, `{vbot_version}`, `{operating_system}`, `{model}`, `{identity_workspace}`, `{vbot_root}`, `{data_root}`, `{thinking_effort}`, `{current_utc_date}` substitute exactly, one pass, non-recursively; unrelated `{...}` stays literal; vBot-authored path values use the forward-slash Model presentation; retired names have no aliases and remain literal. `runtime.md` (owner `always`) carries OS/model/thinking effort/UTC date - unset effort renders "provider default", never blank. `identity_runtime.md` (owner `identity`) carries hostname/version/workspace/roots, so Config Agents receive none of those.
