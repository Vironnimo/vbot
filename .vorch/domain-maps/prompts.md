...
It does not own raw file I/O, workspace memory, HTTP/RPC transport, or tool execution. Storage persists layout and text overrides (`PromptBlockStore`) plus bundled fragments; memory declares its block and renders files; tools/skills/channels provide catalogs; extensions declare blocks; Chat sends the assembled system message whole. `blocks.py` is the pure assembly engine; `prompts.py` coordinates prompt-epoch rendering and Tool policy. Internal `_catalog.py` owns scope validation, editable block definitions, layouts and the shared override cascade used by both editing and assembly. `_types.py` holds contributor contracts and block identifiers; `_formatting.py` renders the Tool/Channel/Skill catalogs. Existing public manager methods remain the RPC and Chat boundary.
...
The bundled default layout lives in `resources/prompts/layout.json` shipping soul -> memory -> runtime -> identity_runtime -> tools -> project/subagent dynamic blocks -> tools_list (off) -> channels -> skills -> skill_maintenance -> agent_body -> working_project. A missing, explicitly empty, or broken persisted layout uses the bundled defaults; a broken file logs a warning and remains untouched. Empty means no custom order or enablement, not no blocks: disabling all blocks requires explicit `enabled: false` entries. Storage validates the entire persisted layout before use; a malformed entry discards that layout rather than partially applying it (`test_prompt_blocks.py`, `test_prompts_edit_facade.py`). The seven core text blocks read defaults from their fragment resources; `working_project.md` backs the non-editable dynamic block. There is no fragment-edit facade anymore - the only prompt-edit surface is the manager's block facade below.
...
3. **Resolve text:** skip disabled or inactive blocks before rendering; dynamic blocks call `render(context)` in isolation (exception drops only that block); static text takes the override cascade, then `expand_block_template` tokenizes that author-controlled template once and substitutes `{generated:...}` and `{include:...}` (fail-soft) plus build-time replacements in the same pass. Producer output, included file content, and replacement values are inserted verbatim and never scanned again, so a Memory entry or Skill description containing `{include:...}` or `{model}` stays literal and cannot read files or fail the build (`test_blocks.py`, `test_prompts_assembly.py`); data stays verbatim. Explicit inspection via `block_details` also renders disabled active blocks to show their content, without including them in the combined prompt.
...
**Override cascade:** agent override <- default-scope override <- owner default; composed by the manager from two store reads, never by the store. Dynamic and data blocks have no override path. Request-local data namespaces therefore never reach the persistent override store (`blocks.py`, `test_prompts_layouts_overrides.py`).
...
- The `core:skills` catalog is origin-grouped and path-free (`SkillPromptMetadata`: name/description/origin); `_formatting.py` wraps `core.skills.skills.format_skill_catalog_entries` (which owns the English origin labels) in `<available_skills>`. `render_skill_catalog(...)` produces the `PinnedSkillCatalog` Chat pins per epoch.
- `core/prompts/pinned_context.py` owns the Chat-side pinned-snapshot assembly persisted as Session prompt pins between Compactions, plus `stamp_prompt_files_read` for read-before-write state. Initial SOUL/Memory/Project renders stamp their reported files; snapshot reuse never updates stamps. Memory snapshots carry their rendering mode and refresh when that mode changes, while unchanged-mode file edits stay pinned. The Working Project Context and Skill catalog pins carry the `working_project_id` they were rendered for (the catalog: its Skill-scope Project), because Chat re-resolves the working Project per Run admission: re-rooting A -> B (directly or via an unrooted Run) re-renders both at the next Run instead of showing A until Compaction; an unrooted Run ignores a stale Working Project pin. A pin missing a qualifier re-renders (`tests/core/chat/test_chat_prompt.py`). `prompt_epoch_pins` returns the same qualified shapes for the Compaction commit's `PromptEpoch`. Called directly by ChatLoop/Compaction via a narrow dependency slice, no chat-owned copy.
...
- Prompt code consumes Protocols for Agent/Tool/Skill/Channel/Storage shapes; contributions arrive as plain `BlockDefinition` objects handed in via Runtime. The manager uses the Memory domain's default renderer when no custom memory provider is injected.
