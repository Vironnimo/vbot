...
- `core/prompts/pinned_context.py` owns the Chat-side pinned-snapshot assembly persisted into Session metadata between Compactions, plus `stamp_prompt_files_read` for read-before-write state. Initial SOUL/Memory/Project renders stamp their reported files; snapshot reuse never updates stamps. Memory snapshots carry their rendering mode and refresh when that mode changes, while unchanged-mode file edits stay pinned (`tests/core/chat/test_chat_prompt.py`). Called directly by ChatLoop/Compaction via a narrow dependency slice, no chat-owned copy.
