...
- `API_VERSION` is the public contract version and is currently 7; API v2 adds Slash Command declarations, API v3 adds Extension-owned Tool Family declarations, and API v4 adds management operations, host capabilities, and live Tool catalogs; API v5 adds explicit opt-in Tools; API v6 adds page declarations and explicit Tool catalog visibility; API v7 adds owner-bound Extension databases (`host.open_database`). An optional directory-form `extension.json` may add display metadata and an `api_version`; a manifest requiring a newer version fails that Extension.
