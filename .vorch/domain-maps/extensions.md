...
- `API_VERSION` is the public contract version and is currently 6; API v2 adds Slash Command declarations, API v3 adds Extension-owned Tool Family declarations, and API v4 adds management operations, host capabilities, and live Tool catalogs; API v5 adds explicit opt-in Tools; API v6 adds page declarations and explicit Tool catalog visibility. An optional directory-form `extension.json` may add display metadata and an `api_version`; a manifest requiring a newer version fails that Extension.
...
- Ordinary Hook, Command, lifecycle, and interaction handlers may be synchronous or asynchronous. Their dispatch isolates failures. Session-owning Extensions instead use the explicit required Session runtime contract; failed delivery or completion must remain observable to the owned Run.
