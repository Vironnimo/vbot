...
The memory system **owns its two workspace files** (`USER.md`, `MEMORY.md`): they exist only when there is memory content — created lazily on the first tool write — and are **never** seeded by the agent/workspace layer (which owns `SOUL.md` only). So a memory-off agent never gets them, and deleting them while memory is off does not resurrect them. Before a file physically exists, its prompt appearance is synthesized from default "no entries" content (see Prompt Block), so behavior is seamless.
...
- **The file contents are the data half.** The `{generated:memory_files}` marker expands to `read_prompt_files(workspace, mode)` — the `<file>`-wrapped file contents only. When the block renders (memory tool on), every file the mode selects is embedded; a **not-yet-created** file renders its default "no entries recorded" content (identical to an empty on-disk file), so the framing is byte-identical before and after the file physically exists — and reading never creates the file. Only `off` mode yields `""` (it selects no files, and gate 2 drops the block anyway).
- The guidance text complements the memory tool's WHEN/SKIP description with the writing-quality half: what makes an entry worth its permanent prompt cost (it spares the user future steering) and the one non-obvious rule — write durable, **declarative** facts, not imperative self-instructions (which get re-read as standing directives in later sessions). As block default text it is now user-editable through the override cascade, no longer hardcoded as a prompt string. The old `MemoryService.build_prompt_block(workspace, mode)` (which returned guidance + `<memory>` wrapper + files together) is **gone** — that composition now lives split across the block declaration (guidance + wrapper) and `read_prompt_files`/`read_memory_files` (files).
...
- A missing file is created on first write with a default preamble (`# Agent Memory` for `MEMORY.md`, `# User Profile` for `USER.md`); an empty `## Entries` section renders the placeholder line "No tool-managed memory entries are recorded yet." This first-write creation is the **only** way these files come into being — nothing seeds them ahead of time.
- `read_prompt_files(workspace, mode)` embeds each selected file's *entire* raw content (preamble + `## Entries` + suffix) as one `<file name="...">...</file>` block, joined with a blank line; `agent_user` orders `MEMORY.md` before `USER.md`. A **missing** selected file is rendered from its default "no entries" content (never omitted, never created); only `off` (no files selected) returns `""`. This is **only** the file contents — the `<memory>` wrapper and the guidance live in the `memory:guidance` block declaration (see Prompt Block), and the marker that injects these files is `{generated:memory_files}`.
