...
- Chat pins entries with the selected mode. A mode change replaces only the Memory snapshot on the next Run; unchanged-mode file edits remain pinned until successful Compaction (`core/prompts/pinned_context.py`, `tests/core/chat/test_chat_prompt.py`).
