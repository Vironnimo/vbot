...
1. **Product check:** configure `image_generation` to `openai/gpt-image-2::subscription`, generate one image through the image tool or task-model RPC, and expect a persisted artifact reachable through `/api/images/artifacts/{id}`.
