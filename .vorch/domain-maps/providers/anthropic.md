...
- The Messages renderer derives native Tool failure signaling only from the exact canonical Tool Result envelope: `ok: false` adds `is_error: true`, while successful and legacy/non-envelope Results omit the flag. The block's content is the Result's Model-facing text (see `request-policy.md`); canonical Session history keeps the JSON envelope. This is an Adapter-owned target-wire projection.
...
- Tool `input_schema` is validated and copied (`core/providers/tool_schema.py` -> `sanitize_anthropic_tool_input_schema`) before it reaches the wire. Anthropic requires the schema root to be an object with `type: object` and 400s a root union (`oneOf`/`anyOf`/`allOf` with no `type`) - verified live 2026-07-09. The validator rejects an invalid root but never rewrites a valid canonical schema. Anthropic only enables strict Tool use when `strict: true` is explicitly requested, so vBot omits that field and keeps runtime argument validation authoritative. `github_copilot_messages` shares the same validation boundary - do not add per-adapter schema rewriting.
