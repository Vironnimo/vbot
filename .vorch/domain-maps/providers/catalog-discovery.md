...
Connections with `auto_refresh: true` are refreshed by `Runtime.maybe_refresh_local_catalogs()` only while enabled and usable. Startup triggers a background sweep; `model.list` waits within a short budget; sweeps are throttled, including failures, so an offline local server is not probed on every picker open. A sweep copies, validates, commits, and discards the complete Model DB on the `local-catalog` worker pool and swaps the live registry through `ModelRegistry.reload_async`, so a picker open never blocks the Event Loop on catalog files; the staged copy is discarded even when the sweep is cancelled.
