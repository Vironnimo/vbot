...
- vBot currently resolves the two Providers independently: Go reads `OPENCODE_GO_API_KEY`, Zen reads `OPENCODE_API_KEY`, with no cross-Provider credential reuse. A read-only live check using the development Go credential returned HTTP 200 from `GET https://opencode.ai/console/api/config`; an invalid control key returned 401. This establishes shared account authentication, not paid Zen inference entitlement. No credentials or configuration were changed.
...
- The public Zen listing contains `mimo-v2.6-flash-free`, but `_opencode_zen_profiles.py` lacks its exact protocol entry, so vBot discovery skips it. Refresh alone cannot add it. The authoritative [OpenCode catalog](https://models.opencode.ai/api.json) advertises zero input/output/cache-read prices, Context 200000, and output 32000 for that free id, versus Context 1048576 and output 131072 for Go's `mimo-v2.6-flash`. These are catalog facts, not measured limits; never copy Go's capability profile merely because the base Model name matches.
- Deferred integration work: shared credential enrollment without mixing billing routes; current protocol/catalog reconciliation; and explicit fatal access classification for `FreeTierError`. The current Zen classifier special-cases only `RegionError` on 403, so the free-tier restriction falls through to `ProviderAuthError` even though replacing the key does not remove the client restriction. This investigation changed documentation only.
