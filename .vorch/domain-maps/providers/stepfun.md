...
- `step-3.7-flash` and `step-router-v1` accept `low`/`medium`/`high`; `step-3.5-flash-2603` accepts `low`/`high`; base `step-3.5-flash` has reasoning but no documented effort control, so selected effort is omitted. Reasoning replay inherits the shared `full_history` default; any demonstrated incompatibility must be encoded as an explicit Provider/Model override.
