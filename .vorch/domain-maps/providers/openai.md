...
- Subscription catalog GETs use the same rejected-token getter capability with one recovery per request. Discovery keeps the selected Account's getter instead of freezing its access token before fetching; Authorization and `chatgpt-account-id` are rebuilt after recovery. The policy now belongs to the shared `OAuthRequestRecovery` in `token_getter.py`, also used by the other OAuth Providers and by OpenAI Usage; static API-key credentials have no recovery capability. Regression coverage lives in `tests/core/models/test_discovery_provider_refresh.py` and `tests/server/rpc/test_connection_methods_oauth.py`.
