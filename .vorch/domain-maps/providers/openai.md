...
- Subscription catalog GETs use the same rejected-token getter capability with one recovery per request. Discovery keeps the selected Account's getter instead of freezing its access token before fetching; Authorization and `chatgpt-account-id` are rebuilt after recovery. The policy is Adapter-declared and does not enable auth retries for API-key or xAI catalog Connections. Regression coverage lives in `tests/core/models/test_discovery_provider_refresh.py` and `tests/server/rpc/test_connection_methods_oauth.py`.
