...
- The shared default classifies HTTP 401/403 as fatal auth errors, 429 and 502/503/504 as retryable, and Provider POST 500 as fatal; concrete Adapters or task clients can add verified codes through their explicit extra-status policy (Anthropic and embedding task requests opt into 529). Billed non-idempotent task generation applies the stricter `TaskRequestRetryPolicy` described below instead of blindly inheriting gateway retries. Retryable errors can carry parsed `Retry-After`, used as a capped floor over backoff.
