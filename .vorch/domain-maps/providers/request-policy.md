...
Chat owns history shaping. `full_history` survives only when the persisted `reasoning_scope` exactly matches the resolved Provider/Model/Connection/Account identity; an interrupted Assistant turn, Model switch, Connection or Account switch, run-local fallback, and textual vBot Compaction are hard boundaries. Interruption preserves native Reasoning only as canonical evidence and replays readable work solely through the provider-neutral Continuation reminder, never through `reasoning` or `reasoning_meta`. `phase` is semantic assistant history, not opaque reasoning state, so it survives ordinary request shaping and Compaction. Adapters serialize whatever reasoning survives shaping and must not apply a second history-wide strip except for a documented wire incompatibility such as explicitly disabled thinking.
