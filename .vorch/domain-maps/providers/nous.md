...
- Nous reasoning uses `reasoning: {enabled: true, effort?: ...}` for an enabled/on effort. A disabled intent omits the field; it does not send `enabled: false`. Replay stays the conservative shared `current_run` policy because Nous publishes no full-history replay contract.
