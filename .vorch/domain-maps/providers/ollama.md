...
- Streaming: `"stream": true` → **NDJSON lines** (one JSON object per line, no SSE framing); the final line has `"done": true` and may carry the usage counters. The adapter maps chunks to the normalized deltas; a stream ending without a done chunk is a `NetworkError`; an in-band `{"error": …}` line is a fatal `ProviderError`.
...
- Tool results replay as `{"role": "tool", "content", "tool_name", "tool_call_id"}` when canonical identity is available; assistant reasoning replays via the `thinking` field.
...
Ollama thinking controls and replay are Model/template-specific. `/api/show` `capabilities` containing `"thinking"` marks support; unknown support leaves `think` absent because Ollama rejects it on non-reasoning Models. Model capability metadata chooses binary or level control: active level intent sends the snapped level string, including Cloud Models whose catalog exposes `max`; off sends `think: false` except for GPT-OSS Models, which reject boolean false and therefore omit `think`.
...
- Direct Cloud inference uses the documented native API with `Authorization: Bearer`; authenticated Chat including streaming and Account usage are live-verified, while Tool Calls and reasoning still need credential-backed live probes (FLAGGED.md).
- Direct Cloud may omit `prompt_eval_count` on a follow-up response while still reporting `eval_count`. The adapter preserves each valid counter independently; Chat estimates only the missing canonical field and records field-level provenance. Never replace the reported output with an estimate merely because input usage is absent.
