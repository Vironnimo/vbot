...
`DeviceFlowEngine` owns OAuth device sessions, polling cadence, provider-specific exchange, terminal errors, and token persistence. `connect()` requests authorization and registers owned polling before returning; `is_flow_active()`, `cancel_flow()`, and `aclose()` hide task storage and drain cancelled/replaced polls on shutdown. Connection RPCs use this managed lifecycle against declared OAuth metadata; UI/CLI code must not reproduce flow policy. `device_flow` selects the standard OAuth2 shape, OpenAI Codex's two-step device authorization, or MiniMax's PKCE/user-code variant; provider-specific secrets remain engine-internal.
