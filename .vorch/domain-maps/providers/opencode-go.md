...
- Tool schema strictness is a verified per-Wire capability, not a Provider-wide assumption. The OpenAI route remains `best_effort`: a live `deepseek-v4-flash` request on 2026-07-28 rejected `strict: true` with HTTP 400. The Messages route uses the shared `anthropic_strict` renderer: the same closed object-root schema was accepted live with one schema-valid Tool Call by all seven current Anthropic-route Models on 2026-07-28. The shared renderer still downgrades the complete request to best effort when the Tool set exceeds 20 or violates an Anthropic strict-shape constraint; vBot runtime argument validation remains authoritative in every mode.
