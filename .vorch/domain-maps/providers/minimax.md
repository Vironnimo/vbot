...
- Reasoning replay inherits the shared `full_history` default for known and unknown Models unless an explicit top-level override narrows it. On key Connections the OpenAI wire round-trips `reasoning_details`; on the subscription Connection the Messages wire round-trips complete signed `thinking` blocks. **Neither path has been probed against the live MiniMax API in this environment**; wire behavior is pinned by unit tests and live verification remains deferred in `.vorch/FLAGGED.md`.
