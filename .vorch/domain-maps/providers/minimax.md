...
- Reasoning replay is `full_history` only for exact Models covered by `MINIMAX_MODEL_FACTS` (currently M2.x and M3); unknown Models remain `current_run`. On key Connections the OpenAI wire round-trips `reasoning_details`; on the subscription Connection the Messages wire round-trips complete signed `thinking` blocks. **Neither path has been probed against the live MiniMax API in this environment**; behavior is pinned by unit tests and live verification remains deferred in `.vorch/FLAGGED.md`.
