...
- `file_delivery.py` separates Chat image embedding from browser disposition: HTML/SVG, PDF, text and recognized media open inline, other binaries download; `?download=true` forces saving with the original filename. Only sniffed text is refined by HTML/SVG suffix. Active documents receive an opaque-origin CSP sandbox allowing scripts and HTTPS presentation assets, but blocking API connections, frames and forms; filesystem-relative assets are not exposed. Keep this isolation when extending previews (coverage: `tests/server/test_file_delivery.py`).
