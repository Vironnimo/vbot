...
- Extension page operations and selected Run streams use `extensions_methods.py` and short-lived owner/page capabilities. Canonical history and owned Run detail pass through the existing public payload sanitizer; page metadata is not authority to select another owner or arbitrary RPC method. Coverage: `test_extensions_methods.py`, `test_app.py`, `test_file_delivery.py`.
