...
- `server/rpc/payloads.py` owns common public projections and recursive removal of opaque Provider metadata. It also consumes internal Assistant `output_files` uniformly for RPC, History, SSE, and WebSocket payloads: safe raster images become Markdown images, other regular files become Markdown links, and the internal reference field is always removed. `server/rpc/error_mapping.py` converts expected domain errors to stable provider-agnostic RPC errors; unexpected errors are not hidden.
...
- Public Run/history/SSE/WebSocket payloads recursively strip `reasoning_meta`, `output_files`, and other internal data through one shared projector. When file delivery is available, that projector replaces only the referenced Assistant lines with freshly signed `/api/files/` Markdown URLs; ordinary history filters `role: "note"`, while visible errors, checkpoints, run summaries, and takeover dividers follow the Chat/public-history contract.
