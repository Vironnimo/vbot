...
- `POST /api/rpc` accepts `application/json` containing `{method, params?}` and returns `{ok:true,result}` or `{ok:false,error:{code,message}}`, including malformed JSON/params after media-type validation. Non-JSON media types fail with HTTP 415 before dispatch. `server/rpc/dispatcher.py` owns envelope parsing; `server/rpc/methods.py` builds the static `METHODS` table from domain-indexed registries.
...
- The app-wide browser-origin guard runs before routing for both HTTP and WebSocket scopes. A supplied `Origin` must exactly match the request target's HTTP(S) scheme, host, and effective port; foreign, opaque (`null`), malformed, or duplicate origins fail closed. Origin-less requests remain valid for non-browser Accessors such as the CLI and Desktop wakeword worker. Do not replace this execution guard with CORS response headers, which do not prevent CORS-simple requests from reaching handlers.
