...
- **Agent tool `skill_manage`** (`core/tools/skill_manage.py`, internal, always-registered, *not* gated on having a skill): writes **only the calling agent's own home** `<data_dir>/agents/<id>/skills/` (no scope parameter), `author="agent"`; on success calls `invalidate_agent_skills(agent_id)` so the new skill is loadable by name in the same session. It is offered to the model **only for identity agents** — the prompt layer (`provider_tool_definitions`) gates it on a non-empty `workspace`, because a config/project agent has no private home to write to.
...
- The internal `skill` tool is **always** included in provider tool definitions — it loads or lists skills, both of which can change mid-session (authoring, opt-in, activation), so it is never gated on the agent already having one. The internal `skill_manage` tool is included **only for identity agents** (non-empty `workspace`), since its sole write target is the agent's own private home. Neither is controlled by `allowed_tools`, and both must stay out of normal tool lists and Agent tool toggles.
