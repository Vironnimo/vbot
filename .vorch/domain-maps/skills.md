...
**Not:** A removal notice (additions only), a replacement for the Prompt-Epoch Catalog, or the `skill` Tool's list mode. Successful Compaction folds current Skills into the refreshed catalog; announcements cover additions between those boundaries.
