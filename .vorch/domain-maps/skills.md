...
**Project-scoped pool.** A project run uses a *merged* registry: the project's own skills under `<cwd>/.opencode/skills/` scanned **first** (so a project skill wins a name collision with a bundled one), then the same bundled scan roots the global registry uses. The single seam is `runtime.skills_for(project_id)` — `None` returns the global registry (identity runs, byte-identical), a set `project_id` returns the cached per-project merge. The runtime caches these per `project_id` (like the resolver's Team cache) and drops them on the same triggers (cwd change, project removal, global `reload_skills`). All run-time skill consumers — prompt assembly, `/`–`$` triggers, the internal `skill` tool, and autocomplete — resolve through this one seam, so project skills never leak between projects or to the home agent.
