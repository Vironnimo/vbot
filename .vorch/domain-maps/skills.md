...
**Project- and agent-scoped pool.** `runtime.skills_for(project_id, identity_agent_id=None)` merges selected Project, global/bundled, and optional Identity-private layers. A Rooted Identity Agent or an Identity Session with explicit Project Context passes the effective Project id plus its own id, yielding agent > project > global > bundled; the scoped registry always allows the Agent's private Skills and the exact effective Project Skill set while the Agent's configured `allowed_skills` continues to govern other shared Skills. A Config Agent passes its Session Project plus no identity id and receives the same Project-derived effective names through its resolved allowlist. Live Runs, compaction, preview, activation, and autocomplete share this policy.

Skills are playbooks, not normal user-managed tools. A bundled Skill may package a script-backed capability without registering more Tools: its required binary gates catalog visibility, activation supplies the script path and operating contract, and the Agent invokes the script through `bash`. The registry exposes prompt metadata and internal activation metadata; actual activation is handled by the chat/tool pipeline. The ordinary `skill` Tool requires `name`: without `file_path` it activates that Skill's `SKILL.md`, and with `file_path` it returns one UTF-8 package file by relative path without activating it or exposing an absolute path in the file result. The separate zero-argument `skill_list` Tool returns available Skills grouped by origin; it is Session-scoped but accompanies an available `skill` from the first request so Reflection forks never alter the Tool prefix. A `skill` name miss rescans disk once so a hand-dropped Skill can activate or serve a file without a restart. User messages can also activate Skills deterministically through `/skill-name` at the start of the message or `$skill-name` anywhere before the Provider request is sent.
...
**Not:** A removal notice (additions only), a replacement for the Prompt-Epoch Catalog, or the live `skill_list` Tool. Successful Compaction folds current Skills into the refreshed catalog; announcements cover additions between those boundaries.
...
- **Agent Tool `skill_manage`** (`core/tools/skill_manage.py`, an ordinary allow-list Tool, always registered, not gated on already having a Skill): the public contract is one open flat object requiring `action` and `name`, with optional siblings `scope`, `content`, `file_path`, `file_content`, `old_string`, `new_string`, and `replace_all`. Actions are `create`, `edit`, `patch`, `write_file`, `remove_file`, and `delete`; the handler requires and accepts only each action's applicable fields. `content` carries complete `SKILL.md`; `file_content` carries one complete UTF-8 support file; `file_path` is a relative package path; patches use `old_string` / `new_string` plus optional `replace_all`. The optional `scope` is handler-defaulted to `own` (the calling Identity Agent's private home); `global` is used only when the user explicitly requested a Skill shared by all Agents. The model-facing schema emits neither branch/closure keywords nor string/Boolean defaults. Every successful action invalidates that Agent's registry or reloads the global pool and emits one content-free control-plane event. Project/repo and bundled Skills are never targets. `skill_manage` remains identity-only through `IDENTITY_ONLY_TOOLS` at both dispatch and prompt visibility, is toggleable for Identity Agents, and is excluded from the Project Tool Whitelist.
...
- Script-backed bundled Skills may declare their own non-core binary dependency. `browser-use` wraps `playwright-cli`; `computer-use` wraps `cua-driver`. Neither wrapper installs or upgrades its backend, and an absent binary keeps only that Skill unavailable.
