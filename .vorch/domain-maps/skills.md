...
Policy mutations strictly load the existing document and reject malformed files without replacing them with the tolerant read fallback. Writes validate trigger-safe Skill names and canonical Agent ids before persistence; sharing requires at least one receiver other than the owner.
