...
- Activation helper behavior (`load_skill_content`): read the skill body after YAML frontmatter, substitute `{baseDir}`, scan the resource directories, and build the full `<skill_content>` payload (skill directory + resolution note + resources + body) for session storage. The internal `skill` tool result returned to the model is only a minimal status envelope (`status`, `directory`, `resources`); it must not repeat the full skill body.
