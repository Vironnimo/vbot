...
`core/skills/authoring.py` (`SkillAuthoringService`) is the single validated write core behind the Agent Tool, RPC, WebUI, CLI, and `/learn`. It operates on an already-resolved target root with direct install/create/edit/patch/delete/write_file/remove_file methods; each success publishes immediately. Install prepares a complete package, preserves original file bytes and frontmatter, and publishes it through the same write lock; it neither executes package contents nor changes access policy. Create/edit/SKILL.md patches reuse the loader's lenient document parsing and canonicalize complete YAML frontmatter, stamping `author` plus optional `source` under `metadata.vbot` and writing atomically; a missing name fills from the trigger-safe directory name and an explicit different name is rejected. Support-file authoring still accepts UTF-8 only, confined to `scripts/`/`references/`/`assets/`; patches default to `SKILL.md`, target one support file otherwise, require unique match unless `replace_all=true`. Patch matching normalizes line endings (CR/CRLF tolerated in `match`), non-unique matches report their line numbers, and edit/patch/write_file preserve the target file's detected line-ending style. Every path stays under the target root; protected bundled roots are refused. Failures raise `SkillAuthoringError` with surface-neutral diagnostics.

The Skill owner opts into call-field formatting, known action/envelope forms, and
the explicit `file_pth` alias for `file_path`; generic schema validation does not
guess field names, actions, or Skill names. Explicit empty `content` remains
...
- Both Skill Tools repair padded names, matching enclosing package-path quotes, and backslash separators before validation. This is owner-specific relative package-path formatting, not a fallback based on which filesystem path exists; `skill` can read arbitrary UTF-8 package files while `skill_manage` retains its narrower authoring paths. Skill names are never fuzzy-matched. Only optional `name`/`file_path` empties mean omission, after duplicate-value conflicts are checked; content empties remain meaningful. Repaired targets retain the same package containment and write-scope checks.
