...
- The catalog is **session-pinned**: snapshotted on a session's first build and reused for that session's lifetime, so a skill written mid-session never changes a running session's `<available_skills>` or its prompt cache. Skill *activation* and `/`–`$` triggers stay live (a new skill is loadable at once); a new session pins a fresh snapshot. Mechanics live in `prompts.md` (`PinnedSkillCatalog`) and `chat.md`.
