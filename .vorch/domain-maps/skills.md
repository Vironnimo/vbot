...
- `pyyaml` (core dependency) parses SKILL.md frontmatter. Browser Use and Computer Use execute through opt-in bundled Extensions (see `extensions/browser-use.md` and `extensions/computer-use.md`). Both bundle workflow Skills; the Skills supply instructions, while each Extension's Tool grant controls execution.
