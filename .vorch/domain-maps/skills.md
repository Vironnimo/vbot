...
- Both Skill Tools repair padded names, matching enclosing package-path quotes, and backslash separators before validation. This is an owner-specific package grammar (`SKILL.md`, `scripts/`, `references/`, `assets/`), not a fallback based on which filesystem path exists. Skill names are never fuzzy-matched. Only optional `name`/`file_path` empties mean omission, after duplicate-value conflicts are checked; content empties remain meaningful. Repaired targets retain the same package containment and write-scope checks.
