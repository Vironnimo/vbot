...
Policy mutations strictly load the existing document and reject malformed files (including a superseded schema version) without replacing them with the tolerant read fallback; the RPC edge reports that refusal as a `domain_error` carrying the file diagnostics, not as an unexpected failure. No automatic or bundled converter exists for older policy versions. Writes validate trigger-safe Skill names and canonical Agent ids before persistence; sharing requires at least one receiver other than the owner.
