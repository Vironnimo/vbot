...
`core/skills/authoring.py` (`SkillAuthoringService`) is the single validated write core, shared by every authoring surface. It operates on an **already-resolved target root** (scope→root resolution is the caller's job) and offers `create` / `edit` (full `SKILL.md` rewrite) / `patch` (one unique `old_string`→`new_string`) / `delete` plus `write_file` / `remove_file` for `scripts/`/`references/`/`assets/` support files. It reuses `validate_skill_metadata` + `parse_vbot_requirements` (reject missing name/description and malformed requirements), confines every path strictly under the target root (rejecting traversal, illegal names, and anything outside `SKILL.md` + the resource dirs), refuses a target at/under a protected (bundled `resources/skills`) root, and stamps provenance — `author` (`agent`/`human`) and optional `source` — under `metadata.vbot` in the written front matter (beside `requirements`). It is stricter than the lenient loader: an authored skill's front-matter `name` must equal its directory, and must match `SKILL_NAME_TRIGGER_PATTERN` (letters/digits/`-`/`_`, starting with a letter or digit, ≤64 chars — `core/skills/skill_validator.py`) so it stays loadable via the `/name`/`$name` chat triggers, which use the same character class. Failures raise `SkillAuthoringError` carrying `diagnostics` that each surface forwards.

**Write-scope boundary (v1 is data-dir only — vBot never writes the repo as runtime state):**
- **Agent tool `skill_manage`** (`core/tools/skill_manage.py`, an ordinary allow-list tool, always-registered, *not* gated on having a skill): a `scope` argument picks the target — `own` (**default**) writes the calling agent's own home `<data_dir>/agents/<id>/skills/`, `global` writes the shared user pool `<data_dir>/skills/`; `author="agent"` for both. The default is private; the tool's description instructs the agent to choose `global` **only when the user explicitly asked** to make the skill global — a prompt-level guideline, not a hard gate (a global write is reversible and visible in the WebUI skill editor). The project/repo scope is never a target — repo skills use the ordinary file tools. After the write it runs the matching invalidation so the skill is loadable by name in the same session: an `own` write drops that agent's cached registry (`invalidate_agent_skills(agent_id)`), a `global` write reloads the whole registry (`reload_skills`, since the global pool is layered under every project/agent registry). It is **identity-only**: even when an agent's `allowed_tools` would include it (e.g. via a wildcard), `IDENTITY_ONLY_TOOLS` withholds it from a config/project agent (empty `workspace`), which has no private home and never authors skills — enforced at both the dispatch-time allowlist (`effective_agent_allowed_tools`, see `tools.md`) and the prompt layer's visibility pass (see `prompts.md`), so a widened `allowed_tools` can neither advertise it to the model nor have it actually dispatched. For identity agents it is a normal per-agent toggle, and it is excluded from the Project Tool Whitelist surface entirely.
...
- The prompt catalog includes only skills allowed for the agent (`agent.allowed_skills`) and currently `available`, filtered against the run's agent-aware project-scoped registry (`build_system_prompt`/`provider_tool_definitions` take an optional `skill_registry` plus a pinned `skill_catalog` snapshot; the chat loop passes `runtime.skills_for(skill_project_id, identity_agent_id)` (agent id for identity runs only)). An agent's own private skills are always-allowed for it. Command autocomplete (`chat.commands` RPC) **is** agent-scoped: given an optional `agent_id` (a bare id or an `agent@projekt` address) it resolves the agent and derives the scope through the same shared policy a run uses (`resolve_prompt_project` + `resolve_skill_scope`, see `projects.md`), then returns `skills_for(skill_project_id, identity_agent_id).filter_allowed(agent.allowed_skills)` — so a rooted identity agent's suggestions include its home project's skills exactly like the run would activate them, and a project address names a config agent with no private-skill layer; with no address it returns the global list. The prompt-preview RPC (`prompt.preview`) resolves its skill registry through the same two calls. The WebUI passes the active agent's address. `skill.list` still returns unavailable skills with requirement details (availability state plus missing/optional-missing) so the user can fix local prerequisites.
