...
- `cli/rpc_client.py` posts `{ method, params }` to `/api/rpc`, normalizes success/error envelopes into `RpcPayload`, preserves server RPC error code/message text when present, and reports malformed transport responses as command failures. Calls use a 10s timeout by default; methods in `_LONG_RUNNING_METHODS` (`model.refresh_db` and `session_store.snapshot_create`) use an unbounded read with short connect/write/pool timeouts, so accepted long local work can finish while an unreachable server still fails fast.
...
- `session-store status` prefers the operator-safe live health projection through RPC and falls back to a local stopped-store projection, including `unrecoverable`, when Runtime cannot start; `snapshot list|verify` are local read-only checks; `snapshot create` uses the running Runtime. `snapshot restore` requires `--yes`, stops and verifies the exact vBot target through the shared systemd-aware lifecycle owner when it was running, uses the Sessions-owned incident/restore state machine, verifies a normal open plus read/write path, and restores the prior running state only after success. `incident acknowledge` clears only the warning state and preserves snapshots and quarantine evidence. Legacy JSONL conversion is owned solely by `scripts/converters/session_sqlite.py` and is never an update or startup path.
...
- Shared help explains local target selection, and management examples include the `vbot` executable. The parser regression in `tests/cli/test_cli_main.py` parses every published description example without dispatching it; prose assertions are unnecessary.
...
- Process termination is allowed only after `/health` confirms the target is vBot or a current control record's PID plus process creation time identifies the exact server process after its listener has closed. Local listener lookup matches the resolved host/address and port, not port alone; wildcard listeners match only when they can receive the requested traffic.
