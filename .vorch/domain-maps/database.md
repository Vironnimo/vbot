...
Owners today: the Session store (`sessions.md`, `core/sessions/_store_schema.session_database_spec`), the Provider usage history (`providers/usage.md`, `core/providers/usage_history.provider_usage_database_spec`) and the Channel state (`channels.md`, `core/channels/_state_schema.channel_database_spec`). Decisions, swarm, Statistics and the Recall indexes still open SQLite on their own and are expected to move onto the kernel in later phases; `APPLICATION_IDS` already reserves their ids.
...
- Names: `lower_snake` for core owners, `ext.<owner>.<name>` for Extensions. Canonical paths are fixed by name (`canonical_database_path`): `sessions` -> `sessions.db`, `provider_usage` -> `provider-usage.db`, `channels` -> `channels.db`, `ext.<owner>.<name>` -> `extension-data/<owner>/<name>.db`.
