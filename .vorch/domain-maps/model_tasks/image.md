...
Provider-neutral image generation/edit execution and artifact storage for the configured `image_generation` task-model binding.

## Overview

`core/model_tasks/` (`image*.py`) owns text-to-image and source-image editing after Settings has selected one concrete task-model target. It resolves the `image_generation` binding through `TaskModelService`, loads and validates local source paths when supplied, merges stored options over backend defaults, rejects local targets, and routes provider targets to `ProviderImageClient`. It does not own model discovery, model catalogs, settings validation, UI controls, or the `image_generation` tool schema; those live in `core/models/`, `core/model_tasks/`, `core/settings/`, `webui/`, and `core/tools/image.py`.

## Interfaces

- `ImageService(model_tasks, runtime, data_dir)` — runtime-owned service; stores artifacts under `<data_dir>/images/`. Keeps the injected `model_tasks` reference to resolve the target model for per-call option routing.
...
- `await ProviderImageClient.generate(prompt: str, *, options: dict, input_images=()) -> ImageGenerationResult` — provider-bound HTTP entrypoint; supports OpenRouter generation/edit through the dedicated image API (`POST /images`), OpenAI Platform generation (`/v1/images/generations`) and editing (`/v1/images/edits`), and text-only OpenAI subscription image generation through Codex Responses (`/codex/responses`).

`ImageInput` contains one loaded source image's provider-facing `filename`, sniffed `media_type`, and raw `data`. `ImageGenerationResult` contains `images: tuple[bytes, ...]`, one `media_type` for the result set, `model`, optional provider `usage`, and optional raw response payload. `ImageArtifact.to_dict()` returns `{ id, kind: "image", filename, media_type, size_bytes, url, index }`, where `url` is `/api/images/artifacts/<id>` and is not an attachment URL.
...
- The service reads bytes once, sniffs the media type through `core.attachments.sniff_media_type`, rejects missing/directories/unreadable/non-image files with `ImageInputError`, and supplies a media-type-derived extension when an Attachment blob path has no suffix.
