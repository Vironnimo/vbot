...
- `await ProviderImageClient.generate(prompt: str, *, options: dict) -> ImageGenerationResult` — provider-bound HTTP entrypoint; supports OpenRouter (the dedicated image API, `POST /images`) and OpenAI (`/v1/images/generations`).
