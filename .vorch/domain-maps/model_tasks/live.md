...
- Browser microphone access needs HTTPS or localhost. Non-OpenAI providers are not implemented. The Desktop makes the remote HTTP servers it knows at startup secure contexts; a server added later shows the `desktop_restart_required` notice until the Desktop app restarts. Desktop wakeword Voice no longer conflicts with Live: it pauses during the call.
