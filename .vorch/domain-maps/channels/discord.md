...
- An addressed group text that replies to another non-bot participant's attachment carries its raw `MessageReference` through the engine FIFO. After admission, `build_quoted_message` uses the resolved/cached message or fetches `reference.message_id` from the channel, then downloads and classifies its attachments; the engine adds the current question, original sender/role marker, original message content, and attachment blocks to one current-turn Run. Bot-authored references and referenced plain-text messages retain the normal reply path; an unresolved/deleted reference yields the quoted-message-unavailable marker without preventing the question's Run.
...
- Discord message content is split at 2000 code points by the shared Markdown-aware splitter (`channels.md` -> Conventions). Files are sent in batches of at most 10; text chunks and file batches share sends by index, so the caption appears only on the first send.
...
- Target ids resolve from the client cache first, then `fetch_channel()`. `ensure_outbound_session()` makes no Discord API call: it uses only a target already seen, cached, or resolved by the preceding send, and otherwise fails with `ChannelConfigError`.
