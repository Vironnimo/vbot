...
- Built-in Command behavior is platform-neutral and lives in Chat/`ChannelConversationEngine`; Discord supplies parsing, authorization facts, reply targeting, and `platform_display_name="Discord"`. Bare `/agent` and `/agent <target>` both return exactly `The /agent command is not available through Discord.` before Queue admission or command execution. Commands bypass history backfill because they are already addressed; they still require `owner_user_ids` authorization in guild channels.
