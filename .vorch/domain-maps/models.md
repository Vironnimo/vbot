...
Per-model wire **facts** live in a **Provider-scoped** `metadata` blob keyed by the underscored Provider id, so one Provider's quirk never pollutes the schema for all: `metadata.opencode_go.protocol`, request/response Reasoning carriers, `metadata.mistral.prompt_mode`, and connection-keyed `metadata.openai.wire_policies` such as protocol and public reasoning context. Replay **scope** is the cross-provider exception configured through override files (`Model.reasoning_replay`, root Provider `reasoning_replay`); the precedence chain and the do-not-hide rule are owned by `providers.md` -> Boundaries and invariants. The wire **mechanics** stay in the Adapter - it reads facts via its injected `model_lookup` and owns only the *how*. The full convention lives in `providers/request-policy.md` -> Adapter ownership.
...
**Local catalogs auto-refresh.** A Connection with `auto_refresh: true` (Ollama `local`) gets a throttled background sweep (`Runtime.maybe_refresh_local_catalogs()`) that probes only enabled Connections, keeps the last known catalog on failure, records per-Connection reachability (surfaced as `reachable: false` on `model.list` when every usable Connection is a failed auto-refresh Connection), and republishes the runtime Model DB without any models.dev fetch. Full semantics live in `providers/catalog-discovery.md` -> Local auto-refresh and reachability.
...
- **Refresh is dumb, Load is smart** (see Terms) - never push merge/join logic into Refresh, never make Load fetch.
