...
The system root is the tracked release database at `resources/models/`; the runtime root is the instance database at `<data_dir>/models/`. Each root is a complete set containing generated catalogs, raw inspection data, canonical and Provider override files, plus `manifest.json` with `schema_version`, UTC `refreshed_at`, and `source`. Normal user refresh publishes the runtime root; the maintainer script publishes the system root. Load compares compatible manifests and selects the newer entire root, with the system root winning an equal timestamp; a missing or incompatible runtime manifest falls back to system. Files are never selected or merged independently across roots, so the catalogs and Overrides visible in the active root are exactly the files Load uses.

Both refresh targets are staged and validated before atomic directory replacement. A failed fetch, validation, or publish leaves the previously published root intact. Runtime refresh starts from the currently active complete root, so files not regenerated by that particular refresh — including `*.overrides.json` and raw dumps — travel with the refreshed database.
...
- `ModelRegistry.load(resources_dir, runtime_models_dir=...)` selects one complete database root, assembles from only that root's layers, and caches by both resolved locations. `is_provider_file` decides what counts as a provider file — it excludes `manifest.json`, `*.raw.json`, `*.overrides.json`, and the canonical `models.json`/`models.overrides.json` (shared with the offline validator so the classification can't drift).
