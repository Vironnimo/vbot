...
`api.register_tool(name, description, parameters, handler, *, internal=False, requires_opt_in=False, display=None, ready=None, readiness_hint=None, result_schema=None, parallel_safe=True, open_input_schema=False, argument_normalizer=None, family=None)` declares a normal Tool. Runtime applies Extension Tools after built-ins into the same `ToolRegistry`; they then use the same compiled canonical input contract, optional success-data contract, fingerprint, allowlist, Prompt, Provider definition, dispatch, and result-envelope contracts as built-ins. `open_input_schema=True` permits a model-facing parameter object that omits `additionalProperties`, with unknown-field and conditional validation owned by the handler; the default remains closed for backward compatibility with existing Extensions. An optional pure `argument_normalizer` repairs domain-specific shapes on a copied request before canonical schema validation; the shared dispatch authorization and profile checks still apply. Follow `tools.md` for the intent-preservation contract: no nearest-target substitution, generic field/enum typo inference, or payload rewriting. Invalid Tool registration is diagnosed and skipped without preventing that Extension's other capabilities from applying.
