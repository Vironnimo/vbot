...
`api.register_tool(name, description, parameters, handler, *, internal=False, requires_opt_in=False, display=None, ready=None, readiness_hint=None, result_schema=None, parallel_safe=True, open_input_schema=False, unadvertised_parameters=None, argument_normalizer=None, family=None)` declares a normal Tool. Runtime applies Extension Tools after built-ins into the same `ToolRegistry`; they then use the same compiled canonical input contract, optional success-data contract, fingerprint, allowlist, Prompt, Provider definition, dispatch, and result-envelope contracts as built-ins. `open_input_schema=True` permits a model-facing parameter object that omits `additionalProperties`; its root properties stay the complete parameter list, so dispatch rejects unknown arguments and the handler owns conditional validation. The default remains closed for backward compatibility with existing Extensions. `unadvertised_parameters` declares accepted root parameters that are never offered (see `tools.md` -> Registration and dispatch rules). An optional pure `argument_normalizer` repairs domain-specific shapes on a copied request before canonical schema validation; the shared dispatch authorization and profile checks still apply. Follow `tools.md` for the intent-preservation contract: no nearest-target substitution, generic field/enum typo inference, or payload rewriting. Invalid Tool registration is diagnosed and skipped without preventing that Extension's other capabilities from applying.

Opt-in declarations use the shared Tools policy contract (`tools.md`), including on live catalogs; no Extension-specific grant store is required.

`api.register_tool_family(family_id, label)` declares one Extension-local presentation family. A Tool joins it with `family=family_id`; apply namespaces the persisted registry id as `extension:<extension-name>:<family-id>`, so equal local ids from different Extensions never merge. The registry owns the label and `tool.list` projects it as `family_label`. An undeclared or invalid family reference is diagnosed but does not disable the Tool: it registers without a family. Family membership changes only grouping and current-member bulk actions, never activation or permission semantics.

An existing built-in or earlier Extension Tool name wins. The losing declaration is skipped and diagnosed without failing the Extension. `ready` must be cheap and I/O-free; a false result leaves the Tool registered but hides it from model-facing surfaces. `readiness_hint` is optional English guidance exposed by `tool.list`. Extension Tool siblings run concurrently by default within shared limits; `parallel_safe=False` declares a whole-Tool ordering barrier. Active `tool_call` and `tool_result` callbacks are serialized through a batch-local lock because their transformations and replacements are not assumed commutative, while parallel-safe Tool handlers remain concurrent.
...
Live disable removes the Extension's hook and interaction entries, unregisters its applied Commands and only Tools whose live handler identity matches the declaration, removes its now-unused namespaced Tool Family metadata, fires shutdown, and clears declarations. Full reload removes old Command registrations and uses identity-safe Tool and Tool Family removal before discarding the entire old registry. Collision-skipped capabilities must never be removed from their real owner.
