...
- A completion is marked read only after the exact `run_id` appears in the displayed Session's history or terminal Run events. The `session.mark_read` acknowledgement is Run-specific, and stale list responses or acknowledgements must not clear or resurrect a newer result. The Session drawer preserves the unread projection so the blue Agent state resolves to a concrete Session.
