...
- The expanded OpenRouter row owns a routing editor with a global scope and optional full per-Model overrides. `automatic` preserves Sticky Routing while permitting blocks, `allowed` restricts candidates without imposing order, and `ordered` exposes priority controls plus an explicit cache-affinity warning because OpenRouter ignores Sticky Routing when `provider.order` is present. Global blocks remain effective inside Model overrides.
