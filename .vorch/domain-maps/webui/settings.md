...
- Real Wakeword activation consumes the bridge's explicit `{ enabled, error_code }` result. A rejected STT readiness check keeps the toggle off and renders the actionable reason in the Voice panel; the app-level sequence-numbered event consumer independently raises global Toasts for sent commands, no-speech, transcription failure, and fatal errors so the active main view does not hide Voice feedback.
- Bridge discovery, event subscriptions, and media/runtime listeners require explicit timeout and cleanup behavior. UI state must not claim the wakeword runtime is active merely because the saved setting is enabled, and a disabled activation that failed must retain its red error indicator until the next state transition.
