...
- The helper does not replace the editor's reconciliation or backend validation. A successful response for an older draft must not mark a newer draft saved. Pending-state and save decisions follow the current persisted baseline through `hasChanges`, never a cached formerly successful draft: an external refresh can make the same value dirty again. Successful saves must reconcile that baseline before resolving. `hasChanges`, the scheduling guard, and the save's no-op check must use one comparison: when the payload normalizes a value (a cleared number becomes its default), compare normalized draft and baseline, and reconcile the visible draft through `runSettingsSave`'s `applyResult`. A raw comparison against a normalized payload stays pending after every successful write, so the transition flush repeats identical saves until it fails (Sub-Agent regression: `SettingsView.test.subagents-and-voice.test.js`, mounted through `AutosaveContextHost.svelte`).
