...
- Expanded Team rows show the effective Sub-Agent targets as read-only repository-owned policy: none, the whole current Team, or the explicit member list. The Projects UI has no vBot override tier for this field, and full access never means outside the Project Team.
