...
- Tool and Skill controls use the shared catalogs, but the browser does not own Project Tool policy. `tool.list` marks each Tool with server-owned family, activation, constraint, readiness, `project_configurable`, and `project_configurability_reason` metadata. `buildToolToggleList()` omits entries marked non-configurable without maintaining a Tool-name list and preserves family metadata so the Project Tool Whitelist groups the same real families plus `Individual Tools`, without importing Agent-policy semantics into the ceiling editor. Team Tool overrides reuse `ToolAccessEditor` with the whitelist as a hard ceiling. Catalog membership, Project whitelist, Agent policy, and effective runtime availability are related projections, not interchangeable state.
