...
The detail is one continuous page: Repository, Agent defaults, Team, auto-load files, Tools and Skills. The title scrolls with the content. Internal `components/projects/Project*Panel.svelte` components own local presentation/input helpers, not navigation topics; the parent retains selection and shared autosave. Team members use compact, single-line name/Model rows with descriptions only on hover/focus; overrides expand locally. Repository management actions belong in the Repository section, while Rescan sits beside Team. A tertiary Save follows all fields. At tablet/mobile widths the Project list becomes a horizontal selector. `ProjectDialogs.svelte` keeps the controller-backed add/remove/re-point forms.
...
- Context auto-load rows support handle drag-and-drop and ArrowUp/ArrowDown reordering with focus restoration and a live position announcement. The controller moves entries through the existing ordered `auto_load` draft and manage-project autosave; canceled, external, and stale-list drags do not change it. Source/tests: `ProjectsView.svelte`, `projectsView.js`, and `ProjectsView.test.js`.
...
- Editing a Team field autosaves the changed override; Clear override remains an explicit inheritance reset. `pendingOverrideChanges` and `savePendingOverrides` cover Model, temperature, thinking effort, Compaction and Tool policy together. Scan reconciliation updates untouched inherited fields and preserves other edited fields, so saving one override never materializes unrelated inheritance. The Tool Access editor is the deliberate exception to ordinary fallback semantics: an active `tool_access` override fully replaces the repository policy, may re-enable a repo-denied Tool inside the Project Tool Whitelist, and autosaves exact mode/allowed/denied state; Reset clears the override and restores repository policy without editing the repository. The frontend must not replace other inherited values with copied explicit values merely because that is what the row currently displays.
...
- Tool and Skill controls use the shared catalogs, but the browser does not own Project Tool policy. `tool.list` marks each Tool with server-owned family id and label, activation, constraint, readiness, `project_configurable`, and `project_configurability_reason` metadata. `buildToolToggleList()` omits entries marked non-configurable without maintaining a Tool-name list and preserves `family` plus `family_label` so the Project Tool Whitelist groups the same real Built-in and Extension families plus `Individual Tools`, without importing Agent-policy semantics into the ceiling editor. Project Whitelist and Team override views share `ToolCatalogEditor` presentation, search and family switches. Project bulk changes preserve selections outside the affected filtered group. Team overrides still use `ToolAccessEditor` policy adaptation with the whitelist as a hard ceiling; it translates clicks into exact mode/allowed/denied state before autosave. Catalog membership, Project whitelist, Agent policy, and effective runtime availability are related projections, not interchangeable state.
