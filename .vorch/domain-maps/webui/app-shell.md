...
- Replay-gap and epoch-change invalidations are published before awaiting read owners. Session-store status, Extension-page descriptors, Projects and Agents refresh independently, so a slow or failed optional projection cannot delay unrelated recovery (`appController.test.js`).
