...
Settings currently opens Providers and has a grouped, direct topic index: Models first, then Personal, Capabilities, Sessions & Memory, and System. Model & Thinking links to Agents' Shared defaults. One editor is visible at a time, with a topic heading and short description above flat, divided form rows; there is no enclosing feature card or second expansion step. Simple rows align labels left and controls right, stacking on mobile. The content measure is 860px; below 960px a topic picker replaces the sidebar. Search covers mounted hidden editors and opens a result directly. SettingsView.svelte and styles/settings/pages.css own this arrangement; settings.md records behavior and ownership.
