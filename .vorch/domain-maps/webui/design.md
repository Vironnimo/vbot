...
The bundled typefaces are Geist Sans (`--font-ui`) and Geist Mono (`--font-mono`) as local variable `woff2` files. Sans carries all interface text: labels, headings, tabs, buttons, status pills, badges, counts, dates, and table headers. Mono is limited to code-like values: ids, paths, URLs, Model ids, commands, code/JSON, Tool calls, log lines, and cron expressions. Numbers in tables and stat tiles use Sans with tabular figures. Small headings use sentence case and weight rather than uppercase letter-spaced labels. The current `--fs-*` scale runs from 12px (`mono-xs`) to 24px (`display`); body roles are 13px, 14px (the base), and 15px (Chat prose and composer). No readable text is set below 12px. Full values are in `styles/app/foundation.css`, rather than duplicated as a second token schema here.
...
Current configuration editors commonly combine debounced autosave and an explicit Save action; in Settings that action (`SaveButton.svelte`) shows a quiet "Saved" state once the draft is persisted. Entity creation, destructive confirmations, and write-only secrets use explicit operations. This is existing behavior, not a demand to add a redundant Save button to every new form. Draft preservation and navigation flushing are behavioral contracts: their owner and failure handling are in `settings.md` and `app-shell.md`.
...
Settings currently opens General and offers seven pages: General, Providers, Voice, Memory, Tools, Integrations, and System. The sidebar contains pages, while related settings appear together under section headings in one scrollport. Session titles sit with display preferences; speech Model choices sit with Voice, embeddings with Recall, and web search/reading with the other Tool settings. Providers has a contextual Shared defaults link; each Provider row toggles its details from anywhere on the row head, with a rotating chevron button as the keyboard control. Sections use spacing and dividers without enclosing feature cards; simple rows align labels left and controls right, stacking on mobile. The content measure is 860px; below 960px a page picker replaces the sidebar. Search results identify their owning page and jump to the matching section. SettingsView.svelte and styles/settings/pages.css own this arrangement; settings.md records behavior and ownership.
