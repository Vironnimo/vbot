...
- Two pseudo-options: `provider_options` (passthrough rendered only when advertised, sent nested as `provider.options`) and universal `extra_options` escape hatch (appended last, overriding authored keys - helpers shared via the task client).
