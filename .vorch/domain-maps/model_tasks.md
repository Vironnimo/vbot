...
Supported binding task types are defined by `SUPPORTED_TASK_TYPES` in `core/model_tasks/constants.py`: `speech_to_text`, `text_to_speech`, `image_understanding`, `image_generation`, `text_embedding`, and `video_generation`. `image_understanding` is the text-output-from-image task used by the route-gated `analyze_image` Tool.
...
- `TaskModelService.binding_is_usable(task_type)` is the live, non-mutating availability gate for route-dependent Tools. It rejects missing or stale bindings, unsupported task capabilities, missing or forbidden Connections, unusable credentials, and local targets the provider execution path cannot run.
- `TaskModelService.list_targets(task_type)` returns provider and local `TaskModelTarget` descriptors for one supported task type, sorted by kind, label, and id. Target expansion iterates usable provider connections, but skips a connection a model forbids via `Model.allows_connection(connection.id)` (the single source of the per-model connection rule: empty allowlist permits all, non-empty restricts) — connection-restricted models (e.g. OpenAI Codex models tagged `["subscription"]`) do not cross-product against other usable connections of the same provider.
...
Two schema-owned pseudo-options exist: `provider_options` (JSON, rendered when the model advertises passthrough keys; sent as the nested `provider.options` object) and the universal `extra_options` escape hatch (JSON, appended to every provider target schema for all four executed task types; merged into the request payload last, overriding authored keys — shared helpers `merge_extra_options`/`is_omittable_option` live in `core/providers/task_client.py`).

The per-task execution modules own final option interpretation and wire shaping: the per-task wire clients (`speech_providers.py`, `image_providers.py`, `embeddings_providers.py`) decide which options are sent to which API.
...
- The Settings UI currently renders rows for `speech_to_text`, `text_to_speech`, `image_understanding`, `image_generation`, and `text_embedding`. `video_generation` is accepted by backend validation and discovery, but has no complete UI/execution workflow yet.
...
- Option schemas are model-aware and data-driven: `option_schema_for(task_type, provider_id, target, *, model)` receives the resolved `Model` and renders image fields from `capabilities.task_options` (projected at refresh from the OpenRouter image API, hand-authored in `openai.overrides.json` for OpenAI native — there are no hardcoded model-ID or family profiles in code anymore). Discoverable flat facts (`supported_voices`, `supported_parameters`) also read from the `Model`. Voice selection for TTS uses `model.capabilities.supported_voices` when non-empty, falling back to provider-level choices (OpenAI canonical voices only for OpenAI, free-text input otherwise).
