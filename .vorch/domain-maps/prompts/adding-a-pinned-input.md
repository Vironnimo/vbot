...
3. `core/chat/_run_state.py::_CompactionPromptRefresh`, `core/chat/compaction_host.py`, and `pinned_context.py::replace_prompt_epoch_pins`: a successful Compaction replaces every pin wholesale when the new epoch starts - add the new meta key (with the same attributes the reader checks) to that replacement.
