...
- `webui/src/lib/terminalsView.js` owns Terminal Session start and errors, live-plus-retained-session and server-persisted launch-history reconciliation, selected-stream sequence/gap recovery, authoritative TUI mode/exit snapshot application, exact per-terminal xterm input batching, operator mutations, viewer geometry, and cleanup; it also hardens the live stream: a socket still in `WS_CONNECTING` past 8 s is treated as wedged and force-closed into the ordinary reconnect path, and per-terminal PTY frame output passes through a stateful sanitizer that reassembles ANSI escapes and UTF-8 code points split across WebSocket frames, collapses blank-line bursts, and drops a buffered partial escape at end of stream (held tails reset on every `terminal_ready`). `TerminalsView.svelte` rebuilds bounded xterm.js scrollback from the authoritative snapshot after mount/navigation/reconnect or a server resynchronization point, preselects the newest reusable command/arguments/working-directory configuration whenever New terminal opens, focuses every newly started Terminal Session for direct input, and makes any running terminal immediately writable when clicked regardless of owner. Each xterm viewer fits its tile by adjusting font size (clamped to the PTY bounds) and sends a debounced `terminal.resize` when the fitted grid differs from the session's known dimensions. A newly measured size is only sent after two unchanged fits, and those two fits must be separated by an animation frame — two measurements in the same turn (a tab remount's ResizeObserver plus the initial fit) are not confirmation. The viewer schedules the confirming measurement on the next frame so a one-shot layout change or remount still reaches the PTY, creates WebGL only after that paint, rebuilds the renderer if the host pixels or font size later change without a grid change, tears WebGL down after every snapshot write so a same-size remount snapshot cannot keep quads from the empty pre-snapshot grid, and drops WebGL for that host size when the backing canvas height is not a multiple of the row count — at 125% desktop scale the device-pixel box is often one pixel off the cell grid, which paints regular white row gaps until a later remount happens to land on a whole pixel. Terminal dimensions change only through an explicit Terminal start or resize operation. Finished retained sessions remain read-only history.
