...
- `webui/src/lib/terminalsView.js` owns active Terminal Session reconciliation, selected-stream sequence/gap recovery, input batching, resize debounce, operator mutations, and cleanup; `TerminalsView.svelte` renders the xterm.js control-desk view and keeps direct keyboard/paste input disabled until the operator explicitly takes control.
