...
- `webui/src/lib/terminalsView.js` owns manual Terminal Session start and errors, live-plus-retained-session reconciliation, selected-stream sequence/gap recovery, exact xterm input batching, resize debounce, operator mutations, and cleanup; `TerminalsView.svelte` rebuilds bounded xterm.js scrollback from the authoritative snapshot after mount/navigation/reconnect, starts manual sessions in focused direct-control mode, keeps live Agent-owned sessions in observe mode until a primary click takes control, and presents finished retained sessions as read-only history.
