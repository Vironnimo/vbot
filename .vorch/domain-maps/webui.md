...
- The Agents surface keeps `id` outside debounced `agent.update`: `AgentEditor.svelte` uses an explicit rename modal and `api.js::renameAgent`, then `AgentsView.svelte` replaces the roster entry and selection from the returned Agent. Other connected windows consume the Agents invalidation's `{old_agent_id,new_agent_id}` scope in `appController.js`, remap current/localStorage selection and in-memory old History entries first, then reload the authoritative roster.
...
- Run SSE carries the full ordered event stream for one Run. The app-wide WebSocket carries lifecycle summaries and invalidation signals, not high-volume Run deltas.
