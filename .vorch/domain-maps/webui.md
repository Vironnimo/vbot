...
- `webui/src/lib/terminalsView.js` owns manual Terminal Session start and errors, active-session reconciliation, selected-stream sequence/gap recovery, exact xterm input batching, resize debounce, operator mutations, and cleanup; `TerminalsView.svelte` renders the bounded xterm.js control-desk view, starts manual sessions in focused direct-control mode, and keeps Agent-owned sessions in observe mode until a primary click on the terminal takes control and focuses native keyboard/paste input.
