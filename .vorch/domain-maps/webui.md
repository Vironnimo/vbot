...
- `webui/src/lib/terminalsView.js` owns Terminal Session start and errors, live-plus-retained-session and server-persisted launch-history reconciliation, selected-stream sequence/gap recovery, authoritative TUI mode/exit snapshot application, exact per-terminal xterm input batching, operator mutations, and cleanup; `TerminalsView.svelte` rebuilds bounded xterm.js scrollback from the authoritative snapshot after mount/navigation/reconnect or a server resynchronization point, preselects the newest reusable command/arguments/working-directory configuration whenever New terminal opens, focuses every newly started Terminal Session for direct input, and makes any running terminal immediately writable when clicked regardless of owner. The xterm FitAddon fills each tile locally without propagating viewport, grid, maximization, or browser-window changes to the PTY; Terminal dimensions change only through an explicit Terminal start or resize operation. Finished retained sessions remain read-only history.
