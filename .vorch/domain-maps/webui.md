...
- `webui/src/lib/terminalsView.js` owns manual Terminal Session start and errors, live-plus-retained-session and server-persisted launch-history reconciliation, selected-stream sequence/gap recovery, authoritative TUI mode/exit snapshot application, exact xterm input batching, operator mutations, and cleanup; `TerminalsView.svelte` rebuilds bounded xterm.js scrollback from the authoritative snapshot after mount/navigation/reconnect or a server resynchronization point, preselects the newest reusable command/arguments/working-directory configuration whenever New terminal opens, starts manual sessions in focused direct-control mode, keeps live Agent-owned sessions in observe mode until the explicit Take control button is used (pointerdown or Enter only selects and focuses), decouples the user viewport from the Agent PTY — the xterm FitAddon fills the tile locally with no hard limit for the viewer (manual terminals drive the PTY via debounced resize up to 800x400, Agent terminals keep their creation/agent-requested PTY dimensions with no viewport-driven resize), and presents finished retained sessions as read-only history.
