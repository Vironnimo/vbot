...
- SQLite opens with foreign keys, `synchronous=FULL`, a bounded busy timeout (1s SQLite, 20s/60s/0.5s application jittered budgets for BUSY/LOCKED), application id, schema version, a startup structural integrity check (`quick_check`), one serialized writer with `BEGIN IMMEDIATE` and time-based retry, and a bounded LIFO read pool (8 permits, 60s backoff). `required_journal_mode()` selects WAL only on SQLite builds without the WAL-reset corruption bug; affected builds use rollback-journal mode and an existing WAL database is never live-downgraded. Reader connections are query-only and open only after the writer has reconciled schema; close drains the pool and releases permits. Operational failures (busy/locked/permission/disk-full/transient I/O) surface as `SessionStoreUnavailableError` and never trigger quarantine; only classified corruption (malformed/not-a-db/integrity/FK/schema mismatch) may enter recovery. Quarantine moves `sessions.db` plus `-wal/-shm/-journal` as one bundle under `<data-dir>/session-quarantine/<timestamp>-quarantine/` with all-or-rollback semantics; version-protocol failures still fail closed without quarantine.
- Verified snapshots for startup, periodic, shutdown, update, manual, and recovery operations live in `<data-dir>/session-snapshots/<snapshot-id>/` via `core/sessions/snapshots.py`; each holds a standalone `sessions.db` plus `manifest.json` with SHA-256, file size, sqlite version/source, schema/application identity, counts, integrity results, revision watermarks, and completeness. Captures stage as `.<id>.<pid>.partial/` and publish only after hash/integrity/FK verification and fsync. One OS-backed operation lock serializes snapshot and recovery work; it is held open for the operation and never broken solely because its metadata is old. Retention keeps at most `SNAPSHOT_KEEP_COUNT` and `SNAPSHOT_KEEP_BYTES` and never deletes the last verified snapshot; incomplete captures never prune. Recovery quarantines the failed database bundle before replacement and records a durable possible-loss interval until explicit incident acknowledgement.
- Integrated FTS: `message_search` mirrors `messages` via triggers and `messages_fts` is an external-content FTS5 table (`content='message_search'`, `content_rowid='message_key'`, `tokenize='trigram'` with fallback to plain FTS5). Triggers keep the index live; on FTS write corruption the writer detaches the index (drops FTS triggers/table, sets a stale breadcrumb) and retries the canonical write so derived-index failure never blocks canonical persistence. Backfill is chunked and resumable with generation, target/completed high-water marks, transactional progress, and a final coverage check; interruption reports rebuilding/degraded state and never healthy incompleteness. Canonical search falls back to substring scan when FTS is stale or unavailable; short queries (<3 chars) also use scan.
- `SCHEMA_SQL` is the declarative source of truth at the current `SCHEMA_VERSION`. Writer open accepts only the supported current schema after additive reconciliation; older destructive versions and newer versions raise `SessionStoreSchemaMismatchError` and never enter corruption recovery. Marker, database, and snapshot schema/application identity must agree before mutation or publication. Primary-key, existing-column contract, table-constraint, generated-column, table-option, and unique-index definition changes are destructive: raise `SCHEMA_CONVERSION_FLOOR` and use the offline converter rather than teaching startup to rewrite data. Readers require exactly `SCHEMA_VERSION` after reconciliation.
- Startup authorizes `sessions.db` only via the current-format marker `<data-dir>/session-store.json` (`bootstrap`/`ready` with `database_id`/`schema_version`, strict closed schema, atomic write+fsync, `validate_session_store_paths` enforces `sessions.db` parent == `data_dir`). A fresh directory is bootstrapped only when `initialize_data_directory` creates the root; an existing directory without a marker is a hard `SessionStorageFormatError` without any JSONL inspection. When the store is `ready` but `sessions.db` is missing or fails classified corruption, startup attempts auto-restore of the newest verified snapshot (quarantining the failed DB as a bundle) before surfacing an error; busy/locked/permission/disk-full never quarantines.
- `scripts/converters/session_sqlite.py` is the one standalone offline conversion entry point (commands: `inventory`, `dry-run`, `convert`, `verify`, `install`, `resume`, `export-jsonl`) and `jsonl_sessions.py` is its private legacy capture/parser. It inventories all six physical roots and sidecars, reads each artifact once into immutable bytes, records torn tails and rejected paths, maps every generation deterministically, imports transactionally with semantic verification, and keeps an external manifest. `install` is the only source-mutating command and requires a stopped exact server, an external backup, and a verified staged database before replacing the target and publishing the `ready` marker last. Current-format snapshot and recovery maintenance belongs to `ChatSessionManager` and the `session-store` CLI/RPC surface; no retired converter or duplicate database owner remains.
