...
- `ChatSession(path)` is one session file handle: `append`/`append_many` (one fsynced batch), `load()` (validated raw records, ordered), `load_active()` (folded active lineage), and `load_since(cursor)` returning a raw batch plus opaque `{byte_offset, message_count, last_message_id}` cursor (a cursor whose anchor line no longer matches returns `None`, forcing disposable consumers to rebuild). `active_session_messages()` folds `history_edit` records by removing the targeted User message and every active successor while retaining every raw record on disk; active edit targets must be plain-text WebUI User messages after the latest Agent Takeover. Async counterparts run off the Event Loop and hold cancellation until started writes settle.
...
- Recall/statistics indexes are disposable rebuildable projections; JSONL stays canonical. Recall uses the active lineage, while Statistics consumes raw records through the Sessions-owned cursor so edits do not erase historical Usage, Runs, Tool activity, or checkpoints.
