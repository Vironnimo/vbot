...
The current Assistant turn is appended through `_assistant_continuation_dict`, which strips `usage` but preserves permitted reasoning. Mid-Run rebuilds restore that reasoning across a new Compaction checkpoint. When run-local Model fallback begins, Chat rebuilds request state from canonical Session data with the fallback target's replay policy, input modalities, wire media support, and route-gated Tool definitions, then strips prior Assistant reasoning/reasoning metadata before the first fallback request regardless of the primary adapter policy.
