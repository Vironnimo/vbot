...
Each provider request is assembled from one loaded canonical Session snapshot. Chat first derives Session-scoped Tool grants from that snapshot, builds the exact provider Tool-definition set, passes those Tool names into System Prompt assembly, and only then shapes message history. The System Prompt is assembled whole by `SystemPromptManager`; it is never appended to Session history, and an empty/whitespace prompt is omitted from the provider request.
...
Adapters declare `ReasoningReplayPolicy` (`none`, `current_run`, or `full_history`) per Model. Chat owns the history shaping and never interprets `reasoning_meta`. `none` strips readable and opaque reasoning everywhere; `current_run` preserves it only across Tool continuation inside the active Run; `full_history` may replay persisted reasoning only when the historical Assistant message used the same Model as the current request. A different Provider/Model must never receive stale opaque metadata.

The current Assistant turn is appended through `_assistant_continuation_dict`, which strips `usage` but preserves permitted reasoning. Mid-Run rebuilds restore that reasoning across a new Compaction checkpoint. When run-local Model fallback begins, the reused request list strips prior Assistant reasoning/reasoning metadata before the first fallback request regardless of the primary adapter policy.
