...
`core:working_project` renders the file-backed `working_project.md` template from the admitted Project's id, display name, absolute Project Workspace, and readable auto-load files. A Project Config Agent receives that live frame on every build, even when no auto-load file is readable. For a Rooted Identity Agent only, `_pinned_working_project_context` renders the frame before the first provider request, stores the exact text under `pinned_working_project_context` in Session metadata, and supplies it verbatim on every later build. A Project-file change or an explicit `project` Tool call cannot regenerate or replace the pinned frame. This pins only the automatic Working Project block; the explicit `project` Tool retains its ordinary persisted Tool result, and every unrelated System Prompt input keeps its existing lifecycle. Prompt preview renders a prospective Rooted Identity frame without persisting it and a live Project Config frame from the addressed Project.
