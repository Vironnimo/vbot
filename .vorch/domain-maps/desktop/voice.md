...
- `retry()` rereads the stored config and forces a transition; in the unavailable mode it also re-probes the stack. It refreshes the PortAudio device list after the old session shut down and before the new stream opens (PortAudio caches devices and the default input until reinitialized).
...
- Acoustic settings are global: `microphone` (stable identity or `null` for automatic), `echo_cancellation` (default `true`), `active_model_ids` (1..8 unique ids, `MAX_ACTIVE_PHRASES`; default Okay Nabu + Hey Nabu; only newly added ids must be in the catalog, so a phrase whose model is gone may stay active and can always be removed), `model_sensitivities` (0.05..0.95, default 0.5, threshold = 1 - sensitivity; inactive models keep theirs). Sensitivity must stay strictly below 1.0: a zero threshold with `score >= threshold` would turn every zero score into a detection.
...
- The retired global `model_actions` key is migrated: each `live_voice` entry becomes `{type: "live_voice", mode: "start"}` (the old behavior) in every stored profile without an entry for that model, on read and persisted by the next write (`command` entries are the default and dropped). Without any profile the key stays until the first profile is created, then carries over before the user's change applies. Writers keep unknown keys.
...
- `listMicrophones()` -> `[{index, name, host_api, default_sample_rate, supported, capture_sample_rate}]` (shared-mode inputs only; empty without sounddevice). It refreshes the device list first unless a Voice stream is open; while listening the list stays as is until the next Retry.
...
- `MultiWakewordEngine` (`engine.py`) runs every active head over one shared `pyopen_wakeword` feature stream. `speech_present=False` zeroes a chunk's scores for firing, confirmation, and the score listener (calibration sees the same gated scores). A score >= threshold + 0.15 confirms at once; a marginal crossing must cross again within the last 5 chunks. Each phrase re-arms when its raw (ungated) score falls below its threshold, so a gated pause cannot re-arm a phrase that still scores high and fire it twice; overlapping phrases (catalog `overlaps`) share one arm group. Among confirmations the highest score-to-threshold ratio wins, so independently tuned heads stay comparable.
...
- The stage may hold microphone audio back briefly while it waits for the matching reference and release it later; the hold is bounded. The alignment algorithm is internal to `echo.py`: it places both streams from arrival times and sample counts only (never the audio), so gated or silent device output does not affect it, and it keeps pairing for devices that lose or insert samples or run off-rate (common with wireless and USB microphones). Its thresholds derive from each stream's measured jitter, not from device-specific constants; clean devices place exactly as a plain contiguous clock would.
- Diagnostics: `WebRtcEchoStage.stats()` returns read-only per-stream timing counters; the first lost samples or a rate error beyond 1000 ppm log once at INFO (stream, ppm, step counts and sizes), later changes at most once a minute at DEBUG. No audio content is logged.
- Limit: device-side processing (noise gates or suppression in a headset) makes the echo path non-linear. A real test with a wireless gaming headset lying on the desk reached only 3-4 dB even with perfect alignment, and distant speech never scored as a wake phrase there; a close-talk position or a desk microphone without aggressive on-device suppression is needed for loudspeaker use.
