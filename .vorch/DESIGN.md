...
**Section content that owns primary actions anchors them in one shared list-head row (`.s-list-head` in `styles/settings.css`), never as floating right-aligned buttons between the section header and the list.** The row carries a muted count summary on the left (`s-list-head-info`, e.g. "3 connected") and the actions pinned right (`s-list-head-actions`), with a hairline bottom border separating it from the list. Providers, Channels, and Extensions use it; new settings sections with list content follow the same pattern. At narrow widths, the summary and action group wrap onto separate rows and button labels remain intact.
