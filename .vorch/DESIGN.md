...
Skills uses a collection-first manager: the shared Secondary bar lists collections and Agent owners with counts, while the main area leads with collection context, search, and a status filter. Results are compact text rows with a readable description and source, with at most 50 per page; normal availability does not add a badge to every row. Selecting a Skill opens one adjacent reading panel with instructions, access context, editing/sharing, and secondary disclosures for diagnostics and availability/removal. Shared originals stay in their owner's collection. At tablet/mobile widths the collection index becomes a Dropdown and the selected detail replaces the list, with an explicit Back to list action restoring keyboard focus. Skill creation/editing/sharing uses the shared Modal with a padded, height-bounded scrolling body and explicit saves. Creation uses separate name, description, and instruction fields; creation and original-text editing use a wider 760px dialog capped to the viewport; scan locations are a separate destination. Disabled rows retain readable text contrast.
