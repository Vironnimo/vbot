...
The Chat Activity surface is a deliberately quiet, free-floating control near the Chat's right edge, not another permanent navigation sidebar. Collapsed, it is a 26px rail inset by 8px, vertically centered, and approximately 30% of the Chat height; it carries only a centered left chevron, plus one small amber dot while background work is active. Opening grows the same surface leftward to a compact 276px panel and approximately 48% of Chat height, reverses the chevron, and reveals the single title “Background tasks” above a contained scrolling list. The list separates running work under a small Mono-caps “Active” heading from completed, failed, and cancelled work under “Finished”; either empty group disappears rather than leaving a placeholder. Each automatic-delivery Sub-Agent task occupies exactly one borderless 34px row with only the Agent address and a semantic status symbol (animated amber working ring, green completion check, muted cancellation cross, or red failure warning); the accessible row label names the state. The entire row navigates to its Child Session, with no separate View Session or cancellation action. Fine dividers separate rows and groups, while hover/focus provide the only local surface emphasis. At ≤640px the rail grows to a 32px touch target and the expanded panel stays within the Chat width. Escape or the chevron closes it, and reduced-motion users receive no growth, entrance, arrow, or working-ring animation.
