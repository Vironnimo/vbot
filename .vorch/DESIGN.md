...
Shared Agent defaults are a separate destination below the Agent roster, not a synthetic Agent. Their header states the scope: inherited values change, explicit Agent/Project choices remain. The selected Agent editor stays mounted, and returning retains its selection. The normal Agent landing always opens the individual Agent. Model and Thinking form the primary control cluster; shared defaults use a bounded field grid and a secondary Compaction disclosure.
