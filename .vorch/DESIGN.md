...
Terminals uses the same control-desk master-detail shell but treats the selected PTY as a monitoring surface rather than a form. The Secondary list contains only active Terminal Sessions and identifies each by command, readable Agent/Project owner, abbreviated Session, and start time. The detail header keeps state and stream health visible alongside PID, dimensions, and working directory; observe mode is the safe default, direct keyboard/paste input requires an explicit Take control toggle, and a separate compact composer supports deliberate context or answers followed by Enter without enabling direct control. The xterm.js stage is the dominant dark technical surface and preserves ANSI colors, cursor, and TUI layout; it owns its scrolling and resizes the underlying PTY to its fitted rows/columns. Stop is a danger action behind `ConfirmDialog`, and empty, reconnect, and error states use the shared `EmptyState`/`Banner`/`StatusChip` language. At the mobile breakpoint the active-terminal list stacks above the detail while the TUI keeps horizontal integrity inside its own viewport.
