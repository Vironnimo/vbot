...
Settings is a collection of topic pages with one shared navigation/search surface. Navigation groups are General, Models, Conversation, Capabilities, Integrations, and System. The selected topic begins at the top of the content pane beneath its group eyebrow; its title uses heading-lg and its body stays within the narrow content measure. Global search replaces the visible editor with a list of topic results, each with a title and concise description; opening a result returns to that topic's canonical editor. The search accepts spacing and punctuation variants, including “timezone” for “Time zone”. Keep the same editor instances alive across visibility changes so drafts are not lost.

Settings has no scrollspy, continuous sequence of unrelated cards, or trailing reading-line spacer. An ordinary return remembers the selected topic and its scroll position; explicit navigation starts at the top. Through tablet widths the second navigation column becomes a search field and compact topic picker above the content, leaving room for the actual settings. General preferences owns timezone/setup; System retains runtime information, connected clients, and keep-awake. Related tasks link to their canonical configuration, such as Voice to Specialized Models, rather than duplicating controls.

Agents and Projects use a common management header, shared underline TabList, bounded topic body, and save footer outside the scrollport. Agents has Overview (name, Project, Model), Behavior (Compaction, System Prompt, Memory), Tools & Skills, and Details (ID, Workspace, metadata, deletion). Projects has Overview (identity and defaults), Team, Context (auto-load files), and Tools & Skills. This local navigation always remains visible while a topic scrolls. Use the narrow content measure for these editors. At tablet/mobile widths their master lists become horizontally scrolling selectors so two permanent navigation columns cannot squeeze the form.
