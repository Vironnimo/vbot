...
Settings is the one-document pattern, built on the shared left-to-right depth sequence: the Main menu sits on `--surface`, the Settings index on `--secondary-surface`, the document scroll area on `--bg`, and each section card lifts back to `--surface` (`--content-max-narrow` measure, `lg` radius, `border` outline) headed by a 15px/600 title and a `text-med` subtitle above a divider. The Secondary bar starts with the common muted mono-xs pane title, then a search input and grouped table of contents; index entries are 12.5px/500 `text-med` rows with a 2px left accent rail as the active marker (accent text, `--accent-dim` tint, radius only on the free right edge), hover brightens to `text-hi`, and non-matching entries dim to 35% during a search. Connect / Models / Conversation / Behavior / System are the shared server-backed groups; Conversation bundles the conversation-lifecycle controls — Compaction, Session titles, and Recall — so the context/session/memory concerns sit together, while Models keeps only the true model-selection panes (Agent defaults and Specialized Models). Voice remains under Behavior because its transcription-audio controls are server-wide, while the Desktop accessor reveals a Connection section inside System (for local Connection controls) and Wakeword rows inside Voice. Section rows use a tightened ~12px vertical rhythm, and the header divider keeps a matching clean gap so no boxed control ever sits flush on it. A row whose control is a list or multi-field surface (the Desktop Wakeword phrase cards, the guided-calibration panel) uses the stacked full-width variant — label and description above, control spanning the content column below — rather than a narrow right-floating column; the phrase cards form a responsive grid (one or two columns) with `align-items: start` so inactive short cards do not stretch. On mobile the full index is replaced by the same search plus a compact current-section dropdown; the document remains the sole scrolling content region directly below it, and selecting an option uses the same scroll-to-section behavior as the desktop index.
