...
`webui/public/brand/vbot-icon.png` is the canonical square icon treatment: the same unmodified mark centered on a dark rounded tile. Browser and OS-specific derivatives are `webui/public/favicon.ico`, `webui/public/apple-touch-icon.png`, `desktop/icon.ico` for Windows, and `desktop/icon.png` for other Desktop platforms. Use the transparent mark when the surrounding surface already provides contrast and the square treatment when the asset must stand alone, including browser tabs and native app chrome.
...
The app shell is a 210px sidebar plus a fluid main content area. The main area holds views that each fill the full remaining width and height. Desktop users can collapse the Main menu to its 64px icon rail through a compact, inset toggle that interrupts the sidebar header divider; this display preference persists locally. The inset toggle keeps its 30px by 20px footprint independently of the shared Button minimum height, with a muted `text-lo` chevron that turns accent on hover. The compact rail centers the canonical brand mark, navigation symbols, health indicator, and Desktop Voice indicator, hiding all supporting labels while Quick tooltips and accessible names preserve their meaning. Expanding restores the full label-led menu.

Live voice uses one compact Start/Stop control at the bottom of the Main menu, directly above the microphone and connection indicators. It appears only after enabling Live voice in Settings. The collapsed rail shows the same control as an accessible play/stop icon with a shared Quick tooltip; Live voice does not occupy a banner or panel above the main content. The footer remains visible at short window heights while the navigation list above it scrolls.

Sidebar navigation is grouped by usage cadence into Work / Configure / Insights, each group introduced by a mono-xs uppercase section label (`--text-lo`, no divider lines). The compact desktop icon rail retains the same top navigation inset as the expanded menu, so the first icon clears the inset toggle; labels remain hidden but each later group begins after a small gap marked by a centered, short, low-contrast `border` hairline. This restores group scanability without turning the rail into a second footer. Interactive inactive entries use `--text-med`, hover brightens them, and the active entry uses the accent treatment. On mobile (<=640px) the compact desktop preference does not apply: the sidebar becomes a single scrolling top-bar row with readable item labels, and a direct link or tab change scrolls the active entry into view.

On desktop, Secondary bars share a 216px width: the Agents, Projects, and Cron selection lists and the Settings index all use the shared `.secondary-pane` contract (`--secondary-sidebar-width`, `--secondary-surface`, a thin `border` divider, a muted mono-xs uppercase pane title, and contained scrolling). Their content differs by purpose — Settings alone has search and five category destinations — with shared width, depth, and title hierarchy; each view adapts its navigation to the available space. Agents, Projects, and Schedules additionally share `.secondary-list` / `.secondary-list__item`: list content, including empty states, sits inside a 12px inset on every edge, and the active surface mirrors the Settings index with a 2px left accent rail, `--accent-dim` fill, and `md` rounding only on the free right edge; feature styles own only the row's inner text and metadata layout. Agents and Projects replace this vertical list with a horizontal selector through tablet widths; Schedules retains its existing mobile stacking behavior.
...
Terminals is a full-width control desk that treats the selected PTY as a native monitoring, history, and direct-control surface rather than a form. Its 50px top bar replaces the otherwise persistent Secondary bar and view identity copy with horizontally scrollable group navigation, so group selection never reserves terminal width; the selected group has the same accent underline language as Chat Agent navigation, terminal counts stay compact, and editable-group actions remain reachable on hover or keyboard focus. A quiet plus immediately after the final group creates a group; the active non-finished group carries a persistent terminal-plus action beside its name. Empty groups expose a central New terminal action. Populated groups append a narrow 48px plus tile immediately after the last terminal in the same grid row: it uses the next free cell, or shares the last occupied cell by reducing only that terminal's width. It never adds a row, reduces terminal height, or reserves a permanent column, and follows the last terminal as the layout changes. Maximizing hides the append tile; starting from the group action restores the grid and focuses the new terminal. A primary New terminal action opens a compact 520px shared `Modal`/`FormField` form: one command line combines the executable and literal arguments, followed by working directory and a paired Name / Group row. The row stacks on mobile. Sentence-case Sans labels use readable primary text, and explanations live in shared InfoHint question-mark popovers. Blanks select the server user's default shell and home directory. When persistent launch history exists, a full-width Recent setup dropdown appears before those fields, shows command plus exact arguments with working directory as secondary metadata, defaults to the newest setup whenever the modal opens, and fills the command line and directory together while keeping them editable. Quoted values preserve spaces; malformed quotations produce an inline field error before launch. The canvas contains live Terminal Sessions plus temporarily retained finished sessions, each identified in its tile bar by a custom name, program-announced title, or launch command, plus readable Agent/Project or local-operator provenance; attachment is backend Tool/event metadata and never a UI control mode. Every running Terminal Session uses the same interaction: clicking its tile selects it, focuses native xterm keyboard/paste input, and permits immediate typing regardless of who started it; a newly created terminal receives that focus automatically. There is no Observe, Take control, Release control, attach, or detach UI state. Finished sessions expose only a read-only retained snapshot, and there is no secondary text composer. The bounded xterm.js stage is the dominant dark technical surface and reconstructs the server-retained scrollback plus current screen after mount, navigation, selection, or reconnect while preserving current-screen ANSI colors, cursor, TUI layout, mouse-wheel/trackpad scrollback, an always-visible draggable scrollbar, and a Jump to latest action while reading history. It owns its scrolling within the tile. The design requirement is to give the Agent a compact, useful terminal view independently of the operator's available UI space. Enlarging or maximizing the UI must not by itself increase the full screen content sent to the Agent. Different Agent/UI dimensions are intentional when content and input mapping remain correct; the operator view must avoid distortion and unnecessary cropping. The current implementation does not yet fully enforce this separation: it uses xterm's FitAddon to derive a column/row grid from the available tile space, starting with a 12px font and increasing it when necessary to stay within the server's 240-column/80-row maximum. The fitted grid is clamped to at least 40 columns and 10 rows. Tile layout, maximization, and browser-window changes can send `terminal.resize` and therefore change the live PTY dimensions; there is no separate Agent grid or CSS surface stretching. A settled mismatch between fitted and server dimensions appears as a tile hint. Close removes the tile immediately while stopping and forgetting the Terminal Session in the background; a failed stop restores it and surfaces the error. Empty, reconnect, start-error, and stream-error states use the shared feedback primitives. At the mobile breakpoint the compact creation actions remain within the horizontally scrolling group navigation while the TUI keeps horizontal integrity inside its own viewport.

Terminal tile bars include the same microphone icon as Chat before maximize and close. It is available for connected, running terminals and uses the active accent treatment while recording. A compact status label reports microphone startup, recording, and transcription; a separate discard action stays reachable throughout. Dictation inserts editable text at the terminal cursor without submitting it; finished terminals retain only their read-only actions.

Standalone views use the shared `.view-frame` / `.view-header` / `.view-toolbar` chrome from `webui/src/styles/app.css`. The header is a plain identity block — mono-xs uppercase eyebrow, 20px title, `text-med` subtitle, and optional compact status — while local navigation, filters, scope selection, and contextual actions belong in the bordered toolbar immediately below it. Toolbars choose the smallest fitting layout modifier: `--split` for one context/action row, `--stack` for dense filters plus metadata, and `--tabs` for a `TabList` plus its contextual actions. At ≤640px the view inset drops to 16px, tab toolbars become a horizontally scrolling tab row followed by a separate action row; overflow stays inside the local control rather than widening the page. Statistics, Logs, Debug, and System Prompt are the reference consumers.

Place view actions beside the content or control they affect, starting at the left of the local toolbar; do not place action clusters at the top right of a page header. Familiar repeated actions such as Add, Share, and Delete use compact icons with shared Quick tooltips and accessible names. Keep visible labels where an icon cannot communicate the action clearly, and for explicit form submissions. Do not add Refresh buttons by default: use existing load, mutation, and server-event reconciliation. Add a manual refresh only for an established user need that automatic updates do not cover; recovery Retry remains available with errors. Existing screens are not precedents that override these rules.

Settings has five direct category entries without group headers: General, Sessions & Memory, Tools & Media, Connections, and System. It opens General, where Appearance and Region/setup are directly editable in adjacent cards on desktop. Other categories show related feature cards together in a two-column grid, each with a clear heading, short purpose and a factual status or capability summary. Expanded forms span the reading measure; compact general cards retain their columns. Search and explicit deep links open the exact editor, with all editor instances retained to preserve drafts. The mobile/tablet picker contains categories only. Agent Model/Thinking/fallback and Compaction defaults belong under Agents -> Shared defaults, never as Settings forms.

Agents and Projects use a common management header, shared underline TabList, bounded topic body, and save footer outside the scrollport. Agents has Overview (Model and Thinking first, followed by identity and Project), Behavior (Compaction, System Prompt, Memory), Tools & Skills (ordered as Tools, Skills, then Sub-Agent settings), and Details (ID, Workspace, metadata, deletion). Projects has Overview (identity and defaults), Team, Context (auto-load files), and Tools & Skills. This local navigation always remains visible while a topic scrolls. Use the narrow content measure for these editors. At tablet/mobile widths their master lists become horizontally scrolling selectors so two permanent navigation columns cannot squeeze the form.

Sub-Agent settings show Identity Agents first, then a Project Agents disclosure that starts closed. Its heading always includes the selected/total count in parentheses. Each group has its own search and all-on/all-off actions; group actions leave the other group unchanged.

Management form labels use readable Sans in sentence case and text-hi; secondary help uses text-med. Reserve Mono for technical values and metadata. Number fields stay compact, effort choices have bounded width, Model pickers receive a larger measure, and paths may span the editor. Existing surface layers, borders, accent use, and Chat appearance remain unchanged. Topic navigation preserves drafts and passes through the shared autosave boundary; the footer describes autosave and retains the explicit save action.

Dense detail inside settings rows (whole provider cards with their connections and accounts, extension settings forms) collapses behind a disclosure: a 22px bordered chevron square (`▸`, `text-med`, rotates 90° when open, accent on hover) at the row's trailing edge, opening an indented sub-block carried by a 2px `border-2` left rail — the same visual language as expanded tool calls in chat. The collapsed sub stays in the DOM (`hidden` attribute) so settings search still matches its text. An Extension row exposes this disclosure only when the Extension declares a Settings Schema; the shared schema renderer separates the disclosure from the row summary by `md`, separates complete field groups by `lg`, and keeps tightly related secret status, input, and actions at `sm`.

Specialized Models show each selected target's options directly below its Model picker, without a second disclosure inside the feature card. Fields use the shared responsive two-column grid and stack on mobile. Local speech setup and the TTS preview follow the options, so users configure the voice before trying it. The preview text has a visible, associated FormField label.
...
Chat supports up to two retained areas without adding a header, content tabs, duplicate Session title, or focus stripe. One neutral split icon joins the existing floating Sessions / New Session control. Its on-demand action menu offers Split view, Back to chat, Close area, and Show preview only for an already opened file; the same menu sits inside Preview's compact file toolbar. The menu uses the shared floating-panel positioner and portal, supports keyboard navigation and Escape, and restores focus to the surviving area's control after closing. Closing or switching content preserves the area's Chat draft and scroll position. Preserve the simple Chat surface when adding area capabilities; do not introduce another persistent toolbar.

The divider is a 9px neutral surface with a centered grip, accent on hover/focus/drag. Pointer capture keeps dragging reliable across preview frames; Arrow keys adjust, Shift accelerates, Home/End reach the bounds, and Enter or double-click restores equal sizes. Desktop areas keep approximately 340px minimum width; below 760px available workspace width they stack vertically and the divider changes orientation. The ratio is an accessor-local preference. Preview opens directly from an Agent-shared HTML file link in Chat, automatically revealing the opposite area. There is no path entry, Open form, or menu entry into an empty Preview. Delivered HTML links retain the filename as the primary Preview action, with one quiet, neutral external-browser arrow on the same text baseline. File actions open through right-click or the keyboard context-menu shortcut; no visible menu button is added. The context menu provides Preview, Open in browser, and Download; these controls add no Chat header or input form. Its single compact file toolbar contains the current relative page label, Live toggle, Reload, entry-page navigation, and area actions. Website colors belong to its isolated document; all surrounding chrome uses existing design tokens.
...
The floating Sessions drawer keeps that same master-list row language instead of per-row cards: borderless flat entries with no fill at rest, hover and keyboard focus tinting them `surface-2`, and the current Session marked only by the 2px accent left rail with an `accent-dim` fill and `md` rounding on the free right edge. Rows stay compact — about 28–30px tall with 4px between entries. Each row's trailing actions control stays reserved in the layout and reveals itself only on hover, keyboard focus within the row, or while its menu is open; pointer-coarse devices keep it visible at rest.

The Chat Activity surface is a deliberately quiet, free-floating Session-info control near the Chat's right edge, not another permanent navigation sidebar. Collapsed, it is a 26px rail inset by 8px, vertically centered, and approximately 30% of the Chat height; it carries only a centered left chevron, plus one small amber dot while background work is active. Opening grows the same surface leftward to a compact 276px panel and approximately 48% of Chat height, reverses the chevron, and reveals the single title “Session”. A displayed Sub-Agent Session, Reflection Session, or other Fork adds a compact “Parent Session” group immediately below the header once the source Session is resolvable: its real Session name appears in Mono beside a return arrow, and the complete row is a keyboard-focusable navigation control. Invalid or unavailable provenance leaves the group absent. Session stats follow, then a contained scrolling background-work list. The list separates running work under a small Mono-caps “Active” heading from completed, failed, and cancelled work under “Finished”; either empty group disappears rather than leaving a placeholder. Every task occupies one borderless compact row with a semantic status symbol (animated amber working ring, green completion check, muted cancellation cross, or red failure warning), and its accessible label names the state. Active rows additionally end in a 24px red danger icon with an × glyph, accessible label, Quick tooltip, and disabled in-flight state; finished rows never show it. An automatic-delivery Sub-Agent row keeps the Agent address as the Child Session navigation control while its separate cancel icon stops the exact Child Run or queued item without navigating. A handed-off Bash row keeps a muted `$` marker plus the compact one-line command and no Session/navigation affordance; its separate cancel icon terminates the owned Process Session directly. Reflection reviews appear under a small Mono-caps "Reflections" caption inside each group: a row keeps the Session drawer's return-loop-with-spark mark plus the localized review scope (memory / skill / combined) as its navigation control into the forked review Session, shows coarse elapsed time after the scope while running, and never carries a cancel affordance because reviews are server-owned background Runs. Fine dividers separate rows and groups. At ≤640px the rail grows to a 32px touch target and the expanded panel stays within the Chat width. Escape or the chevron closes it, and reduced-motion users receive no growth, entrance, arrow, or working-ring animation.
...
3. **Tertiary (`btn-tertiary`)** — Quiet supporting action: transparent border and fill, readable `text-med` Sans text, 6px radius, 5px 10px padding, and a 30px minimum height. Hover uses `surface-3` and `text-hi`; it does not compete with the primary action. With `icon`, becomes a centered 30px borderless square with no padding (composer mic/attach); the engaged state adds the `btn-icon--active` accent tint. Secondary icon buttons use a 32px square. Icon actions always retain an accessible name and Quick tooltip.
...
The Desktop accessor replaces pywebview's production-disabled browser menu with one compact AppShell-owned menu; ordinary browsers keep their native menu. The panel is fixed at the pointer or keyboard-invocation target, measured and clamped inside an 8px viewport margin, and uses `surface-2`, `border-2`, `--floating-elevation`, `--z-floating`, a 6px outer radius, and 4px panel padding. Each 32px row is a shared tertiary `Button` visually flattened into a full-width menu item: 16px line icon, 10px gap, Sans body-sm label, `text-med` at rest, and `surface-3` plus `text-hi` on hover/focus. A single `border` separator divides link actions from editing actions; no accent color or entrance animation is used. Keyboard behavior mirrors a native menu (first-item focus, Arrow Up/Down, Home/End, Escape), and action/dismissal restores the original focus where appropriate.

### Inputs

**Every ordinary labeled control is wrapped by the shared `FormField` component (`webui/src/components/ui/FormField.svelte`).** The caller keeps the value and validation state; `FormField` owns the visible Mono-caps label, optional required marker, help text, error text, spacing, and the generated ids that connect help/error copy to the control through its snippet contract. Optional `actions` render after feedback in a left-aligned flex row (`form-field__actions`) — a bare button would stretch to the full field width as a flex-column child — and `full` spans a two-column form grid. Agents, Projects, provider connection, Channels, Extensions, and Specialized Models use this shell around `TextField`, `TextArea`, `Dropdown`, `SearchableDropdown`, and `Toggle`; toolbar filters, Chat Composer, and other specialized controls remain outside it. The guard rejects raw canonical classes and the retired modal/settings/agent field-shell classes.
...
**Tool Access Policy editor** — `components/tools/ToolAccessEditor.svelte` is the one Agent-policy surface. It uses one compact All / Choose / None segmented control, a small search field, and a responsive grid of real registry-family cards; unrelated Tools and families with fewer than two members in the current catalog/ceiling share the presentation-only Individual Tools card and never receive fake singleton families. Search filters members without changing an established family into Individual Tools. Built-in families use their i18n labels and Extension families use the server-projected `family_label`. The overview renders only Tool names as wrapping binary chips — never Tool descriptions, per-Tool rows, textual policy states, or separate Off/Block choices. Orange means directly enabled, grey means off, and neutral dashed chips identify automatic Tools; activation, readiness, identity, and route explanations appear only on hover/focus, while unavailable and stored-but-unregistered Tools remain visible and editable. One switch in each real family header changes its current members without showing internal policy vocabulary. The frontend maps the binary action to the canonical policy: off becomes `denied` in `all`, absence from `allowed` in `selected`, and `denied` for an automatic Tool; the user never chooses those storage mechanics directly. Opt-in Tools also require an explicit `granted` entry: All does not activate them, their ordinary chip grants/revokes access, and the hover explanation names this requirement. Mode switches preserve existing grants. The `analyze_image` hover/focus card additionally offers the shared small Toggle labeled "Available with vision"; its explicit grant is independent of ordinary Tool/family selection, and the control is disabled while Tool access is off. The control has no horizontal overflow in the narrow Agent pane and autosaves through the owning Agent or Project transition boundary. A nested Project Team override has no Apply button: every policy interaction is the explicit autosaved change, while Reset to repository policy remains a separate deliberate action. The Project Tool Whitelist stays a ceiling editor rather than adopting Agent-policy modes, but `ToggleChipList` groups its Tool chips with the same family labels for comprehension.
...
The Session drawer is the deliberate icon-only Badge context: its narrow rows render Channel origin, Subagent, Fork, Cron, and Reflection as fixed 18px square micro-badges with an explicit 11px SVG, quick tooltip, and accessible label instead of visible badge text. Telegram and Discord use recognizable monochrome platform marks, an unknown Channel uses the generic broadcast mark, Subagent uses connected parent/child nodes so it cannot be confused with the separate branch-shaped Fork marker, Cron uses a clock, and Reflection uses a return loop with a spark. These markers stay in one non-wrapping strip after the Session name; unread completion is the shared blue unread dot, without an `Unread` text label. The trailing action uses a vertical ellipsis inside its 24px interaction target and remains vertically centered in the Session row.
...
**Section content that owns primary actions anchors them beside the list controls in one shared list-head row (`.s-list-head` in `styles/settings.css`).** Use the action-placement and icon rules above: start actions at the left beside their context, with a hairline bottom border separating the row from the list. Existing Providers, Channels, and Extensions currently pin `.s-list-head-actions` right of a count summary; this is existing implementation, not the prescribed layout for new or redesigned surfaces. At narrow widths controls wrap while preserving readable context and touch targets.
...
**Chat Agent activity navigation** — Identity and Project Team Agent bars use a 7px status dot independently from selection. Neutral `text-lo` means idle, amber means a Run is active (a restrained pulse, disabled under reduced motion), and blue means a durable unread terminal result. State priority is running over unread over idle. The selected Agent is identified only by accent text plus the 2px accent underline; selection never recolors the status dot. Every Agent button exposes the same state in its accessible label and a two-line Quick tooltip: the localized Agent/status line followed by the effective Model in canonical `provider/model` form without a Connection or Account suffix. The Session drawer repeats blue as a compact dot marker on the concrete unread Session rather than inventing a status chip. Its dense resting rows contain only the ellipsized title, unread marker, non-wrapping metadata Badge strip, and vertically centered action menu; the complete title plus last-active time, optional source Channel, and optional Subagent Parent belong in one shared multi-line Quick tooltip on the row, reachable through hover, keyboard focus, and touch.
...
The Chat context ring uses the Structured hover card for its usage breakdown and a secondary `Force compaction` button. Pointer travel into the card keeps it open; keyboard focus and touch use the shared behavior. Pending and running Compaction replace the button label and disable repeat submission; the action is disabled outside an eligible active Run.
...
**Quick tooltip** (`use:tooltip` action from `lib/tooltip.js`, or the `tooltip` prop on `Button`/`CopyButton`) — the short hover label: what a button does, the untruncated value, a data point. One shared floating element per app, portaled to `<body>`: `surface-3` fill, `border-2` border, 3px radius, `text-hi` at 11.5px `font-ui`, 5px 10px padding, `--dropdown-elevation` shadow. Appears after 150ms above the anchor (falls below near the top edge) with a 100ms fade, keeps line breaks for structured hover text (token badge), and caps at `min(1000px, calc(100vw - 24px))` width and `min(1000px, 60vh)` height so oversized content clips instead of covering the screen. Purely presentational — icon-only buttons still carry `aria-label`.

**Structured hover card** (`use:floatingHoverCard` action from `lib/tooltip.js`) — rich or interactive hover content that cannot be flattened into a quick tooltip, such as a copyable complete Tool value, Tool/Skill descriptions with warnings and actions, or Attachment image previews. The card is fixed-positioned and portaled to `<body>`, uses the shared `--z-floating` layer, prefers the space above its anchor, falls below when needed, and clamps to the viewport; never implement one as an absolute child of a card or scroll container. Cards open on hover, keyboard focus, or touch; call sites may require intentional pointer dwell without delaying keyboard or touch access. They keep a short pointer-crossing grace period, toggle closed on a second touch, and close on outside touch/click, Escape, resize, or ancestor scroll. Descriptive cards link through `aria-describedby`; decorative previews remain hidden from the accessibility tree.

**Info hint** (`ui/InfoHint.svelte`) — the "?" dot for explanations: a 15px circular `border-2` outline button with a mono "?" in `text-lo`, turning accent on hover/focus/open. Its popover is an elevated card (`surface-2`, `border-2`, 6px radius, dropdown shadow, `body-sm` in `text-med`, max-width `min(320px, calc(100vw - 24px))`) that opens on hover, pins on click/tap for touch, and closes on Escape or outside click. Use it wherever an explanation would otherwise sit in the UI as permanent grey text; keep permanently visible inline hints only for must-see state (inherited values, "this model does not support reasoning") and warnings.

### Tool call events

Eligible running foreground Bash rows expose a neutral 24px `Move to background` icon Button beside Cancel. The server supplies eligibility. Activating it leaves the disclosure state intact, and the row then follows the existing background-process presentation.

Inline dot-text lines within assistant messages. No box or card: one line orders the colored `●` dot and monospace Tool name, a flexible semantic primary group, fixed facts, status/live-or-final duration, and compact actions. Primary values render as bare text without JSON-style quotes or enclosing parentheses; multiple primary values use only a centered dot separator. Primary values have a 64-character deterministic limit before responsive overflow: paths keep the suffix and at most two parent directories, prose keeps the prefix, URLs/ids preserve both ends; fixed facts, timing, and actions never shrink. Count and line-range facts stay `text-med`; file-mutation line changes are terse monospace `+N` in `green` followed by `-N` in `red`, including explicit zero values. Complete non-copyable values use the Quick tooltip, while copyable full values use the Structured hover card and shared Copy Button after 500 ms of uninterrupted mouse dwell; pressing the Tool disclosure cancels a pending card, while keyboard focus and touch remain immediate. Opening the disclosure never changes this summary line. Its body continues the timeline as one thin `border-2` Rail with separate Args, Stdout, Stderr, and Result sections marked by small neutral, green, or red nodes. Each section places its mono-xs label and hover/focus Copy action in one header; plain objects render below as aligned key/value fields, while scalar or multiline content remains one whitespace-preserving block. Outer object wrappers and decorative String quotes are omitted, nested objects keep indentation without outer braces, arrays use one unwrapped item per line, and explicit empty array/object field values remain visible as `[]`/`{}`. Keys stay `text-med`, ordinary values use `text-hi`, numeric/Boolean values use blue technical syntax color, null/empty values use `text-lo`, and failed values use red; mobile fields stack without widening Chat. Preparing previews use the neutral/running bucket without elapsed time; dispatched running Tools use an amber blinking dot and live elapsed time; done is green, and failures are red. Background Bash and Sub-Agent cancellation use shared 24px danger icon Buttons; the Sub-Agent Session action is a neutral 24px Session/arrow icon Button and remains after completion.

Expanded Tool details can include a wrapping row of compact image previews within the existing Args or Result section. Each preview has a 128px bounded width, an 88px image area with contain sizing, a truncated Mono filename, the standard surface and border tokens, and an accent hover border. Clicking or keyboard activation opens the shared Chat image lightbox; image loading is lazy. Missing or unloadable images retain the preview footprint with a crossed-image outline icon and a localized unavailable label. The lightbox uses the same placeholder in a bounded surface, with its ordinary Close action.
...
Compact Working blocks: when Appearance selects Compact work details, a contiguous span of at least two Thinking/Tool entries inside one Assistant Run becomes one closed disclosure, while a single Thinking or Tool entry keeps its established inline presentation and visible Assistant prose ends the span. The disclosure summary follows the Tool Call line rather than card language: transparent and borderless, no status dot, lowercase Mono `working...` while current, optionally followed only by the latest Tool name even when later Thinking is streaming; Thinking is never named in the summary. Once inactive or terminal, the summary contains only `done working`; it never retains a Tool name, arguments, or timing. A muted chevron opens the unchanged nested Thinking and Tool disclosures on the same thin indented left rail as Tool details. Normal mode retains the complete inline sequence.
...
Shared Agent defaults are a separate navigation destination above the Agent roster, using the same active-row treatment. Their header states the scope: inherited values change, explicit Agent/Project choices remain. The selected Agent editor stays mounted, and returning retains its selection. The normal Agent landing always opens the individual Agent. Model and Thinking form the primary control cluster. Shared defaults use one bounded scrollport containing the title, section jump links, and both open settings cards; the title scrolls away with the content. Compaction leads with an accessible two-option radio group: Classic and With tail. Tail settings follow only for With tail; automatic enablement and trigger limits form the next group. The same Compaction editor is used in every scope, with a single-column layout at narrow widths.


System Prompt uses an inspection-first layout: Agent context and the shared Prompt / Tools / Edit blocks tabs stay above one scrolling content region. The Prompt document uses Sans body-lg, text-hi, distinct heading levels, a bounded reading measure, and the preview surface; Original text retains exact whitespace in Mono. Tools use a searchable selection list beside one complete definition on desktop and stack on tablet/mobile. Block editing uses compact named rows with explicit Edit/Inspect disclosures, independently operated inclusion switches, and a sticky autosave/footer action. Inherited and disabled instructions remain text-hi; badges and a dashed disabled border carry state without fading readable content. Mobile drops redundant identity copy, keeps Agent/Refresh in one row, and puts estimates beneath them. Preview scope explanations stay behind a disclosure.


Skills uses a collection-first manager: the shared Secondary bar lists collections and Agent owners with counts, while the main area leads with collection context, search, a status filter, and a compact Add skills plus icon immediately before search. The header contains no actions and the view has no Refresh button. Category headings share emphasized mono-xs text, an adjacent separator, and indented entries; the navigation has no action footer. Desktop and mobile collections begin with All skills, Global, Bundled, and Shared skills, then Skill locations as a normal folder-icon navigation entry, followed by Agent owners and Projects; Global and Bundled each filter their own origin, without an Other sources group. Results show compact names and explicitly labelled Agent, Project, or source metadata, with at most 50 per page. Descriptions appear only through the shared Quick tooltip on hover/focus/touch; normal availability does not add a badge to every row. Each row exposes Share for Agent-owned Skills, Delete for writable packages, and a master enable switch; these controls act without opening the Skill; Share and Delete use icon Buttons with Quick tooltips and accessible names. The switch hint explains its name-wide effect. Selecting a Skill opens one adjacent reading panel whose name and management actions stay outside the scrolling content; editing is a tertiary action beside the content tabs, and diagnostics remain a disclosure. Shared originals stay in their owner's collection. At tablet/mobile widths the collection index becomes a Dropdown and the selected detail replaces the list, with an explicit Back to list action restoring keyboard focus. Narrow rows wrap their actions, preserving readable names and touch targets. Skill locations opens folder setup with the new-folder field before existing locations; Add skills opens the same destination and focuses that field, preserving an existing directory draft; custom creation is a tertiary action below setup. Skill creation/editing/sharing uses the shared Modal with a padded, height-bounded scrolling body and explicit saves. Creation uses separate name, description, and instruction fields; creation and original-text editing use a wider 760px dialog capped to the viewport. Disabled rows retain readable text contrast.


Swarms uses the shared 216px Secondary bar for reusable Swarms and their Runs, beside one bounded work area. Each Swarm row contains only its name and Agent count in parentheses on one line. Runs are grouped under Active runs (preparing, running, stopping) and Inactive runs (including idle, needs_attention, interrupted and cancelled); each row contains one ellipsized goal title in regular weight. Selecting a Swarm opens its editor directly; creation uses the same Overview / System Prompt / Tools & Skills / Communication anatomy with explicit Save, while saved Swarms autosave and retain an explicit Save action in the fixed footer. The Overview groups Model, Thinking effort and participant count before working directory. The System Prompt tab contains the full editable default, separate switches with content disclosures for all additional blocks, a combined preview for a selected formation Model, separately inspectable Tool definitions, and switches with exact wording for event-triggered Swarm reminders. Draft changes invalidate an older preview; prompt text wraps inside bounded reading areas. Refresh belongs only to the operational view, never the Swarm editor. New run opens a form ordered as Swarm picker, working directory and Goal. The directory starts with the selected Swarm default, resolving a selected Project to its directory; edits apply only to this Run. Selecting a participant opens Activity directly. Extension pages receive a connected-node navigation symbol in both expanded and collapsed Main menus.
