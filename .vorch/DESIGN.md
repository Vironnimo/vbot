...
**Tool Access Policy editor** — `components/tools/ToolAccessEditor.svelte` is the one Agent-policy surface. It leads with three equal intent cards (All Tools, Selected Tools, No Tools), then search and bounded family sections derived only from server registry metadata; unrelated Tools share the presentation-only Individual Tools section and never receive fake singleton families. Each row keeps policy state in a shared `StatusChip` (Included, On, Automatic, Inactive, Off, or Blocked) while readiness/identity/route limitations remain secondary text, so unavailable and automatic Tools do not disappear. Configurable rows expose default/explicit/block choices; automatic rows expose only automatic/default versus block. Family actions say that they affect current members, carry family-specific accessible labels, and never imply future membership. The control uses the shared focus ring, stacks intent cards and row controls below 760px, and autosaves through the owning Agent or Project transition boundary. A nested Project Team override has no Apply button: every policy interaction is the explicit autosaved change, while Reset to repository policy remains a separate deliberate action. The Project Tool Whitelist stays a ceiling editor rather than adopting Agent-policy modes, but `ToggleChipList` groups its Tool chips with the same family labels for comprehension.
