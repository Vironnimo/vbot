...
The Chat Activity surface is a deliberately quiet right edge, not another permanent navigation sidebar. Collapsed, it is a full-height 26px `secondary-surface` rail separated by one structural border and carrying only a centered left chevron; running background work adds one small amber dot. Expanded on desktop, the chevron reverses and a 300px `secondary-surface` panel joins the layout, with a `surface` identity header, Mono-caps section heading, contained task-list scrolling, and compact `surface` task cards. Running automatic-delivery Sub-Agent tasks sort ahead of recent terminal tasks; each card shows Agent, semantic `StatusChip`, task preview, latest Tool or duration when available, direct Child Session navigation, and cancellation only while running. At ≤960px the surface overlays the Chat instead of compressing it; at ≤640px the rail becomes a 40px touch target and the panel is capped at 320px or the remaining viewport width. Escape and either chevron close it, and reduced-motion users receive no entrance or arrow transition.
