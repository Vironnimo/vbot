...
The Session drawer is the deliberate icon-only Badge context: its narrow rows render Channel origin, Subagent, Fork, Cron, and Reflection as fixed 18px square micro-badges with an explicit 11px SVG, quick tooltip, and accessible label instead of visible badge text. Telegram and Discord use recognizable monochrome platform marks, an unknown Channel uses the generic broadcast mark, Subagent uses connected parent/child nodes so it cannot be confused with the separate branch-shaped Fork marker, Cron uses a clock, and Reflection uses a return loop with a spark. These markers stay in one non-wrapping strip after the Session name; unread completion is the shared blue unread dot, without an `Unread` text label.
