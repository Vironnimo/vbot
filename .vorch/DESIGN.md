...
Compact Working blocks: when Appearance selects Compact work details, a contiguous span of at least two Thinking/Tool entries inside one Assistant Run becomes one closed disclosure, while a single Thinking or Tool entry keeps its established inline presentation and visible Assistant prose ends the span. The disclosure summary follows the Tool Call line rather than card language: transparent and borderless, no status dot, lowercase Mono `working...` while current or `done working` after completion, the latest Thinking label or Tool name without arguments or timing, and a muted chevron. Opening it reveals the unchanged nested Thinking and Tool disclosures on the same thin indented left rail as Tool details. Normal mode retains the complete inline sequence.
